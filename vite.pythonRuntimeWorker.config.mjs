...
export default ({ mode }) => {
  ...
  return defineConfig({
    ...
    build: {
      rollupOptions: {
        input: {
          PythonRuntimeWorker: path.resolve(
            __dirname,
            "src/childprocess/PythonRuntimeWorker.ts"
          ),
        },
        output: {
          dir: "dist/childprocess",
          entryFileNames: "PythonRuntimeWorker.js",
          format: "cjs",
        },
        external: ["sqlite3", "better-sqlite3", "bindings", "typeorm"],
      },
      sourcemap: true,
      ssr: true,
    },
