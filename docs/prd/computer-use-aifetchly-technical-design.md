# AiFetchly Computer Use Integration — Technical Design
...
| Date | 2026-09-26 |
| Implementation owner | `aiFetchly` |
| Companion document | [Product requirements](computer-use-aifetchly-prd.md) |
...
These are host implementation anchors. The Hub subsection records external acceptance dependencies, not work assigned to AiFetchly or the plugin repository.
...
### 3.1 Plugin artifact boundary

The independent repository layout and producer implementation are specified in the [plugin technical design](computer-use-plugin-technical-design.md#2-repository-layout). AiFetchly consumes:

| Artifact | Host consumer |
| --- | --- |
| Pinned schemas and fixtures | Generated/validated types, IPC and MCP validation, conformance tests |
| Windows backend package and dependency lock | Managed installation and supervised MCP transport |
| Signed Mac native component | Verified private launch and permission readiness |
| Model manifest/file set and parity report | Reviewed catalog lock, compatibility checks, local runtime installer, built-in model adapter |
| Geometry and W-1 fixtures, benchmark definitions | Host transform/native/E2E and replay tests |

Schemas and fixtures are shared; executable host code is not loaded from plugin packages. The plugin's model exports must match a built-in host adapter and declared processing revisions. No source checkout, compiler, export pipeline, or upstream dependency resolution runs during a host session.
...
Sections 5.1–5.8 specify the host implementation and model evidence intake. Section 5.9 implements the shared grounding contract; future adapters provide their own preprocessing, decoding, and calibration without changing the planner or desktop backends.

### 5.1 Model selection

GUI-Actor-2B is the default, subject to plugin export/parity evidence and host qualification. GUI-Actor-3B remains blocked pending commercial licence clearance. The plugin owns the [model/licence matrix](computer-use-plugin-technical-design.md#61-model-selection), exports, and candidate qualification evidence.

The host offers only allowlisted configurations with supported adapter/architecture, manifest/contract, processing/calibration identity, runtime/provider, app version, memory, and health status (§5.9, §7). A new architecture requires a host adapter implementation; changing a weights path alone is insufficient.

### 5.2 Inference path

`GuiActorOnnxAdapter` implements the plugin's pinned [reference inference path](computer-use-plugin-technical-design.md#62-reference-inference-path): one prefill forward pass with the pointer head, without token-by-token generation. Host TypeScript performs prompt/image preprocessing and output decoding; exported ONNX graphs perform the vision encoder, decoder, and pointer-head computation.

The graph I/O contract, tokenizer/template, pointer token, patch layout, and output interpretation come from the reviewed manifest and reference fixtures. The host implementation must reproduce those fixtures and record its processing revision; it must not infer a model architecture from arbitrary downloaded code.

### 5.3 Export evidence intake

The plugin owns the [ONNX export/parity pipeline](computer-use-plugin-technical-design.md#63-export-pipeline). Before adding an allowlisted configuration to the host release, verify its immutable manifest, licence chain, reference/parity report, graph interfaces, per-file hashes, and declared provider/resource evidence.

Run the host adapter against the plugin's golden tensors and labeled regions under the target Electron/ONNX binding. Reject graph/operator, preprocessing, calibration, or region-hit divergence. Host code updates and plugin model-data updates are independently versioned but must pass this consumer gate together.
...
#### 5.9.1 Contract consumer

The plugin owns the versioned [grounding schema and conformance fixtures](computer-use-plugin-technical-design.md#34-grounding-contract). AiFetchly implements `GroundingModelAdapter` inside its worker and a compile-time `GroundingAdapterRegistry`; the registry maps approved IDs to app-bundled implementations. Capabilities, load, ground, cancel, and unload obey the pinned shared contract.

`GuiActorOnnxAdapter` wraps §5.2–5.8. A test-only second adapter exercises interchangeability without promising a second production model. The worker cannot execute desktop input, access the database, authorize actions, send network requests, or convert to desktop coordinates.

#### 5.9.2 Boundary validation

Validate the shared envelope and loaded identity, request/generation/observation IDs, image byte/dimension limits, crop bounds, candidate counts, finite geometry, declared coordinate space, and preprocessing transform at the worker boundary and before creating a target handle. Reject unsupported capabilities or malformed output with the shared typed errors. Keep model-specific decoding/calibration semantics; never interpret scores as comparable accuracy.

The host validates and applies transforms (§11), owns target handles, and records the complete identity in trace metadata. Absent/ambiguous output cannot dispatch input. An adapter without qualified absence handling cannot be selected for autonomous use. Cancellation suppresses results immediately; unresponsive inference is terminated by the supervisor independently of the worker's cooperative cancellation support.
...
### 6.1 Windows backend consumption

The plugin supplies the pinned [Windows-MCP backend](computer-use-plugin-technical-design.md#4-windows-backend), reviewed tool subset, dependency lock, and integrity/window-at-point tools. AiFetchly installs it using §8 and launches a persistent private MCP session.

- Filter both `tools/list` and `tools/call`; allow only the reviewed observation/capture/focus/input primitives required by the host. Raw backend tools never reach the planner; shell, registry, filesystem, process-management, and clipboard tools are unreachable.
- Normalize bounded UI Automation observations and capture geometry to the shared schema. Validate target ownership and foreground state before dispatch.
- Use `process_integrity` for target and host elevation checks and window-at-point for own-window/elevation rejection (§10.5). Unknown integrity fails closed. Do not add host FFI solely for these operations.
- Configure and verify the pinned backend's disabled telemetry; exercise CJK and non-English application names in consumer integration.

### 6.2 macOS backend consumption

The plugin owns the Phase 0 Ghost OS fork versus in-house Swift helper comparison and the selected native backend. Consume its [Mac package and backend contract](computer-use-plugin-technical-design.md#5-macos-backend); do not build a second host-side helper. The decision stays open until the spike passes geometry, CJK, cancellation, packaging, and maintenance criteria.

Both choices use the host's shared ONNX grounder. Plugin-owned CU-MAC-01 and CU-MAC-03 guarantee a managed native package and disabled internal vision/workflows; host CU-MAC-02, CU-MAC-04, and CU-MAC-05 enforce private access, transforms, and integrated permission/stop behavior.

#### 6.2.1 Managed helper launch

Resolve the verified native-component executable by absolute path from the Hub install plan. For Ghost, launch argv `["mcp"]` with `shell: false`, sanitized environment, app-owned cwd, private persistent stdio, bounded stderr, and whole-tree ownership. Verify contract/build/signing identity before activation. Never run `ghost setup`, Homebrew, a global installer, or another MCP client's configuration workflow.

Qualify Accessibility and Screen Recording attribution in signed AiFetchly.app and after updates; keep signing identity stable. Development attribution to a terminal/IDE is not release evidence. Learning is disabled; Input Monitoring for future recording is not required by this design. Permission status and recovery live in AiFetchly's UI.

#### 6.2.2 Host enforcement and qualification

Filter tool discovery and calls independently of backend filtering. Reject any capability outside the reviewed atomic observation/input subset. Backend errors must return to the supervisor instead of invoking internal vision, recipes, or unapproved alternate execution paths.

Validate captured window identity, image dimensions, logical frame/origin, resize/scale, and desktop units; apply §11 instead of calling Ghost's grounder for conversion. Observation must not unexpectedly change focus. A focus or target change invalidates the observation and requires revalidation.

Do not depend on an MCP cancel message interrupting a synchronous native call. Revoke host grants immediately, supervise deadlines/termination independently, and verify backend dispatch timestamps against the stop gate. A failing packaged helper is not enabled for input even if its standalone contract tests passed.

### 6.3 Target workflow W-1: Excel lead list

W-1 ([AiFetchly PRD workflow](computer-use-aifetchly-prd.md#31-target-workflow)) drives adapter qualification, fixtures, and the benchmark for Phases 1 and 2.
...
Two runtime groups drive host UI listing: `local-ai` (`embedding-xenova`, `voice-sherpa`) and `computer-use` (the grounding IDs).

### 7.5 Model file-set consumption

Consume the plugin's [model package format](computer-use-plugin-technical-design.md#64-model-package-format), including graph files, external-data shards, tokenizer/preprocessing, health fixture, licences, and immutable manifest. The host stores each configuration under its runtime ID/version with an `active.json` pointer. It validates the producer manifest against catalog v2 (§7.3); it does not export or quantize models.
...
## 9. Shared tool contract consumption

The plugin repository owns [public-tool and record schemas](computer-use-plugin-technical-design.md#3-shared-contracts). Pin a released schema/fixture artifact by version and digest. Generate TypeScript types or validate them against that schema; host application code uses explicit return types and `unknown`, never `any`.

### 9.1 Host tool routing

| Shared tool group | Host implementation |
| --- | --- |
| Capabilities, session start/stop, handoff/resume | Supervisor, lease, generation, AI/permission gates, backend readiness |
| Observe/find | Backend observations plus selected grounding adapter; create opaque host-owned target handles |
| Act/verify | Validate current target and grant, dispatch one authorized action, verify and report uncertainty |

The host implements these model-visible tools; the plugin backend exposes only private primitives. Tool descriptions from the plugin cannot alter authority or permit raw desktop coordinates. Keyboard-only actions still need a current session, target focus, and authorization.

### 9.2 Shared records and target store

Consume `ObservationMetadata`, `GroundingMetadata`, `ResolvedTarget`, and `ActionOutcome` from the pinned [record contract](computer-use-plugin-technical-design.md#32-core-records). Keep native identifiers, transforms, coordinates, generation, expiry, and grants in the bounded supervisor store. Never accept grants or raw image bytes as model-supplied authority fields or persist them in ordinary tool JSON.

### 9.3 Error normalization

Normalize backend/worker failures to the [shared error codes](computer-use-plugin-technical-design.md#33-errors), carrying a safe message, stage, retry classification, and correlation ID. Add the current `GroundingReadiness` when returning `model_not_ready`. No traceback, secrets, absolute path, argv, or screenshot bytes reaches renderer error text. A timeout after input is `execution_uncertain`, not permission to retry.
...
Offline replay reuses the same grounding worker and TypeScript transform code as live runs. The plugin repository's separately implemented `eval` CLI can also replay bundles against the PyTorch reference to separate export errors from model errors.

### 13.2 Trace bundle consumption

Write reviewed diagnostic evidence using the plugin's [versioned trace bundle schema](computer-use-plugin-technical-design.md#36-trace-bundle). Record complete adapter/model/runtime/provider/processing identity, geometry, dispatch timing, outcomes, and foreground state. Images remain opt-in; typed text is redacted by default.

Replay uses the same host worker and transforms as live grounding with desktop adapters disabled. Exact replay needs matching compatible installed artifacts; missing revisions fail explicitly. A run with a different model is labelled as a comparison. Host storage, retention, reviewed export, and database summaries remain host features.
...
### 14.1 Plugin evidence intake

Consume the plugin's pinned schema/geometry/model fixtures and its [producer test reports](computer-use-plugin-technical-design.md#8-tests-and-evaluation). Verify the release's source, artifact, schema and licence identities. The host release gate reruns consumer conformance against the actual installed backend/model and does not substitute a plugin CI result for native integrated safety tests.
...
### 14.5 Integrated benchmark execution

Run the plugin's versioned [W-1 benchmark](computer-use-plugin-technical-design.md#82-benchmark) with the host implementation in accessibility-only, grounder-only, combined, and visual-planner arms. Preserve synthetic lead rows, labeled controls, absent-target cases, hardware/configuration identities, and held-out separation.

AiFetchly owns the integrated report and acceptance against its PRD §8: task success, handoffs, mismatches, unauthorized actions, no input after stop, planner cost, and cold/warm step timing. Plugin backend/model reports are inputs to this report. A fixture/model release alone is not end-to-end qualification.

## 15. Change map

Only the Desktop row is implemented in this repository. Plugin and Hub rows are explicit release dependencies; their producer implementation belongs in their own repositories.
...
These are integrated host milestones. Schema releases, backend packages, model exports, fixtures, and Mac spike reports are supplied by the [plugin release sequence](computer-use-plugin-technical-design.md#9-release-and-handoff); the host implements their consumers and accepts the joint evidence.

| Phase | Host implementation | Required producer handoff |
| --- | --- | --- |
| 0 | MCP SDK transport/session pool; generated schema consumers and conformance tests; supervisor/worker prototypes; Electron ONNX binding/provider checks; measurement harness and baseline runner | Plugin contract/fixture release, W-1 fixtures, Windows exposure/integrity prototype, export/sharding/parity/resource report, Mac backend comparison, and Windows delivery decision |
| 1 | Windows provisioning, supervisor, lease/grants, stop/hotkey/strip/overlay, own/elevated-window checks, one display, opt-in visual planner, W-1 accessibility execution and measured baseline | Plugin pinned Windows package/lock; Hub install plan |
| 2 | Catalog v2/dual publication, file-set downloader, consent install sets, leases/pruning, probes, win32-x64 ONNX binding, built-in GUI-Actor/test adapters and registry, between-session selection, crops, calibrated output validation, locate-only/replay/viewer | Plugin immutable default model package, processing/calibration identities, parity/health/absence fixtures and reports |
| 3 | Private Mac helper launch/permissions, CoreML/CPU runtime package, geometry and Mac safety-control integration, packaged-app qualification | Plugin selected signed/notarized helper and CU-MAC-01/03 evidence; Hub target-aware native-component plan |
| 4 | Update/repair, clean-machine matrix, published compatibility and measured SLOs | Joint release evidence for the pinned artifact combination |
| 5 | Separately approved multi-display/vendor-provider/model-tier/offline consumer support | Independently qualified producer artifacts and Hub dependencies |
...
Backend source/packaging, model export configuration, and model licence decisions are plugin-owned inputs. Host planner/routing, local runtime consumption, safety/UI, and integrated SLO decisions are owned here; distribution decisions require Hub coordination.
...
Resolved in version 1.3: host-implemented grounding adapters against the shared contract and between-session selection (§5.9), and the conditional Ghost OS packaging/integration plan (§6.2). Additional production models still need qualification; the Mac backend winner remains open.
...
This table maps host-owned requirements from the [AiFetchly PRD](computer-use-aifetchly-prd.md). Producer-owned CU-INST-11, CU-MAC-01, and CU-MAC-03 are mapped in the plugin technical design; their artifacts/evidence are prerequisites for host acceptance.
...
| W-1 workflow ([AiFetchly PRD](computer-use-aifetchly-prd.md#31-target-workflow)) | §6.3, §14.5 | Excel fixture cases and benchmark arms |
...
| CU-MAC-02 | §4.1, §6.2, §9 | Private launch and host discovery/invocation allowlists |
| CU-MAC-04–05 | §6.2.2, §10, §11, §14.3 | Native geometry, packaged permissions, CJK, stop and cleanup gates |
