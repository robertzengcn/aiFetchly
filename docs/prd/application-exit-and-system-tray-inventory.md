...
| Bulk email send | Retry stays user-initiated | `BuckEmailTaskModule.reconcileInterruptedTasks` (durable-tasks participant, BEFORE the force kill): Processing rows → existing **Error** + `[interrupted]` note in the error file; completed/not-started untouched | untouched | **send outcome may be unknown** if killed mid-SMTP — recorded as interrupted/not-sent; blind retry prevented because batches require user initiation |
| Social tasks | Retry stays user-initiated (next run is a NEW taskrun row) | `reconcileInterruptedSocialRuns` (durable-tasks participant): module-level live-run registry (the run ENTITY has no status column) logs an interruption marker per live run and clears the registry | untouched | post/publish outcome may be unknown — marker records interruption; no auto-resume |
