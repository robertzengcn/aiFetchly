...
The scoped slash-command MVP and Claude-compatible prompt command loading are mostly implemented. This TODO tracked the remaining gaps found during the PRD implementation audit. **All five items are now resolved** (see per-section resolution notes).
