...
- **D-TrustUX [informational] (Phase 17, locked):** binary workspace approval = all capabilities; `trust.skills` writes as a block on approval. The per-capability entity already has the `skills` column.
- **D-Vocabulary [informational] skill-ref (Phase 17, locked):** the `skill:<name>` sentinel command is the marker Phase 18 detects (Phase 17 registered it; Phase 18 resolves it).
