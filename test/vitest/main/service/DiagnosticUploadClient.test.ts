...
import { projectToWirePayload } from "@/modules/diagnostics/CrashReportWireSchema";
...
describe("DiagnosticUploadClient", () => {
  ...
  test("returns reportId on 200", async () => {
    ...
    // The server rejects unknown fields with 400, so the posted body must be
    // the slim wire payload — never the rich CrashRecord.
    const postedBody = fakeHttp.post.mock.calls[0][1] as Record<
      string,
      unknown
    >;
    expect(postedBody.crash).not.toHaveProperty("crashId");
  });
  ...
  test("surfaces 429 as rate-limit error", async () => {
    const c = new DiagnosticUploadClient({
      ...
      http: {
        post: vi.fn().mockResolvedValue({
          status: 429,
          data: { status: false, msg: "slow down" },
        }),
      },
    });
    const res = await c.upload(basePkg);
