...
describe("DiagnosticReportBuilder", () => {
  ...
  test("builds package with crash + recent errors + breadcrumbs", () => {
    ...
    // Assert redaction on the crash message this test controls — NOT on the
    // whole serialized package. buildUploadPackage now attaches mainLogTail
    // (today's real main.log tail), which contains log lines from OTHER
    // concurrently running tests and is not deterministic in a suite run.
    expect(pkg!.crash.message).not.toContain("secret");
    expect(pkg!.crash.message).toContain("[REDACTED]");
  });
  ...
  test("package size stays under 200KB by default", () => {
    ...
    expect(pkg).not.toBeNull();
    expect(Buffer.byteLength(JSON.stringify(pkg))).toBeLessThanOrEqual(
      200 * 1024
    );
  });
