...
describe("resolveContainedPath", () => {
  ...
  test.each([
    ...
    expect(() => resolveContainedPath(ROOT, ...segs)).toThrow(
      LocalAiRuntimeError
    );
    try {
...
describe("LocalAiRuntimePathService", () => {
  ...
  test("builds per-version paths beneath runtime dir", () => {
    ...
    expect(p.packageManifestPath).toBe(
      path.join(p.versionRoot, "manifest.json")
    );
  });
  ...
  test("operation paths live in hidden siblings, not version dirs", () => {
    ...
    expect(op.archivePath).toBe(
      path.join(
        ...
        "local-ai-runtimes",
        ".downloads",
        "11111111-2222-3333-4444-555555555555.zip.part"
      )
    );
  ...
  test("ensureOperationDirectories creates download and staging roots", async () => {
    const userData = path.join(ROOT, "ensure-op-dirs");
    const svc = new LocalAiRuntimePathService(userData);
    await svc.ensureOperationDirectories();
    await expect(access(svc.downloadsRoot)).resolves.toBeUndefined();
    await expect(access(svc.stagingRoot)).resolves.toBeUndefined();
  });

  test("rejects unknown runtime id", () => {
    const svc = new LocalAiRuntimePathService(ROOT);
    expect(() => svc.getRuntimePaths("evil" as never, "1.0.0")).toThrow(
      LocalAiRuntimeError
    );
  });
  ...
  test("rejects invalid runtime version (path-separator / non-semver)", () => {
    const svc = new LocalAiRuntimePathService(ROOT);
    expect(() => svc.getRuntimePaths("voice-sherpa", "../../x")).toThrow(
      LocalAiRuntimeError
    );
    expect(() => svc.getRuntimePaths("voice-sherpa", "not-a-version")).toThrow(
      LocalAiRuntimeError
    );
  });
  ...
  test("rejects malformed operation id", () => {
    const svc = new LocalAiRuntimePathService(ROOT);
    expect(() => svc.createOperationPaths("not-a-uuid")).toThrow(
      LocalAiRuntimeError
    );
    expect(() => svc.createOperationPaths("../../pwn")).toThrow(
      LocalAiRuntimeError
    );
  });
  ...
  test("isBeneathRuntimeRoot detects escapes", () => {
    ...
    expect(svc.isBeneathRuntimeRoot(path.join(svc.runtimeRoot, "x"))).toBe(
      true
    );
