...
const handlers = new Map<string, (event: unknown, arg: unknown) => Promise<unknown>>()
...
const tokenGetValue = vi.fn<(key: string) => string>()
...
import { registerValidatedHandler, registerAiValidatedHandler } from '@/main-process/communication/_shared/registerValidatedHandler'
...
async function invoke(channel: string, arg: unknown): Promise<{ status: boolean; msg: string; data: unknown }> {
...
describe('registerValidatedHandler', () => {
  it('registers a handler that accepts valid input', async () => {
    ...
    registerValidatedHandler('valid:ch', schema, async (input) => `got-${input.x}`)
...
describe('registerAiValidatedHandler', () => {
  ...
  it('proceeds (and validates) when USER_AI_ENABLED is "true"', async () => {
    ...
    const schema = lazySchema(() => z.strictObject({ q: z.string() }))
    registerAiValidatedHandler('ai:ok', schema, async (input) => `ai:${input.q}`)
