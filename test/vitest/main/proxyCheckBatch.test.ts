...
function makeControllerWith(overrides: {
  ...
  checkProxy?: (proxy: { host: string }, timeout?: number) => Promise<ProxyCheckres>;
  checkGooglePass?: (proxy: { host: string }, timeout?: number) => Promise<boolean>;
}): ProxyController {
...
describe("ProxyController.checkProxyBatch", () => {
  ...
  it("records a setup error for a proxy missing protocol", async () => {
    const controller = makeControllerWith({
      proxyApi: {
        getProxyDetail: async (id) => ({
          status: true,
          code: 200,
          msg: "ok",
          // protocol intentionally missing
          data: { id, host: "h", port: "80", protocol: "" },
        }),
      },
      checkProxy: vi.fn(async () => ({ status: true, msg: "", data: true })),
    });
