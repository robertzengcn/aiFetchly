...
describe("PageStateCapture", () => {
  ...
  it("should generate unique request IDs", async () => {
    const result1 = await capturePageState(mockPage, "op1", "google", "error", [
      ".sel",
    ]);

    const result2 = await capturePageState(mockPage, "op2", "google", "error", [
      ".sel",
    ]);
  ...
  it("should handle screenshot failure gracefully", async () => {
    ...
    // capturePageState logs through the shared Logger (createLogger),
    // which prefixes the message: "[timestamp] [WARN] [PageStateCapture]
    // Failed to capture screenshot:". Assert on the message suffix, not
    // the exact string.
  ...
  it("should handle accessibility tree failure gracefully", async () => {
    ...
    // Same Logger-prefix format as the screenshot failure above.
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("Failed to get accessibility tree:"),
      expect.any(Error)
    );
