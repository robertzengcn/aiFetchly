...
describe("outbound email completion", () => {
  ...
  it("reports a missing SMTP service as a send failure", async () => {
    ...
    expect(failure).toHaveBeenCalledWith(
      ...
      "Campaign subject",
      "<p>Body</p>",
      // FR-014: no service was resolved, so the identity metadata is empty.
      {}
    );
  });
  ...
  it("reports SMTP 535 Invalid login as a send failure without unhandledRejection", async () => {
    ...
    expect(failure).toHaveBeenCalledWith(
      ...
      "Campaign subject",
      "<p>Body</p>",
      // FR-014: identity metadata from the resolved service row.
      {
      ...
      // §19.2: the failure classifier tags auth failures specifically.
      "smtp_auth_failed"
    );
  });
});
