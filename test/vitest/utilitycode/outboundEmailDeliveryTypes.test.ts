...
describe("outboundEmailDeliveryTypes schemas", () => {
  it("accepts the three delivery modes and rejects others", () => {
    ...
    expect(outboundEmailDeliveryModeSchema.parse("review_first")).toBe(
      "review_first"
    );
    expect(outboundEmailDeliveryModeSchema.parse("draft_only")).toBe(
      "draft_only"
    );
  ...
  it("accepts draft, authorization, attempt, and recipient outcome statuses", () => {
    ...
    expect(
      outboundEmailAuthorizationTypeSchema.parse("explicit_user_instruction")
    ).toBe("explicit_user_instruction");
    expect(
      outboundEmailAuthorizationTypeSchema.parse("exact_draft_approval")
    ).toBe("exact_draft_approval");
    expect(outboundEmailSendAttemptStatusSchema.parse("claimed")).toBe(
      "claimed"
    );
    expect(outboundEmailRecipientOutcomeStatusSchema.parse("suppressed")).toBe(
      "suppressed"
    );
    expect(() =>
      outboundEmailRecipientOutcomeStatusSchema.parse("bounced")
    ).toThrow();
  });
  ...
  it("validates an authorized envelope requires a 64-char hash and nullable html", () => {
    const base = {
      ...
      emailServiceId: 3,
      senderAddress: "s@b.com",
      subject: "Hi",
      bodyText: "Body",
      bodyHtml: null,
      envelopeHash: "x".repeat(64),
    };
    expect(authorizedOutboundEnvelopeSchema.safeParse(base).success).toBe(true);
    expect(
      authorizedOutboundEnvelopeSchema.safeParse({
        ...base,
        envelopeHash: "short",
      }).success
    ).toBe(false);
  ...
  it("validates the v2 worker payload discriminated shape", () => {
    const payload = {
      ...
      batchId: 1,
      sendAttemptId: 2,
      batchHash: "h".repeat(64),
      envelopes: [],
      emailServices: [],
    };
    expect(
      authorizedEmailWorkerPayloadV2Schema.safeParse(payload).success
    ).toBe(true);
    expect(
      authorizedEmailWorkerPayloadV2Schema.safeParse({ ...payload, version: 1 })
        .success
    ).toBe(false);
    expect(
      authorizedEmailWorkerPayloadV2Schema.safeParse({
        ...payload,
        mode: "legacy",
      }).success
    ).toBe(false);
  ...
  it("validates the three discriminated worker→main event shapes", () => {
    const base = {
      batchId: 1,
      sendAttemptId: 2,
      draftId: 3,
      revisionId: 4,
      envelopeHash: "h".repeat(64),
    };
    const submitted = {
    ...
    const failed = {
      type: "authorized-email-failed" as const,
      ...base,
      errorCode: "smtp_rejected",
      retrySafety: "safe" as const,
    };
    const complete = {
      type: "authorized-email-worker-complete" as const,
      batchId: 1,
      sendAttemptId: 2,
    };
    expect(authorizedEmailWorkerEventSchema.safeParse(submitted).success).toBe(
      true
    );
    expect(authorizedEmailWorkerEventSchema.safeParse(failed).success).toBe(
      true
    );
    expect(authorizedEmailWorkerEventSchema.safeParse(complete).success).toBe(
      true
    );
  });

  it("rejects worker→main events that fail the trust boundary shape", () => {
    const base = {
      batchId: 1,
      sendAttemptId: 2,
      draftId: 3,
    ...
    // Unknown event type.
    expect(
      authorizedEmailWorkerEventSchema.safeParse({
        type: "authorized-email-unknown",
        ...base,
      }).success
    ).toBe(false);
    // Wrong envelope hash length (must be 64).
    expect(
      authorizedEmailWorkerEventSchema.safeParse({
    ...
    // Complete event must carry integer ids.
    expect(
      authorizedEmailWorkerEventSchema.safeParse({
        type: "authorized-email-worker-complete",
        batchId: "not-a-number",
        sendAttemptId: 2,
      }).success
    ).toBe(false);
  });
