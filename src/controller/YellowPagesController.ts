...
export class YellowPagesController {
  ...
  async createTask(taskData: YellowPagesTaskData): Promise<number> {
    try {
      ...
      const platform = await this.platformModule.getPlatformByName(
        taskData.platform
      );
      if (!platform) {
      ...
      if (taskData.account_id) {
        const accountCookies =
          await this.accountCookiesModule.getAccountCookies(
            taskData.account_id
          );
        if (!accountCookies) {
          console.warn(
  ...
  ): Promise<PaginatedResponse<YellowPagesResult>> {
    try {
      ...
      const results = await this.resultModule.getResultsByTaskId(
        taskId,
        page,
        size
      );
  ...
  private async validateTaskData(taskData: YellowPagesTaskData): Promise<void> {
    ...
    if (
      taskData.delay_between_requests &&
      taskData.delay_between_requests < 0
    ) {
    ...
    if (
      taskData.headless !== undefined &&
      typeof taskData.headless !== "boolean"
    ) {
      throw new Error("Headless parameter must be a boolean value");
    }

    // Validate local browser selection if provided (empty string means "disabled")
    if (taskData.localBrowser !== undefined) {
      const selected = String(taskData.localBrowser).trim();
      if (
        selected.length > 0 &&
        selected !== "chrome" &&
        selected !== "firefox"
      ) {
        throw new Error("localBrowser must be 'chrome' or 'firefox'");
      }
    }
    ...
    if (
      platform &&
      platform.settings?.authentication?.requiresCookies &&
      !taskData.account_id
    ) {
      throw new Error(
        `Platform '${platform.display_name}' requires cookies for authentication. Please select an account.`
      );
    }
  }
  ...
  private convertTaskUpdatesToEntity(updates: Partial<YellowPagesTask>): any {
    ...
    if (updates.platform !== undefined)
      entityUpdates.platform = updates.platform;
    if (updates.keywords !== undefined)
      entityUpdates.keywords = JSON.stringify(updates.keywords);
    if (updates.location !== undefined)
      entityUpdates.location = updates.location;
    if (updates.max_pages !== undefined)
      entityUpdates.max_pages = updates.max_pages;
    if (updates.concurrency !== undefined)
      entityUpdates.concurrency = updates.concurrency;
    if (updates.status !== undefined) entityUpdates.status = updates.status;
    if (updates.delay_between_requests !== undefined)
      entityUpdates.delay_between_requests = updates.delay_between_requests;
    if (updates.account_id !== undefined)
      entityUpdates.account_id = updates.account_id;
    if (updates.proxy_config !== undefined)
      entityUpdates.proxy_config = updates.proxy_config
        ? JSON.stringify(updates.proxy_config)
        : null;
    if (updates.headless !== undefined)
      entityUpdates.headless = updates.headless;
    if (updates.aiSupportEnabled !== undefined)
      entityUpdates.ai_support_enabled = updates.aiSupportEnabled;
    if (updates.localBrowser !== undefined)
      entityUpdates.local_browser = updates.localBrowser
        ? updates.localBrowser
        : null;
    if (updates.scheduled_at !== undefined)
      entityUpdates.scheduled_at = updates.scheduled_at;
    if (updates.completed_at !== undefined)
      entityUpdates.completed_at = updates.completed_at;
    if (updates.error_log !== undefined)
      entityUpdates.error_log = updates.error_log;
