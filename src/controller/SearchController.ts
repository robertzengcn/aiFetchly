...
import {
  SearchDataParam,
  SearchResEntityDisplay,
  SearchResEntityRecord,
  SearchResEntity,
...
export class SearchController {
  ...
  public async searchData(data: Usersearchdata) {
    ...
    // Validate search prerequisites (e.g., Google requires account with cookies)
    await this.searchModel.validateSearchPrerequisites(data.searchEnginer);
  ...
  public async listSearchresult(
    ...
    // await seModel.init();
    const res = await this.searchModel.listSearchtask(
      page,
      size,
      sortBy,
      search
  ...
  public async listtaskSearchResult(
    taskId: number,
    page: number,
    size: number,
    search?: string
  ): Promise<SearchResEntityRecord> {
    // const seModel=new searhModel()
    const res = await this.searchModel.listSearchResult(
      taskId,
      page,
      size,
      search
    ...
    await Promise.all(
      res.map(async (item) => {
        ...
        const keyEntity = await this.searchModel.getkeywrodsEntitybyId(
          item.keyword_id
        );
    ...
    if (search && search.trim().length > 0) {
      ...
      const allResults =
        await this.searchResultModule.getAllSearchResultsByTaskId(taskId);
  ...
  public async retryTask(taskId: number): Promise<void> {
    ...
    // Validate search prerequisites before retrying (e.g., Google requires account with cookies)
    const engineName = this.searchModel.convertNumtoSE(
      Number(taskEntity.engine)
    );
    if (engineName) {
      await this.searchModel.validateSearchPrerequisites(engineName);
    }
  ...
  public async updateSearchTask(
    ...
    if (
      updates.num_pages !== undefined &&
      (updates.num_pages < 1 || updates.num_pages > 100)
    ) {
    ...
    if (
      ...
      (updates.concurrency < 1 || updates.concurrency > 10)
    ) {
   ...
   */
  public async exportSearchResults(
    taskId: number,
    format: "json" | "csv" = "csv"
  ): Promise<any> {
    if (!taskId || taskId <= 0) {
  ...
  public async killProcessByPID(pid: number): Promise<{
    ...
    try {
      ...
      if (taskId) {
        try {
          await this.searchModel.updateTaskStatus(
            taskId,
