...
export class ExtraModuleController {
  ...
  public async getExtraModuleList(
    offerset: number,
    length: number
    ...
    for (const module of this.extramodules) {
      ...
      if (module.installed) {
        ...
        const instRes = await this.extraModulesModule.getExtraModuleByName(
          module.name
        );
        if (instRes) {
  ...
  public async installExtraModule(
    packagename: string,
    success?: () => void,
    strerr?: (message: Error) => void
  ) {
    //valid package name
    const valid = this.extramodules.find(
      (module) => module.packagename === packagename
    );
    if (!valid) {
    ...
    if (valid.ispip) {
      ...
      await this.downloadInstallPackage(
        valid.packagename,
        valid.link,
        () => {
          const module = new ExtraModuleEntity();
      ...
      fs.stat(this.extraModulePth, (err, stats) => {
        if (err) {
          if (err.code === "ENOENT") {
            ...
            fs.promises
              .mkdir(this.extraModulePth, { recursive: true })
              .then(() => resolve())
              .catch(reject);
          } else {
            reject(err);
  ...
  public async downloadInstallPackage(
    packagename: string,
    installLink: string,
    success?: () => void,
    strerr?: (message: Error) => void
  ...
  //remove modules
  public removeExtraModule(
    packagename: string,
    success?: () => void,
    strerr?: (message: string) => void,
    strout?: (message: string) => void
  ) {
    //valid package name
    const valid = this.extramodules.find(
      (module) => module.packagename === packagename
  ...
  public async isPipModuleInstalled(moduleName: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      // Security: pass arguments as an array to execFile so moduleName
      // can never be interpreted as shell syntax (command injection).
      execFile("pip", ["show", moduleName], (error, stdout, stderr) => {
        if (stderr) {
  ...
  public async installPipPackage(
    packageName: string,
    version?: string
  ): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // can never be interpreted as shell syntax (command injection).
      execFile("pip", ["install", packageName], (error, stdout, stderr) => {
        if (error) {
  ...
  public async checkRequirement(toolName = "openai-whisper"): Promise<boolean> {
    ...
    if (!dPackage) {
      throw new CustomError(
        toolName + " package not installed",
        20250120152922
      );
    }
    if (dPackage.requirePy) {
      ...
      if (!res) {
        throw new CustomError(
          "the caption tool must install python first",
          20250120152928
        );
      }
    }
    ...
    if (!res) {
      throw new CustomError(
        "generate capation must install " + toolName,
        2024120511189
      );
    }
    return true;
