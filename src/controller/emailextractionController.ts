...
export class EmailextractionController {
   ...
   */
  public async startEmailSearchTask(taskId: number): Promise<void> {
    const task = await this.emailSeachTaskModule.getTaskDetail(taskId);
    if (!task) {
      throw new Error("Task not found");
    }
    await this.emailSeachTaskModule.startTask(taskId);
  }
  ...
  public async listEmailSearchtasks(
    page: number,
    size: number,
    sortby?: SortBy
  ): Promise<{ records: EmailsearchTaskEntityDisplay[]; total: number }> {
    const res = await this.emailSeachTaskModule.listSearchtask(
      page,
      size,
      sortby
    );
    ...
    for (let i = 0; i < res.records.length; i++) {
      ...
      if (value.id) {
        const taskStatus = this.emailSeachTaskModule.taskstatusConvert(
          value.status
        );
        const taskType = this.emailSeachTaskModule.taskTypeconvert(
          value.type_id
        );
        const urls = await this.emailSeachTaskModule.getTaskurls(
          value.id,
          0,
          10
        );
        const displayValue: EmailsearchTaskEntityDisplay = {
          id: value.id,
          record_time: value.createdAt
            ? this.formatDateTime(value.createdAt)
            : undefined,
  ...
  public async Emailtaskresult(
    taskId: number,
    page: number,
    size: number
  ): Promise<EmailResultDisplay[]> {
    const res = await this.emailSeachTaskModule.getTaskResult(
      taskId,
      page,
      size
    );
   ...
   */
  public async exportEmailResults(
    taskId: number,
    format: "json" | "csv" = "csv"
  ): Promise<any> {
    if (!taskId || taskId <= 0) {
    ...
    const allResults = await this.emailSeachTaskModule.getAllTaskResults(
      taskId
    );
   ...
   */
  public async updateEmailSearchTask(
    taskId: number,
