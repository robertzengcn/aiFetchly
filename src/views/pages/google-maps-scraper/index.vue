      ...
      <v-window-item value="history">
        <v-card>
          <v-card-title class="d-flex align-center">
            {{ t('googleMaps.history_tab') || 'History' }}
            <v-spacer />
            <v-btn
              icon="mdi-refresh"
              variant="text"
          ...
          >
            ...
            <template #item.actions="{ item }">
              ...
              >
                {{ t('googleMaps.view_results') || 'View' }}
              </v-btn>
              <v-btn
                size="small"
                variant="text"
              ...
              >
                {{ t('googleMaps.delete_btn') || 'Delete' }}
              </v-btn>
            </template>
          </v-data-table>
          <v-card-text v-else-if="!historyLoading" class="text-center py-8">
            ...
            <p class="text-h6 mt-4 text-medium-emphasis">
              {{ t('googleMaps.history_empty') || 'No search history yet' }}
            </p>
          </v-card-text>
        </v-card>
      </v-window-item>
...
// ── History state ───────────────────────────────────────────────────────
const historyRecords = ref<GoogleMapsHistoryRecord[]>([]);
...
const historyHeaders = computed(() => [
  { title: t("googleMaps.col_name") || "Query", key: "query", sortable: true },
  { title: t("googleMaps.col_address") || "Location", key: "location", sortable: true },
  { title: t("googleMaps.col_reviews") || "Results", key: "totalResults", sortable: true },
  { title: t("googleMaps.col_date") || "Date", key: "createdAt", sortable: true },
  { title: t("googleMaps.col_actions") || "Actions", key: "actions", sortable: false },
]);
...
onUnmounted(() => {
  ...
  if (requestId.value && searchState.value === "running") {
    cancelGoogleMapsSearch(requestId.value).catch(() => { /* worker may already have exited */ });
  }
});
</script>
