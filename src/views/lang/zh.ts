export default {
  common: {
    more: "更多",
    actions: "操作",
    confirmation: "确认",
    cancel: "取消",
    confirm: "确认",
    installing: "安装中",
    uninstalling: "卸载中",
    reset: "重置",
    submit: "提交",
    save: "保存",
    update: "更新",
    success: "成功",
    fill_require_field: "请填写所有必填字段",
    record_time: "记录时间",
    return: "返回",
    created_time: "创建时间",
    created_at: "创建时间",
    delete_item_failure: "删除项目失败",
    error: "错误",
    yes: "是",
    no: "否",
    ok: "确定",
    save_success: "保存成功",
    save_fail: "保存失败",
    delete_confirm_tip: "您确定要删除吗",
    comfirm_delete: "确认删除",
    single_choose: "单选",
    multiple_choice: "多选",
    test: "测试",
    unkonw_error: "未知错误",
    send: "发送",
    loading: "加载中",
    loading_task_data: "正在加载任务数据...",
    change_proxy: "更换代理",
    select_proxy: "选择代理",
    id: "ID",
    status: "状态",
    configured: "已配置",
    not_configured: "未配置",
    not_start: "未开始",
    pending: "待处理",
    processing: "处理中",
    complete: "完成",
    unknown: "未知",
    error_retry: "发生错误，请重试",
    install: "安装",
    uninstall: "卸载",
    upgrade: "升级",
    upgrading: "升级中",
    log_content: "日志内容",
    optional: "可选",
    import: "导入",
    export: "导出",
    clear_filters: "清除筛选",
    columns: "列",
    information: "信息",
    close: "关闭",
    refresh: "刷新",
    page: "页",
    of: "共",
    export_success: "导出成功",
    export_failed: "导出失败",
    export_cancelled: "导出已取消",
    exporting: "导出中...",
    download_template: "下载模板",
    drag_drop_file: "拖拽文件到此处或点击选择文件",
    search: "Search",
    replace: "替换",
  },
  route: {
    dashboard: "仪表板",
    insights: "洞察",
    home: "首页",
    statistic: "数据统计",
    system_setting: "系统设置",
    campaign: "活动",
    campaign_list: "活动列表",
    edit_campaign: "编辑活动",
    social_task: "社交任务",
    social_task_list: "社交任务列表",
    edit_social_task: "编辑社交任务",
    create_social_task: "创建社交任务",
    run_task: "运行任务",
    task_run_list: "任务运行列表",
    task_result_list: "任务结果列表",
    social_account: "工具账户",
    account_list: "工具账户列表",
    edit_account: "编辑工具账户",
    add_account: "添加工具账户",
    tool_account: "工具账户",
    tool_account_list: "工具账户列表",
    edit_tool_account: "编辑工具账户",
    add_tool_account: "添加工具账户",
    schedule: "计划",
    schedule_list: "计划列表",
    create_schedule: "创建计划",
    edit_schedule: "编辑计划",
    schedule_detail: "计划详情",
    proxy: "代理",
    proxy_list: "代理列表",
    edit_proxy: "编辑代理",
    add_proxy: "添加代理",
    parse_proxy: "批量上传代理",
    search: "市场洞察",
    search_scraper: "市场洞察浏览器",
    search_task_list: "市场洞察列表",
    search_task_detail: "搜索任务详情",
    edit_search_task: "编辑搜索任务",
    create_search_task: "创建搜索任务",
    email_extraction: "联系人画像洞察",
    email_extraction_form: "联系人画像洞察",
    email_extraction_edit: "编辑联系人画像洞察任务",
    email_extraction_list: "联系人画像洞察任务列表",
    email_extraction_detail: "联系人画像洞察详情",
    yellow_pages: "目录助手",
    yellow_pages_list: "目录助手",
    create_yellow_pages_task: "创建目录助手任务",
    edit_yellow_pages_task: "编辑目录助手任务",
    yellow_pages_task_detail: "任务详情",
    yellow_pages_results: "任务结果",
    yandex_maps_scraper: "本地商家查找",
    email_marketing: "外联营销",
    bulk_email_task_list: "邮件任务列表",
    email_send_log: "邮件发送日志",
    email_send_log_detail: "邮件发送日志详情",
    sending_bulk_emails: "发送批量邮件",
    email_template: "邮件模板",
    email_template_detail: "邮件模板",
    create_email_template: "创建邮件模板",
    email_filter: "邮件过滤器",
    email_filter_list: "邮件过滤器列表",
    email_filter_create: "创建邮件过滤器",
    email_filter_edit: "编辑邮件过滤器",
    email_service: "邮件服务",
    email_service_create: "创建邮件服务",
    email_service_edit: "编辑邮件服务",
    email_receive: "收件箱",
    email_receive_detail: "收件详情",
    ai_auto_replies: "AI 自动回复",
    ai_auto_reply_detail: "AI 自动回复详情",
    ai_chat_workspace: "AI 聊天工作区",
    knowledge_library: "知识库",
    ai_provider: "AI 提供商",
    mcp_tools: "MCP工具",
    skills: "技能",
    skills_management: "技能管理",
    ai_memory_management: "AI 记忆",
    hooks_management: "Hooks 管理",
    plugins: "插件",
    community_plugins: "社区插件",
    managed_browser: "托管浏览器",
    subagents: "子智能体",
    google_maps_scraper: "本地商家查找",
    map_scraper: "本地商家查找",
    login: "登录",
    not_found: "未找到",
  },
  knowledge: {
    subscription_required: "需要订阅",
    subscription_required_message: "知识库仅限订阅用户使用。请升级您的套餐。",
    upload_document: "上传文档",
    documents: "文档",
    chat: "聊天",
    select_files: "选择要上传的文件",
    upload: "上传",
    no_files_selected: "未选择文件",
    drag_drop_files: "拖拽文件到此处",
    drop_files_here: "在此处放置文件",
    supported_formats: "支持格式：PDF, TXT, DOC, DOCX, MD",
    browse_files: "浏览文件",
    selected_files: "已选文件",
    initializing_rag_system: "正在初始化RAG系统",
    setting_up_knowledge_library: "正在设置知识库...",
    rag_system_initialized_successfully: "RAG系统初始化成功",
    failed_to_initialize_rag_system: "RAG系统初始化失败",
    document_uploaded_successfully: '文档 "{name}" 上传成功',
    document_deleted_successfully: "文档删除成功",
    found_results: "找到 {count} 个结果",
    upload_failed: "上传失败",
    upload_failed_max_files: "最多允许上传 {max} 个文件，请先移除已有文件。",
    upload_failed_file_too_large: '文件 "{name}" 超过了 {maxSize} 的限制。',
    upload_failed_unsupported_type:
      '文件 "{name}" 的类型不支持。支持格式：图片、CSV、PDF、DOCX、XLS、XLSX。',
    embedding_billing_denied: "远程嵌入配额或计费额度已用尽",
    embedding_billing_denied_detail:
      "您账户的远程嵌入配额似乎已耗尽。请在服务端充值或更换套餐后重试，或联系管理员处理。",
    error: "错误",
    // Document Management translations
    document_management: "文档管理",
    search_by_name: "按名称或标题搜索",
    status: "状态",
    file_type: "文件类型",
    name: "名称",
    title: "标题",
    processing: "处理中",
    size: "大小",
    uploaded: "上传时间",
    actions: "操作",
    status_active: "活跃",
    status_archived: "已归档",
    status_processing: "处理中",
    status_completed: "已完成",
    status_pending: "待处理",
    status_failed: "失败",
    processing_completed: "已完成",
    processing_processing: "处理中",
    processing_failed: "失败",
    processing_error: "错误",
    processing_pending: "待处理",
    file_type_text: "文本",
    file_type_word: "Word",
    file_type_markdown: "Markdown",
    delete_selected: "删除选中项 ({count})",
    archive_selected: "归档选中项",
    select_file: "选择文件",
    description: "描述",
    tags: "标签",
    tags_hint: "按回车键添加标签",
    loading_content: "正在加载内容...",
    confirm_delete_document: '您确定要删除 "{name}" 吗？',
    confirm_bulk_delete: "您确定要删除 {count} 个文档吗？",
    confirm_reembed_document:
      '您确定要重新嵌入文档 "{name}" 吗？这将重新生成所有分块和嵌入。',
    reembed_success:
      '文档 "{name}" 重新嵌入成功：创建了 {chunks} 个分块，生成了 {embeddings} 个嵌入',
    reembed_error: '重新嵌入文档 "{name}" 失败：{error}',
    download_error: '下载文档 "{name}" 失败：{error}',
    delete_success: '文档 "{name}" 删除成功',
    delete_error: '删除文档 "{name}" 失败：{error}',
    bulk_delete_success: "成功删除 {count} 个文档",
    bulk_delete_partial: "成功删除 {success} 个文档，删除 {failed} 个文档失败",
    bulk_delete_error: "删除文档失败：{error}",
    // Error log dialog translations
    error_log_title: "错误日志 - {name}",
    loading_error_log: "正在加载错误日志...",
    no_error_log: "此文档没有可用的错误日志",
    // Settings translations
    settings: "设置",
    embedding_model: "嵌入模型",
    embedding_model_hint: "选择用于文档处理的嵌入模型",
    model_free: "免费",
    embedding_remote_failed_local_fallback:
      "远程嵌入失败。AiFetchly 已改用本地免费嵌入模型。",
    embedding_failed_after_fallback:
      "远程重试和本地回退后嵌入生成仍然失败。请查看文档错误日志了解详情。",
    embedding_partial_remote_search:
      "由于远程查询嵌入失败，部分远程索引的文档无法进行搜索。",
    current_model: "当前模型",
    update_model: "更新模型",
    model_category_fast: "快速",
    model_category_accurate: "准确",
    model_category_balanced: "平衡",
    model_category_default: "默认",
    priority: "优先级",
    embedding_model_updated_successfully: "嵌入模型更新成功",
    failed_to_update_embedding_model: "更新嵌入模型失败",
    local_runtime_not_installed_title: "本地 AI 运行组件未安装",
    local_runtime_not_installed_message:
      "本地嵌入模型“{model}”需要本地 AI 组件（{runtime}）。立即下载并安装即可继续。",
    local_runtime_download: "下载并安装",
    local_runtime_download_size: "下载大小：{size}",
    local_runtime_install_failed: "本地 AI 组件已安装但尚未就绪，请重试。",
    max_dimensions: "最大维度",
    model: "模型",
    dimensions: "维度",
    // Document processing translations
    processing_document: "处理文档",
    chunking_and_embedding_document: "正在分块和嵌入文档...",
    document_processed_successfully:
      '文档 "{name}" 处理成功：{chunks} 个块，{embeddings} 个嵌入',
    document_processing_failed: '文档 "{name}" 处理失败：{error}',
    document_processing_error: '处理文档 "{name}" 时出错：{error}',
    document_uploaded_pending_processing: '文档 "{name}" 已上传，等待处理',
    file_saved_but_database_error: "文件已保存但数据库错误",
    // 重复文件检查翻译
    duplicate_detected: "检测到重复文件",
    duplicate_files_found: "以下文件已存在于您的知识库中：",
    upload_anyway: "仍然上传",
    skip_duplicates: "跳过重复文件",
    // AI Chat translations
    ai_assistant: "AI 助手",
    type_message_placeholder: "输入消息... (Shift+Enter 换行)",
    start_conversation: "开始与 AI 助手对话",
    clear_chat_confirm: "您确定要清除聊天记录吗？",
    stop_generating: "停止生成",
    rag_context_enabled: "知识库上下文已启用",
    rag_context_disabled: "知识库上下文已禁用",
    just_now: "刚刚",
    minutes_ago: "{count}分钟前",
    hours_ago: "{count}小时前",
    resend_message: "重新发送消息",
    queue_title: "排队中的消息",
    queue_send: "排队发送",
    send_message: "发送",
    queue_item_label: "消息 {index}: {preview}",
    queue_remove: "移除",
    queue_clear_all: "清空全部",
    queue_empty: "没有排队中的消息",
    queue_auto_dispatch: "当前回复完成后，消息将自动发送。",
    queue_confirm_clear: "有排队中的消息，确定清空吗？",
    queue_confirm_switch: "有排队中的消息，放弃它们并切换对话吗？",
    import_website: "导入网站",
    website_import_title: "将网站导入知识库",
    website_import_subtitle:
      "将公开网页转换为 Markdown，并添加为可搜索的文档。",
    website_import_mode: "导入模式",
    website_import_mode_single: "单个页面",
    website_import_mode_list: "URL 列表",
    website_import_mode_crawl: "站点抓取",
    website_import_url: "URL",
    website_import_url_hint:
      "公开的 http(s) 链接，例如 https://example.com/pricing",
    website_import_seed_url: "起始 URL",
    website_import_urls: "URL",
    website_import_urls_hint: "每行一个 URL（最多 {max} 个）。",
    website_import_too_many_urls: "URL 过多：最多 {max} 个。",
    website_import_title_field: "标题（可选）",
    website_import_title_hint: "仅在单页模式下生效。",
    website_import_max_pages: "最大页数",
    website_import_max_depth: "最大深度",
    website_import_tags: "标签",
    website_import_tags_hint: "以逗号分隔。",
    website_import_author: "作者",
    website_import_description: "描述（可选）",
    website_import_duplicate_policy: "重复策略",
    website_import_policy_fail: "跳过重复项",
    website_import_policy_allow: "允许重复",
    website_import_hint:
      "仅支持公开的 http(s) 页面。每个页面会成为一份可搜索的文档。需要登录、localhost 以及内网页面将被拒绝。",
    website_import_url_required: "必须填写 URL。",
    website_import_urls_required: "至少需要一个 URL。",
    website_import_import: "导入",
    website_import_importing: "导入中...",
    website_import_close: "关闭",
    website_import_another: "再导入一次",
    website_import_failed: "导入失败",
    website_import_result_imported: "已导入",
    website_import_result_skipped: "已跳过",
    website_import_result_discovered: "发现 {count} 个同源链接。",
    website_import_chunks: "{count} 个分块",
    website_import_status_done: "已导入 {imported} 个页面；跳过 {skipped} 个。",
    website_import_progress_starting: "正在开始网站导入...",
    website_import_progress_scraping: "正在抓取页面...",
    website_import_progress_importing: "正在导入页面到知识库...",
    website_import_progress_imported: "页面已导入。",
    website_import_progress_skipped: "页面已跳过。",
    website_import_progress_completed: "网站导入完成。",
    website_import_current_page: "当前页面",
    website_import_progress_counts: "已导入 {imported}，已跳过 {skipped}",
    website_import_progress_discovered: "发现 {count} 个链接。",
  },
  notifications: {
    cloudflare_protection_detected: "检测到Cloudflare保护",
    recommendations: "建议",
    wait_before_retry: "重试前等待15-30分钟",
    use_different_proxy: "如果可用，使用不同的代理/VPN",
    reduce_frequency: "Reduce data alignment frequency",
    check_manual_access: "检查手动访问是否有效",
    task_id: "任务ID",
    dismiss: "关闭",
    view_task: "查看任务",
    cloudflare_detected: "检测到Cloudflare保护",
    scraping_paused: "Data alignment paused due to Cloudflare protection",
    manual_intervention_required: "需要手动干预",
  },
  home: {
    dashboard: "仪表板",
    agent_headline: "您的业务运营 AI 代理。",
    agent_placeholder: "让 AI 帮你寻找线索、撰写外联内容、分析营销活动...",
    agent_input_aria: "询问 AI 代理",
    agent_send_label: "打开 AI 聊天",
    refresh_dashboard: "刷新仪表板",
    analytics_trends: "分析与趋势",
    date_range_filter: "仪表板数据的日期范围筛选",
    updating_data: "更新数据中",
    dashboard_summary: "仪表板摘要统计",
    analytics_charts: "分析和趋势图表",
    // Summary Cards
    search_results: "搜索结果",
    emails_extracted: "联系人画像",
    yellow_pages: "目录助手",
    emails_sent: "已发送邮件",
    vs_previous_period: "与上一周期相比",
    success_rate: "成功率",
    // Date Filter
    last_7_days: "最近7天",
    last_30_days: "最近30天",
    last_90_days: "最近90天",
    last_365_days: "最近365天",
    all_time: "全部时间",
    custom: "自定义",
    start_date: "开始日期",
    end_date: "结束日期",
    apply: "应用",
    select_date_range: "选择日期范围",
    // Charts
    trends_over_time: "趋势变化",
    activity_overview: "活动概览",
    search_engine_breakdown: "搜索引擎分布",
    email_status_distribution: "邮件状态分布",
    retry: "重试",
    unable_to_load_chart: "无法加载图表",
    no_trend_data_available: "无趋势数据",
    no_search_engine_data_available: "无搜索引擎数据",
    no_email_status_data_available: "无邮件状态数据",
    no_activity_data_available: "无活动数据",
    no_data_found_for_period: "所选时间段没有数据",
    count: "数量",
    total_count: "总数",
    results: "结果",
    successful: "成功",
    failed: "失败",
    pending: "待处理",
    total_emails: "邮件总数",
    search: "市场洞察",
    search_scraper: "市场洞察浏览器",
    search_description: "整理公开市场洞察数据",
    yellowpage: "黄页",
    yellowpage_scraper: "目录助手",
    yellowpage_description: "整合公开目录数据",
    create_yellow_pages_task: "创建目录助手任务",
    edit_yellow_pages_task: "编辑目录助手任务",
    configure_yellow_pages_task: "配置并创建新的目录助手任务",
    modify_yellow_pages_task: "修改现有的目录助手任务",
    task_configuration: "任务配置",
    task_name: "任务名称",
    task_name_placeholder: "例如：餐厅搜索 - 纽约",
    task_name_required: "任务名称是必需的",
    platform: "平台",
    platform_placeholder: "选择平台",
    platform_required: "平台是必需的",
    keywords: "关键词",
    keywords_placeholder:
      "输入关键词，用逗号分隔（例如：餐厅、披萨、意大利菜）",
    keywords_required: "至少需要一个关键词",
    location: "位置",
    location_placeholder: "例如：纽约，纽约州",
    location_required: "位置是必需的",
    max_pages: "最大页数",
    max_pages_hint: "Maximum number of pages to organize",
    concurrency: "并发数",
    concurrency_hint: "Number of concurrent data alignment processes",
    delay_between_requests: "请求间延迟（毫秒）",
    delay_hint: "请求间延迟以避免速率限制",
    headless_mode: "不在浏览器中显示",
    headless_mode_hint:
      "启用时，浏览器在无可见UI的情况下运行（更快，资源占用更少）",
    account_optional: "账户（可选）",
    account_placeholder: "选择用于身份验证的账户",
    account_hint: "Select an account for the source channel",
    use_proxy: "使用代理",
    proxy_configuration: "代理配置",
    proxy_host: "代理主机",
    proxy_port: "代理端口",
    username_optional: "用户名（可选）",
    password_optional: "密码（可选）",
    scheduling_optional: "调度（可选）",
    schedule_task: "调度任务",
    scheduled_time: "调度时间",
    schedule_type: "调度类型",
    one_time: "一次性",
    recurring: "重复",
    cron_expression: "Cron表达式",
    cron_expression_placeholder: "例如：0 9 * * 1-5（工作日9点）",
    schedule_name: "调度名称",
    schedule_name_placeholder: "例如：每日餐厅搜索",
    schedule_description: "调度描述",
    schedule_description_placeholder: "此调度的描述",
    schedule_active: "调度激活",
    schedule_status: "调度状态",
    run_at: "运行时间",
    next_run_time: "下次运行时间：{time}",
    task_preview: "任务预览",
    platform_info: "平台信息",
    name: "名称",
    country: "国家",
    language: "语言",
    rate_limit: "速率限制",
    requests_per_hour: "请求/小时",
    status: "状态",
    active: "活跃",
    inactive: "非活跃",
    create_start_task: "创建并启动任务",
    create_task_only: "仅创建任务",
    update_task: "更新任务",
    cancel: "取消",
    task_created_successfully: "任务创建成功",
    task_created_message: "您的目录助手任务已成功创建！",
    task_updated_successfully: "任务更新成功",
    task_updated_message: "您的目录助手任务已成功更新！",
    error_creating_task: "创建任务时出错",
    view_all_tasks: "查看所有任务",
    view_task_details: "查看任务详情",
    // Validation error messages
    account_required_when_enabled:
      "启用账户时必须选择账户 - 请至少选择一个账户",
    proxy_required_when_enabled: "启用代理时必须选择代理 - 请至少选择一个代理",
    scheduled_time_required: "一次性调度需要调度时间 - 请选择日期和时间",
    cron_expression_required: "重复调度需要cron表达式 - 请输入有效的cron表达式",
    invalid_cron_expression: "无效的cron表达式 - 请检查格式",
    max_pages_range_error: "最大页数必须在1到100之间",
    concurrency_range_error: "并发数必须在1到10之间",
    delay_range_error: "请求间延迟必须在0到10000毫秒之间",
    platform_inactive_error:
      '平台 "{platformName}" 当前处于非活跃状态 - 请选择活跃的平台',
    socialmedia: "Social Organizer",
    socialmedia_scraper: "Social media data organizer",
    socialmedia_description: "Organizing public social media data",
    socialshare: "社交分享",
    socialshare_subtitle: "社交媒体营销",
    socialshare_description: "向社交媒体分享信息",
    emailmarketing: "邮件营销",
    emailmarketing_subtitle: "邮件营销",
    emailmarketing_description: "向客户发送邮件",
    tiktok_marketing_title: "TikTok营销",
    tiktok_marketing_subtitle: "分享到TikTok",
    tiktok_marketing_description: "向TikTok分享视频",
    youtube_marketing_title: "YouTube营销",
    youtube_marketing_subtitle: "分享到YouTube",
    youtube_marketing_description: "向YouTube分享视频",
    video_translation_title: "YouTube营销",
    video_translation_subtitle: "分享到YouTube",
    video_translation_description: "向YouTube分享视频",
    video_download_title: "视频下载",
    video_download_subtitle: "下载视频",
    video_download_description: "从平台下载视频",
    emailextraction: "联系人画像洞察",
    emailextraction_subtitle: "联系人画像洞察",
    emailextraction_description: "Build contact profiles from public pages",
    account_manage: "账户管理",
    account_manage_subtitle: "账户管理",
    account_manage_description: "管理系统中的账户",
    // Directory Assistant List Page
    yellow_pages_task_management: "目录助手任务管理",
    yellow_pages_task_management_description:
      "Manage data alignment tasks for directory sources",
    new_task: "新建任务",
    import: "导入",
    export: "导出",
    task_overview: "任务概览",
    total_tasks: "总任务数",
    running: "运行中",
    completed: "已完成",
    search_tasks: "搜索任务",
    // status:"状态",
    // platform:"平台",
    priority: "优先级",
    refresh: "刷新",
    clear_filters: "清除筛选",
    tasks: "任务",
    page: "页",
    of: "共",
    // cancel:"取消",
    confirm: "确认",
    task_details: "任务详情",
    delete_task: "删除任务",
    delete_task_confirm: '您确定要删除任务 "{name}" 吗？',
    high: "高",
    medium: "中",
    low: "低",
    paused: "已暂停",
    // Table column headers
    progress: "进度",
    updated_time: "更新时间",
    actions: "操作",
    // Task Results Page
    task_results: "任务结果",
    task_summary: "任务摘要",
    total_results: "总结果数",
    filter_by_category: "按分类筛选",
    export_results: "导出结果",
    business_name: "企业名称",
    contact: "联系方式",
    address: "地址",
    rating: "评分",
    additional_info: "附加信息",
    scraped_at: "Organized At",
    view_details: "查看详情",
    view_results: "查看结果",
    collecting: "收集中...",
    no_results: "暂无结果",
    results_available: "个结果可用",
    pause_task: "暂停任务",
    resume_task: "恢复任务",
    not_available: "暂无",
    invalid_date: "无效日期",
    open_website: "打开网站",
    copy_email: "复制邮箱",
    copy_phone: "复制电话",
    business_information: "企业信息",
    categories: "分类",
    description: "描述",
    year_established: "成立年份",
    number_of_employees: "员工数量",
    specialties: "专业领域",
    contact_information: "联系信息",
    email: "邮箱",
    phone: "电话",
    fax_number: "传真号码",
    website: "网站",
    contact_person: "联系人",
    address_information: "地址信息",
    street_address: "街道地址",
    city: "城市",
    state: "州/省",
    zip_code: "邮政编码",
    // country:"国家",
    rating_and_reviews: "评分和评论",
    review_count: "评论数量",
    business_hours: "营业时间",
    hours: "时间",
    no_hours_available: "暂无营业时间信息",
    additional_details: "附加详情",
    social_media: "社交媒体",
    payment_methods: "支付方式",
    raw_data: "原始数据",
    view_raw_data: "查看原始数据",
    scraping_information: "Data Alignment Information",
    task_id: "任务ID",
    // close:"关闭",
    // Additional keys needed for results page
    result_details: "结果详情",
  },
  campaign: {
    campaignId: "活动ID",
    campaignName: "活动名称",
    campaignDescription: "活动描述",
    campaignStatus: "活动状态",
    campaignTypes: "活动类型",
  },
  layout: {
    login: "登录",
    login_out: "退出登录",
    system_setting: "系统设置",
    platform_subtitle: "AI营销智能体",
    login_title: "AiFetchly 登录",
    login_description: "点击下方按钮登录您的账户",
    login_with_browser: "使用浏览器登录",
    logging_in: "登录中...",
    login_url_message: "如果浏览器没有自动打开，请点击下方按钮复制登录链接：",
    copy_login_url: "复制登录链接",
    close_dialog: "关闭对话框",
    login_timeout: "登录尝试超时，请重试。",
    login_failed: "无法打开登录页面，请重试。",
    url_copied: "登录链接已复制到剪贴板！",
    copy_failed: "无法复制链接到剪贴板，请手动复制。",
    language_updated_successfully: "语言更新成功",
    upgrade_plan: "升级",
    pricing_url_missing: "未配置价格页面 URL",
  },
  subscriptionEntitlement: {
    unlocked: "您的订阅已激活。托管 AI 功能已解锁。",
    cancelled: "您的订阅已变更。部分 AI 功能可能不可用。",
  },
  extramodule: {
    extramoduleName: "额外模块名称",
    extramoduleDescription: "额外模块描述",
    extramoduleStatus: "额外模块状态",
    extramoduleTypes: "额外模块类型",
    installConfirm: "您确定要安装 {package} 吗？",
    uninstallConfirm: "您确定要卸载 {package} 吗？",
    installed: "已安装",
    notInstalled: "未安装",
    actions: "操作",
    require_version: "需要版本",
    installed_version: "已安装版本",
    upgradeConfirm: "您确定要升级 {package} 吗？",
    upgrade_start: "模块升级开始",
  },
  modules: {
    f2: {
      name: "f2视频下载",
      describe: "f2是用于视频下载的模块",
    },
    youtube_dl: {
      name: "youtube-dl",
      describe: "youtube-dl是用于YouTube视频下载的模块",
    },
    openai_whisper: {
      name: "openai-whisper",
      describe: "openai-whisper是用于文本生成的模块",
    },
    faster_whisper: {
      name: "faster-whisper",
      describe: "faster-whisper是用于文本生成的模块",
    },
  },
  video: {
    platform: "平台",
    saved_path: "保存路径",
    video_url: "视频URL",
    input_video_url_hint: "输入视频URL，每行一个",
    input_video_save_path: "输入视频保存路径",
    video_download_error: "视频下载错误",
    download_failed: "视频下载失败",
    download_success: "视频下载成功",
    revice_download_command: "接收下载命令",
    account_id_not_found: "未找到账户ID",
    platform_not_found: "未找到平台",
    link_not_found: "未找到链接",
    savePath_not_found: "未找到保存路径",
    start_download_video: "开始下载视频",
    new_video_download: "新视频下载",
    downloadtype: "下载类型",
    create_download_task_failuer: "创建视频下载任务失败",
    use_proxy_override: "使用选择的代理替代账户的盲代理",
    cookies_type: "cookies类型",
    choose_browser: "选择浏览器",
    task_name: "任务名称",
    url: "URL",
    title: "标题",
    description: "视频描述",
    generate_captions: "生成字幕",
    caption: "字幕",
    caption_status: "字幕状态",
    is_english_video: "是否为英文视频",
    video_language_select: "视频语言选择",
    caption_save_path: "字幕保存路径",
    input_caption_save_path: "输入字幕保存路径",
    caption_generate_failed: "字幕生成失败",
    caption_generate_start: "字幕生成开始",
    video_task__download_retry_start: "视频任务下载重试开始",
    item_not_found_local: "本地未找到项目",
    caption_files: "字幕文件",
    open_caption_file_error: "打开字幕文件错误",
    translate_information: "翻译信息",
    translate_video: "翻译视频",
    source_language: "源语言",
    target_language: "目标语言",
    select_language_error: "语言为空，请选择语言",
    translate_video_information: "翻译视频信息",
    translation_info: "翻译信息",
    select_language: "选择语言",
    keyword: "视频关键词",
    max_page_number: "最大页数",
    publish_video: "发布视频",
    select_platform: "选择平台",
    select_category: "选择分类",
  },
  language: {
    zh: "中文",
    en: "英文",
    es: "西班牙语",
    fr: "法语",
    de: "德语",
    ja: "日语",
  },
  account: {
    accountId: "账户ID",
    type: "账户类型",
    usename: "用户名",
    useproxy: "使用代理",
    cookies_exist: "存在Cookies",
    select_account_hint: "Select an account for the source channel",
    change_account: "更换账户",
    encryption_unavailable: "账号加密服务不可用，请检查网络连接后重试。",
  },
  search: {
    google: "Google",
    bing: "Bing.com",
    yandex: "Yandex",
    platform: "平台",
    keyword: "关键词",
    search_enginer_name: "搜索引擎",
    input_keywords_hint: "输入关键词，每行一个",
    market_insight_placeholder:
      "请输入您需要进行市场洞察的公开网络源或行业关键词...",
    public_information_notice:
      "注：本功能仅用于公开网页信息的自动化结构化整理，请确保您的检索行为符合目标网站的 Robots 协议。",
    show_in_Browser: "在浏览器中显示",
    use_hint: "输入公开来源或行业关键词，用于市场洞察整理",
    concurrent_quantity: "并发数量",
    scraper_failed: "市场洞察整理失败",
    search_enginer_empty: "搜索引擎为空",
    keywords_empty: "关键词为空",
    page_number: "页数",
    choose_proxy: "选择代理",
    select_proxy: "选择代理",
    search_enginer_invalid: "搜索引擎无效",
    use_local_chrome_data: "使用本地Chrome浏览器配置文件",
    use_local_browser: "使用本地浏览器",
    choose_local_browser: "选择本地浏览器",
    use_search_enginer_account: "使用搜索引擎账户",
    edit_task: "编辑市场洞察任务",
    task_updated_successfully: "任务更新成功",
    task_cannot_be_edited:
      "任务无法编辑。只有状态为'未开始'、'错误'、'处理中'或'完成'的任务才能修改。",
    task_not_found: "任务未找到",
    at_least_one_keyword_required: "至少需要一个关键词",
    pages_must_be_between: "页数必须在1到100之间",
    concurrency_must_be_between: "并发数必须在1到10之间",
    save_only: "仅保存",
    task_saved_successfully: "任务保存成功",
    yandex_local_browser_tip:
      "提示：为了获得更好的Yandex搜索结果，我们建议使用本地浏览器。请启用'使用本地浏览器'选项。",
    google_account_tip:
      "提示：为了获得更好的Google搜索结果，我们建议使用搜索引擎账户。请启用'使用搜索引擎账户'选项。",
    google_account_cookies_required:
      "Google搜索需要带有cookies的Google账户。请在工具账户中添加一个带有有效cookies的Google账户。",
    task_id: "任务ID",
    run_task: "运行任务",
    task_status_not_start: "未开始",
    generate_related_keywords: "使用AI生成相关关键词",
    keywords_generated_successfully: "成功生成 {count} 个相关关键词",
    no_keywords_generated: "未生成关键词",
    enable_ai_recovery: "启用AI恢复",
    enable_ai_recovery_hint:
      "启用后，AI 将通过分析页面结构并建议替代操作，帮助从数据对齐错误中恢复",
  },
  searchtask: {
    id: "任务ID",
  },
  searchresult: {
    id: "ID",
    link: "链接",
    title: "标题",
    snippet: "摘要",
    record_time: "记录时间",
    keyword: "关键词",
    status: "状态",
    numbers: "结果数量",
  },
  searchresultdetail: {
    title: "搜索结果详情",
    basic_info: "基本信息",
    columns: "列",
  },
  proxy: {
    add_proxy: "添加代理",
    check_proxy: "检查代理",
    status: "状态",
    id: "ID",
    host: "主机",
    port: "端口",
    user_name: "用户名",
    password: "密码",
    protocol: "协议",
    check_time: "检查时间",
    check_proxy_tip: "正在检查代理，进度为 {process}%",
    pass: "通过",
    failure: "失败",
    remove_failure: "移除失败代理",
    unkonw: "未知",
    host_hint: "输入主机地址",
    port_hint: "输入端口号",
    user_hint: "输入用户名",
    password_hint: "输入密码",
    loading_data: "正在加载数据",
    // check_proxy:"检查代理",
    import_proxy: "导入代理",
    no_proxy_to_import: "可用代理数量少于一个，没有代理可导入",
    no_proxy_selected: "请先选择至少一个要检查的代理",
    import_proxy_success: "导入代理成功",
    import_proxy_fail: "导入代理失败",
    check_settings: "检查设置",
    check_timeout: "检查超时",
    check_timeout_suffix: "秒",
    check_timeout_hint: "代理验证超时时间（秒），范围1-60",
    clear_table: "清除表格",
    // check_proxy:"检查代理",
    save_to_my_proxy: "保存到我的代理",
    remove_fail_proxy: "移除失败代理",
    google_pass: "Google通过",
    google_pass_pass: "通过",
    google_pass_fail: "失败",
    google_pass_not_checked: "未检查",
    search: "Search",
    // host:"主机",
    // port:"端口",
    // protocol:"协议",
    user: "用户",
    // pass:"密码",
    // status:"状态",
    not_check: "未检查",
    download_template_hint: '点击"下载模板"按钮获取CSV模板文件',
    edit_template: "编辑模板",
    edit_template_hint:
      "打开下载的CSV文件并添加您的代理信息（主机、端口、协议、用户名、密码）",
    protocol_hint: "协议可以是 http、https、socks5",
    upload_file: "上传文件",
    upload_file_hint: "使用下面的文件输入选择您编辑的CSV文件",
    check_proxies: "检查代理",
    check_proxies_hint: '使用"检查代理"验证上传的代理',
    how_to_batch_upload: "如何批量上传代理",
    // 批量上传对话框新翻译
    batch_upload: "批量上传",
    paste_text: "粘贴文本",
    paste_proxy_hint: "Paste proxies (one per line)",
    format_hint:
      "格式：host:port, host:port:user:pass, protocol://host:port:user:pass",
    parsed_proxies: "已解析代理",
    proxies_parsed: "个代理已解析",
    import_rows_missing_fields:
      "已跳过 {count} 行：每条代理必须包含主机、端口和协议（文件行号：{rows}）。",
    import_partial_rows_missing_fields:
      "已加载 {valid} 条代理；已跳过 {count} 行（缺少主机、端口或协议）（文件行号：{rows}）。",
    no_valid_proxies: "未找到有效代理",
    enter_proxy_text: "请输入代理文本",
    parse_text: "解析文本",
    no_proxies_to_check: "没有要检查的代理",
    check_complete: "检查完成",
    no_proxies_to_import: "没有要导入的代理",
    import_success: "个代理成功导入",
    import_partial: "部分代理导入失败",
    import_fail: "导入失败",
  },
  emailextraction: {
    max_page_number: "最大页数",
    manualinputurl: "手动输入URL",
    searchresult: "搜索结果",
    googlemaps: "Google 地图",
    yandexmaps: "Yandex 地图",
    choose_maps_record: "请选择一个地图抓取记录",
    extraction_type: "Insight Type",
    extract_emails: "Build Contact Profiles",
    input_urls_hint: "输入URL，每行一个",
    page_length: "page depth for profile insights",
    choose_email_extraction_type: "Insight Type",
    input_urls_empty: "请输入至少一个有效URL，URL应以http://或https://开头",
    choose_search_task: "选择搜索任务",
    id: "ID",
    type: "类型",
    task_id_empty: "任务ID为空",
    url: "URL",
    title: "标题",
    process_timeout: "处理超时分钟数",
    failed: "Contact profile insights failed",
    searchResult_empty: "No search results found for contact profile insights",
    create_task: "创建联系人画像洞察任务",
    edit_task: "编辑联系人画像洞察任务",
    update_success: "任务更新成功",
    update_error: "更新任务失败",
    load_task_error: "加载任务数据失败",
    no_valid_urls: "未找到有效URL",
    ai_support_enabled: "启用AI智能补充",
    ai_support_hint: "使用AI提取电话号码、地址和社交链接",
    ai_support_upgrade_hint: "升级您的套餐以启用AI智能补充。",
    ai_enrichment_status: "AI状态",
    ai_status_none: "未补充",
    ai_status_pending: "处理中",
    ai_status_completed: "已完成",
    ai_status_failed: "失败",
    ai_status_skipped: "已跳过",
    phone: "电话",
    address: "地址",
    social_links: "社交链接",
    ai_confidence: "AI置信度",
    confirm_kill: "确定要停止此任务吗？",
    kill_task: "停止任务",
    kill_success: "任务已成功停止",
    kill_error: "停止任务失败",
    confirm_start: "确定要启动此任务吗？",
    start_task: "启动任务",
    start_success: "任务已成功启动",
    start_error: "启动任务失败",
  },
  contactExtraction: {
    extract_contact_info: "AI Profile Insights",
    contact_extraction_status: "Insight Status",
    extraction_status: "Insight Status",
    email: "邮箱",
    phone: "电话",
    address: "地址",
    contact_info: "联系信息",
    extraction_date: "Insight Date",
    social_links: "社交链接",
    status_completed: "已完成",
    status_analyzing: "分析中",
    status_failed: "失败",
    status_pending: "待处理",
    select_items_hint: "Please select at least one item for profile insights",
    no_valid_items_hint: "所选项目没有有效的ID。请选择其他项目。",
    extraction_started: "Profile insights started",
    extraction_started_notice:
      "Profile insights started for {count} item(s). Processing in the background. Check the Insight Status column for progress.",
    extraction_failed: "Failed to start profile insights",
  },
  emailscrape: {
    failed: "Contact profile organization failed",
    url_empty: "URL列表为空",
    url_invalid: "未找到有效URL",
    searchTaskId_empty: "搜索任务ID为空",
    searchResult_empty: "No search results found for contact profile insights",
    mapsResult_empty: "所选地图记录中未找到网站链接",
    action_error: "无效的操作类型",
    emailsearch_task_start: "邮件搜索任务已成功启动",
  },
  emailmarketing: {
    id: "ID",
    index: "索引",
    marketingtype: "营销类型",
    use_email_source: "使用邮件来源",
    title: "邮件标题",
    title_hint: "输入邮件标题",
    content: "邮件内容",
    content_hint: "输入邮件内容",
    description: "描述",
    description_hint: "输入邮件模板描述",
    insert_time_variable: "插入时间变量",
    insert_sender_variable: "插入发送者变量",
    insert_receiver_variable: "插入接收者变量",
    insert_source: "插入来源",
    insert_description: "插入描述",
    email_template_deleted: "您确定要删除此邮件模板吗？",
    confirm_delete_email_template: "邮件模板删除确认",
    template_id_require: "模板ID是必需的",
    preview: "预览",
    create_template: "创建模板",
    SEARCHRESULT: "搜索结果",
    EMAIL_EXTRA_TASK_LIST: "contact profile insights task list",
    remove_email_template_error: "删除邮件模板错误",
    template_id_is_required: "模板ID是必需的",
    create_email_service_error: "创建邮件服务错误",
    template_item_notexist: "模板项目不存在",
    update_email_template_error: "更新邮件模板错误",
    update_email_filter_error: "更新邮件过滤器错误",
    service_list_error: "服务列表错误",
    delete_email_filter_error: "删除邮件过滤器错误",
    list_email_template_error: "列出邮件模板错误",
  },
  aiTemplateGeneration: {
    title: "AI 生成",
    promptPlaceholder: "描述您想创建的邮件...",
    promptLabel: "邮件描述",
    tone: "语气",
    toneFormal: "正式",
    toneCasual: "随意",
    toneFriendly: "友好",
    toneProfessional: "专业",
    templateType: "邮件类型",
    typeColdOutreach: "冷拓展",
    typeFollowUp: "跟进",
    typeNewsletter: "通讯",
    typePromotion: "促销",
    typeCustom: "自定义",
    advanced: "高级选项",
    useKnowledgeBase: "使用知识库作为上下文",
    generateButton: "生成",
    stopButton: "停止生成",
    generating: "正在生成模板...",
    error: {
      promptTooShort: "提示词必须至少包含10个字符",
      aiDisabled: "AI功能未启用。请升级您的计划以访问AI功能。",
      timeout: "生成超时。请重试。",
      serviceUnavailable: "AI服务暂时不可用。请稍后重试。",
      networkError: "网络连接失败。请检查您的网络连接并重试。",
      parseError: "生成格式无效。请使用更清晰的提示重试。",
    },
    invalidVariables: "模板包含不支持的变量，它们已被删除：",
    regenerateConfirm: "替换当前生成的模板？",
    regenerateConfirmTitle: "重新生成模板",
  },
  emailfilter: {
    id: "ID",
    name: "名称",
    inputname_hint: "输入过滤器名称",
    index: "索引",
    filtercontent: "过滤器内容",
    filtercontent_hint: "输入过滤器内容",
    filtertype_hint: "选择过滤器类型",
    filter_deleted: "您确定要删除此过滤器吗？",
    confirm_delete_filter: "过滤器删除确认",
    filter_id_require: "过滤器ID是必需的",
    create_filter: "创建过滤器",
    description: "描述",
    description_hint: "输入过滤器描述",
  },
  emailReceive: {
    receive_settings: "收件设置",
    receive_enabled: "启用",
    receive_disabled: "禁用",
    receive_protocol: "收件协议",
    folder: "文件夹",
    folder_hint: "监听的邮箱文件夹，如 INBOX",
    imap_host: "IMAP 主机",
    imap_port: "IMAP 端口",
    imap_ssl: "IMAP SSL/TLS",
    pop3_host: "POP3 主机",
    pop3_port: "POP3 端口",
    pop3_ssl: "POP3 SSL/TLS",
    receive_username: "收件用户名",
    receive_username_hint: "留空则使用发件地址",
    receive_password: "收件密码",
    test_connection: "测试连接",
    test_save_first: "请先保存邮箱服务再测试收件连接。",
    test_connection_success: "收件连接成功。",
    test_connection_failed: "收件连接失败",
    messages_title: "收到的邮件",
    select_email_service: "邮件服务",
    select_email_service_hint: "请选择一个邮件服务以查看邮件。",
    sync: "同步",
    reply_status: "回复状态",
    subject: "主题",
    from: "发件人",
    received_at: "接收时间",
    unread: "未读",
    unread_unavailable_pop3: "POP3 邮箱不支持未读状态。",
    classification: "分类",
    message_detail: "邮件详情",
    reply_to: "回复至",
    body_html: "正文 (HTML)",
    body_text: "正文 (文本)",
    status: {
      not_started: "未开始",
      draft_created: "已生成草稿",
      sent: "已发送",
      skipped: "已跳过",
      blocked: "已拦截",
      failed: "失败",
    },
  },
  emailReply: {
    drafts_title: "回复草稿",
    generate_draft: "生成草稿",
    regenerate: "重新生成",
    edit_draft: "编辑草稿",
    send_reply: "发送回复",
    send_confirm: "立即发送此回复？",
    send_success: "回复已发送。",
    send_failed: "回复发送失败。",
    knowledge_sources: "知识来源",
    warnings: "警告",
    identity_profile: "回复身份档案",
    owner_name: "发件人姓名",
    owner_role: "职位",
    company_name: "公司",
    preferred_tone: "偏好语气",
    signature: "签名",
    style_notes: "风格备注",
    forbidden_phrases: "禁用词组（逗号分隔）",
    disclose_automation: "披露 AI 协助",
  },
  emailAutoReplyAudit: {
    title: "AI 自动回复",
    detail_title: "AI 自动回复审计",
    decision_status: "决策",
    classification: "分类",
    search: "搜索主题、发件人、原因、错误",
    created_at: "日期",
    subject: "主题",
    confidence: "置信度",
    requires_approval: "审批",
    reason: "原因",
    knowledge_query: "知识检索",
    knowledge_source_count: "知识来源数",
    generated_subject: "生成的主题",
    generated_body_preview: "生成的正文预览",
    sent_subject: "已发送主题",
    sent_body_preview: "已发送正文预览",
    status: {
      draft_created: "已生成草稿",
      approval_required: "需人工审批",
      auto_sent: "已自动发送",
      blocked: "已拦截",
      skipped: "已跳过",
      failed: "失败",
      needs_human_review: "需人工复核",
    },
  },
  outboundEmail: {
    batch_card_title: "外发邮件批次",
    recipient_count: "收件人",
    mode_send_now: "立即发送",
    mode_review_first: "先审阅",
    mode_draft_only: "仅草稿",
    review_reason: "需要审阅",
    review_action: "审阅",
    sent_summary: "批次已发送。",
    partial_summary: "批次部分发送。",
    unknown_summary: "投递状态未知。",
    failed_summary: "批次发送失败。",
    discarded_summary: "批次已丢弃。",
    review_title: "审阅外发邮件",
    recipient: "收件人",
    sender: "发件人",
    subject: "主题",
    body: "正文",
    mode: "模式",
    send: "发送",
    discard: "丢弃",
    approve: "批准",
    edit: "编辑",
    save: "保存",
    cancel: "取消",
    approval_invalidated: "编辑会使之前的批准失效。发送前请重新批准。",
    preflight_blocked: "存在阻断性检查项，无法批准。",
    send_success: "批次已进入投递队列。",
    send_failed: "批次发送失败。",
    no_approval: "请先批准再发送。",
    progress_title: "投递进度",
    status: "状态",
    submitted: "已提交",
    sent: "已发送",
    failed: "失败",
    delivery_unknown: "投递未知",
    unknown_no_retry: "状态未知——请勿自动重试。",
    retry: "重试",
  },
  emailservice: {
    id: "ID",
    from: "发送者账户",
    from_hint: "输入邮件发送者账户",
    password: "密码",
    password_hint: "输入邮件密码",
    host: "SMTP 主机",
    host_hint: "输入邮件主机",
    port: "端口",
    port_hint: "输入邮件端口",
    name: "名称",
    name_hint: "输入邮件名称",
    ssl: "SSL",
    ssl_hint: "输入邮件SSL",
    email_service_deleted: "您确定要删除此邮件服务吗？",
    confirm_delete_email_service: "邮件服务删除确认",
    service_id_require: "服务ID是必需的",
    create_service: "创建邮件服务",
    port_lenght_error: "端口长度错误",
    test_email_title: "测试邮件标题",
    test_email_title_hint: "输入测试邮件标题",
    test_email_content: "测试邮件内容",
    test_email_content_hint: "输入测试邮件内容",
    test_email_service: "测试邮件服务",
    test_email_receiver: "测试邮件接收者",
    test_email_receiver_hint: "输入测试邮件接收者",
    send_test_email_success: "发送测试邮件成功",
    send_test_email_error: "发送测试邮件错误",
    email_send_success: "邮件发送成功",
    required_fields_missing: "请填写所有必填字段",
  },
  buckemailsend: {
    email_source: "邮件来源",
    email_template: "邮件模板",
    email_filter: "邮件过滤器",
    email_service: "邮件服务",
    choose_email_source: "选择邮件来源",
    choose_email_template: "选择邮件模板",
    choose_email_filter: "选择邮件过滤器",
    choose_email_service: "选择邮件服务",
    email_souce_empty: "邮件来源不应为空",
    email_template_empty: "邮件模板不应为空",
    email_filter_empty: "邮件过滤器不应为空",
    email_service_empty: "邮件服务不应为空",
    choose_at_least_one_souce: "您应至少选择一个来源",
    avoid_duplicate: "避免用户收到重复邮件",
    check_email_souce: "请检查邮件来源是否正确",
    email_list_task_empty: "任务中的邮件列表为空",
    email_list_empty: "邮件列表为空",
    start_send_email: "开始发送邮件",
    receiver_email_list_empty: "接收者邮件列表为空",
  },
  buckemailtask: {
    taskId: "任务ID",
    status: "任务状态",
    type: "任务类型",
    create_task: "创建联系人画像洞察任务",
    email_service: "邮件服务",
  },
  emailtasksendlog: {
    id: "ID",
    status: "状态",
    receiver: "接收者",
    title: "标题",
    content: "内容",
    record_time: "记录时间",
    source: "来源",
    source_legacy: "批量任务",
    source_authorized: "AI授权",
    actions: "操作",
    detail_title: "发送日志详情",
    log: "日志",
    task_id: "任务ID",
    sender: "发件人",
    actor: "生成者",
    body: "正文",
    provider_message_id: "服务商消息ID",
    error_code: "错误代码",
    submitted_at: "提交时间",
    completed_at: "完成时间",
    batch_id: "批次ID",
    draft_id: "草稿ID",
    revision_id: "修订版本ID",
    attempt_id: "发送尝试ID",
    detail_not_found: "未找到发送日志记录",
  },
  socialaccount: {
    create_account: "创建工具账户",
    uploadfilemsg_title: "手动上传cookies文件",
    uploadfilemsg_content: "请从浏览器导出cookies并手动上传cookies文件",
    upload_cookies_button: "上传 cookies (Netscape .txt)",
    open_managed_browser: "打开托管浏览器",
    upload_cookies_cancel: "上传cookies已取消",
    handleCookiesfileSuccess: "处理cookies文件成功",
    handleCookiesfileFailure: "处理cookies文件失败",
    insertCookiesFailure: "插入cookies失败",
    delete_account_errror: "删除工具账户错误",
    delete_tool_account_confirm_title: "确认删除工具账户",
    delete_tool_account_confirm_text: "该工具账户将被删除",
    login_account_error: "登录工具账户错误",
    clear_cookies_success: "清除cookies成功",
    cookies_cleared_successfully: "cookies清除成功",
    clear_cookies_error: "清除cookies错误",
    clear_cookies_confirm_title: "确认清除cookies",
    clear_cookies_confirm_text: "该工具账户的cookies将被清除",
    manuallogin_title: "手动登录通知",
    manuallogin_content:
      "我们将打开一个弹出窗口，请手动登录到所选平台，登录后请关闭弹出窗口，系统将自动保存工具账户的cookies",
    saveCookiesSuccess: "保存cookies成功",
    // Form fields
    col_id: "ID",
    col_type: "类型",
    col_username: "用户名",
    cookies: "Cookies",
    proxy: "代理",
    status: "状态",
    actions: "操作",
    user: "用户",
    user_hint: "输入名称",
    pass: "密码",
    pass_hint: "输入密码",
    platform: "平台",
    name: "姓名",
    name_hint: "输入工具账户姓名",
    phone: "电话",
    phone_hint: "输入工具账户电话",
    email: "邮箱",
    email_hint: "输入工具账户邮箱",
    select_proxy: "选择代理",
    change_proxy: "更换代理",
    inactive: "非活跃",
    active: "活跃",
    // Messages
    form_not_valid: "表单无效",
    save_success: "保存成功，工具账户ID为",
    save_fail: "保存失败",
    session_status_available: "会话可用",
    session_status_missing: "无会话",
    session_status_invalid: "会话失效，请重新登录",
    session_status_migration_pending: "会话升级中",
    import: {
      button: "从浏览器配置文件导入",
      title: "从浏览器配置文件导入",
      subtitle: "仅读取该平台在您浏览器配置文件中已授权的 Cookie。",
      target: "目标账户平台",
      approved_domains: "已授权的 Cookie 域名",
      instructions:
        "在您要导入的 Chrome、Edge 或 Brave 配置文件中打开 AiFetchly 扩展，然后确认平台。仅一次性读取下列已授权的 Cookie。",
      start: "开始配对",
      cancel: "取消",
      close: "关闭",
      reason_feature_disabled: "当前版本未启用浏览器配置文件导入。",
      reason_platform_unsupported: "该平台暂不支持浏览器配置文件导入。",
      reason_unavailable: "该账户无法使用浏览器配置文件导入。",
      result_success: "已导入 {count} 个已授权 Cookie，请验证会话。",
      result_no_cookies: "在配对的浏览器配置文件中未找到已授权的 Cookie。",
      result_expired: "配对请求已过期，请重新开始。",
      result_failed: "导入失败，请重试或手动登录。",
    },
  },
  video_download_list: {},
  system_settings: {
    title: "系统设置",
    no_setting_item_found_exit: "未找到设置项目，请退出",
    no_description: "无描述",
    "2captcha-group": "2captcha",
    "2captcha-description": "2captcha描述",
    "2captcha-token": "2captcha令牌",
    "2captcha-enabled": "2captcha已启用",
    "2captcha-disabled": "2captcha已禁用",
    "2captcha-enabled-description": "您是否要启用2captcha",
    "2captcha-token-description": "输入2captcha令牌",
    choose_file: "选择文件",
    "chrome-path-description": "输入Chrome路径",
    "firefox-path-description": "输入Firefox路径",
    chrome_path: "Chrome路径",
    firefox_path: "Firefox路径",
    embedding_group: "嵌入模型",
    embedding_group_description: "向量搜索的默认嵌入模型。",
    external_system: "外部系统",
    user_preferences: "用户偏好",
    "user-preferences-group-description": "应用程序行为的用户级偏好设置。",
    default_embedding_model: "默认嵌入模型",
    "default-embedding-model-description":
      "选择用于向量搜索和相似度匹配的嵌入模型。",
    language_preference: "语言",
    "language-preference-description": "界面语言偏好。",
    ai_website_analysis_business_info: "业务信息",
    "ai-website-analysis-business-info-description":
      "网站AI分析中使用的业务信息。",
    "external-system-group-description": "外部系统组设置",
    mcp_tools: "MCP工具",
    manage_mcp_tools: "管理MCP工具",
    manage_skills: "管理技能",
    manage_ai_memories: "管理 AI 记忆",
    manage_ai_provider: "AI 提供商",
    manage_hooks: "管理 Hooks",
    manage_subagents: "管理子智能体",
    hooks: {
      title: "Hooks 管理",
      description: "管理 AI 聊天生命周期 hook",
      global_enable: "全局启用 hooks",
      global_disable_banner: "Hooks 已全局禁用 — 任何 hook 都不会触发。",
      add_command: "+ 添加命令 hook",
      filter_event: "事件",
      filter_source: "来源",
      filter_status: "状态",
      filter_hook: "Hook",
      last_rows: "最近条数",
      show_session: "显示会话 hook",
      list_empty: "暂无 hook",
      create_first: "创建你的第一个命令 hook",
      trust_required: "需要信任",
      trust_confirm_title: "信任此命令 hook?",
      trust_confirm_body:
        '信任后，本地进程 "{command}" 将在每次匹配 "{event}" 事件时运行。',
      delete_confirm_title: "删除此 hook?",
      delete_confirm_body: "此操作无法撤销。",
      builtin_cannot_modify: "内置 hook 不可修改",
      audit_title: "近期审计日志",
      audit_empty: "尚无 hook 活动记录",
      audit_time: "时间",
      audit_duration: "耗时",
      audit_reason: "原因",
      auto_refresh_start: "开启自动刷新",
      auto_refresh_pause: "暂停自动刷新",
      field: {
        id: "钩子ID",
        event: "事件",
        matcher: "匹配器",
        if_condition: "条件筛选",
        if_condition_placeholder: "例如: echo *",
        type: "类型",
        command: "命令",
        cwd: "工作目录",
        timeout: "超时(毫秒)",
        failure_mode: "失败处理",
        status_message: "状态消息",
        env_allowlist: "环境变量白名单(JSON)",
        enabled: "启用",
        trusted: "信任",
        source: "来源",
      },
      source: {
        builtin: "内置",
        user: "用户",
        session: "会话",
      },
      status: {
        started: "已开始",
        success: "成功",
        blocked: "已阻止",
        failed: "失败",
        timeout: "超时",
      },
      button: {
        save: "保存",
        delete: "删除",
        trust: "信任",
        untrust: "取消信任",
        cancel: "取消",
      },
      toast: {
        saved: "Hook 已保存",
        deleted: "Hook 已删除",
        trust_updated: "信任状态已更新",
        load_failed: "加载 hook 失败",
        create_failed: "创建 hook 失败",
      },
      untrusted_warning: "此钩子未受信任，不会运行。点击信任以允许执行。",
      scraping_compliance_warning:
        "启用此钩子会在每次抓取工具调用后向AI提示中注入合规上下文，可能会影响抓取结果。",
    },
    ai_preferences: "AI 偏好设置",
    "ai-preferences-group-description":
      "控制 AI 聊天行为、记忆和上下文的设置。",
    user_ai_auto_dream: "自动摘要整合",
    "ai-auto-dream-description":
      "在后台自动汇总过去的对话，以节省 AI 上下文令牌。",
    user_ai_memory_injection: "用户记忆注入",
    "ai-memory-injection-description":
      "将过去对话中的相关用户记忆注入 AI 聊天上下文中。",
    user_ai_custom_context_directive: "自定义上下文指令",
    "ai-custom-context-directive-description":
      "在每次 AI 聊天请求中注入的静态文本（位于系统提示词之后）。清空输入框即可停用。",
    "ai-custom-context-directive-placeholder":
      "例如：始终简洁回答。优先使用要点。我们面向美国客户销售鞋类。",
    user_ai_workspace_memory_injection: "工作区记忆注入",
    "ai-workspace-memory-injection-description":
      "将相关的工作区记忆（项目决策、命令、约定）注入到当前工作区的 AI 聊天上下文中。",
    user_ai_workspace_auto_dream: "工作区自动摘要",
    "ai-workspace-auto-dream-description":
      "在后台从会话和智能体任务中整合工作区专属记忆。",
    user_ai_workspace_manual_memory: "手动工作区记忆",
    "ai-workspace-manual-memory-description":
      "允许手动创建、编辑和删除工作区记忆。禁用不会删除已存储的记忆。",
    user_ai_desktop_notify: "桌面完成通知",
    "ai-desktop-notify-description":
      "当主 AI 代理完成回复或计划待审批时，在桌面右下角显示通知（仅在应用处于后台时）。",
  },
  aiMemory: {
    title: "AI 记忆",
    description: "AI 在跨对话中记住的持久事实和偏好。",
    search_placeholder: "搜索标题或内容…",
    filter_type: "类型",
    filter_status: "状态",
    filter_source: "来源",
    filter_all: "全部",
    button_create: "新建记忆",
    button_refresh: "刷新",
    col_title: "标题",
    col_type: "类型",
    col_content: "内容",
    col_status: "状态",
    col_source: "来源",
    col_updated: "更新时间",
    col_actions: "操作",
    type_preference: "偏好",
    type_fact: "事实",
    type_decision: "决定",
    type_reference: "参考",
    type_workflow: "流程",
    status_active: "活跃",
    status_archived: "已归档",
    status_contradicted: "已冲突",
    source_manual: "手动",
    source_chat_v2: "聊天",
    source_agent_task: "代理任务",
    source_auto_dream: "自动整理",
    loading: "加载中…",
    empty_title: "还没有记忆",
    empty_description: "新建一条记忆，AI 将在跨对话中记住它。",
    error_load: "加载记忆失败。",
    page_of: "第 {page} 页，共 {total} 页",
    dialog_title_create: "新建记忆",
    dialog_title_edit: "编辑记忆",
    field_type: "类型",
    field_title: "标题",
    field_content: "内容",
    field_status: "状态",
    field_confidence: "置信度",
    field_source: "来源",
    button_save: "保存",
    button_cancel: "取消",
    err_title_required: "标题为必填项。",
    err_content_required: "内容为必填项。",
    action_edit: "编辑",
    action_archive: "归档",
    action_delete: "删除",
    confirm_archive_title: "归档此记忆？",
    confirm_archive_text: "已归档的记忆将不会提供给 AI。之后可通过编辑恢复。",
    confirm_delete_title: "永久删除此记忆？",
    confirm_delete_text: "此操作无法撤销。",
    button_archive: "归档",
    button_delete: "删除",
    toast_created: "记忆已创建。",
    toast_updated: "记忆已更新。",
    toast_archived: "记忆已归档。",
    toast_deleted: "记忆已删除。",
    toast_error: "出错了。",
  },
  skills: {
    title: "技能管理",
    no_skill_installed: "没有已安装的技能",
    no_skill_description: "导入技能包以扩展AI能力",
    import_button: "导入技能",
    import_dialog_title: "从Zip导入技能",
    import_success: "技能导入成功",
    import_error: "技能导入失败",
    uninstall_confirm: "确定要卸载此技能吗？",
    uninstall_success: "技能已卸载",
    uninstall_error: "技能卸载失败",
    toggle_enable: "启用",
    toggle_disable: "禁用",
    toggle_success: "技能已更新",
    toggle_error: "技能更新失败",
    built_in: "内置",
    user_installed: "用户安装",
    column_name: "名称",
    column_source: "来源",
    column_category: "分类",
    column_version: "版本",
    column_status: "状态",
    column_enabled: "已启用",
    column_disabled: "已禁用",
    approval_title: "技能权限请求",
    approval_description:
      "此技能需要您的许可才能执行。它可能会访问外部资源或代表您执行操作。",
    approval_deny: "拒绝",
    approval_allow_once: "允许一次",
    approval_always_allow: "始终允许",
    approval_always_allow_session: "本次会话始终允许",
    permission_resume_failed: "已授权但无法继续执行该技能。",
    permission_resume_no_tool_id: "缺少工具调用信息，无法继续执行。",
    permission_resume_no_active_stream: "没有进行中的对话流，请重新发送消息。",
    shell_approval_title: "Shell 命令执行",
    shell_approval_description:
      "AI 请求执行本地 Shell 命令。请在批准前仔细检查命令内容。",
    shell_command_label: "命令",
    shell_cwd_label: "工作目录",
    shell_timeout_label: "超时时间",
    shell_type_label: "Shell 类型",
  },
  systemDependency: {
    dialog_title: "安装系统依赖",
    security_note: "仅安装本地目录中受信任的软件包。",
    approve: "批准安装",
    deny: "拒绝",
    status_installed: "依赖安装成功",
    status_already_installed: "依赖已安装",
    status_permission_denied: "安装被拒绝",
    status_installer_not_found: "未找到包管理器",
    status_unsupported_platform: "此平台不支持",
    status_path_issue: "已安装但命令不在 PATH 中，请尝试重启应用。",
    status_installation_failed: "安装失败",
  },
  schedule: {
    create_description: "使用cron表达式或依赖关系设置自动化任务调度",
    schedule_created_success: '计划 "{name}" 创建成功，ID: {id}',
    create_schedule_failed: "创建计划失败: {error}",
    view_schedule: "查看计划",
    basic_information: "基本信息",
    trigger_configuration: "触发器配置",
    schedule_name: "计划名称",
    schedule_name_placeholder: "输入此计划的描述性名称",
    task_type: "任务类型",
    task_id: "任务ID",
    task_id_placeholder: "输入要调度的任务ID",
    description: "描述",
    description_placeholder: "此计划功能的可选描述",
    trigger_type: "触发器类型",
    active: "激活",
    cron_expression: "Cron表达式",
    cron_expression_placeholder: "例如：0 0 * * * (每天午夜)",
    parent_schedule: "父计划",
    parent_schedule_placeholder: "选择父计划",
    dependency_condition: "依赖条件",
    delay_minutes: "延迟（分钟）",
    delay_placeholder: "父计划完成后的延迟（可选）",
    next_run_time: "下次运行时间: {time}",
    refresh: "刷新",
    cancel: "取消",
    create_schedule: "创建计划",
    update_schedule: "更新计划",
    this_field_required: "此字段为必填项",
    must_be_positive: "必须是正数",
    must_be_non_negative: "必须为非负数",
    invalid_cron_expression: "无效的cron表达式",
    failed_validate_cron: "验证cron表达式失败",
    calculating: "计算中...",
    calculation_failed: "计算下次运行时间失败",
    search_task: "搜索任务",
    email_marketing: "外联营销",
    email_extract: "联系人画像洞察",
    bulk_email: "批量邮件",
    google_maps: "Google地图",
    yandex_maps: "Yandex地图",
    social_task: "社交任务",
    cron_schedule: "Cron调度",
    dependency: "依赖",
    manual_only: "仅手动",
    on_success: "成功时",
    on_completion: "完成时",
    on_failure: "失败时",
    // Schedule List Page
    schedule_management: "计划管理",
    manage_automated_scheduling: "管理自动化任务调度和作业依赖关系",
    new_schedule: "新建计划",
    scheduler_status: "调度器状态",
    running: "运行中",
    stopped: "已停止",
    active_schedules: "活跃计划",
    total_schedules: "总计划数",
    start: "启动",
    stop: "停止",
    scheduler: "调度器",
    started: "已启动",
    // stopped:"已停止",
    search_schedules: "搜索计划",
    schedules: "计划",
    paused: "已暂停",
    inactive: "非活跃",
    failed_to_load_schedules: "加载计划失败",
    failed_to: "失败",
    delete_schedule: "删除计划",
    delete_schedule_confirm: "您确定要删除此计划吗？此操作无法撤销。",
    schedule_deleted_successfully: "计划删除成功",
    schedule_enabled_successfully: "计划启用成功",
    schedule_disabled_successfully: "计划禁用成功",
    schedule_paused_successfully: "计划暂停成功",
    schedule_resumed_successfully: "计划恢复成功",
    schedule_execution_started: "计划执行已开始",
    failed_to_enable_schedule: "启用计划失败",
    failed_to_disable_schedule: "禁用计划失败",
    failed_to_pause_schedule: "暂停计划失败",
    failed_to_resume_schedule: "恢复计划失败",
    failed_to_run_schedule: "运行计划失败",
    schedules_exported_successfully: "计划导出成功",
    failed_to_export_schedules: "导出计划失败",
    import_functionality_coming_soon: "导入功能即将实现",
    action_failed: "操作失败",
    // Schedule Table Headers
    table_name: "名称",
    table_task_type: "任务类型",
    table_status: "状态",
    table_trigger: "触发器",
    table_schedule: "调度",
    table_next_run: "下次运行",
    table_last_run: "上次运行",
    table_executions: "执行次数",
    table_actions: "操作",
    // Schedule Detail Page
    detail_title: "计划详情",
    detail_loading: "正在加载计划...",
    detail_failed_to_load: "加载计划失败",
    detail_retry: "重试",
    detail_edit: "编辑",
    detail_run_now: "立即运行",
    detail_basic_information: "基本信息",
    detail_name: "名称",
    detail_description: "描述",
    detail_no_description: "无描述",
    detail_task_type: "任务类型",
    detail_task_id: "任务ID",
    detail_status: "状态",
    detail_active: "激活",
    detail_trigger_type: "触发器类型",
    detail_created: "创建时间",
    detail_schedule_configuration: "调度配置",
    detail_cron_expression: "Cron表达式",
    detail_not_applicable: "不适用",
    detail_next_run_time: "下次运行时间",
    detail_not_scheduled: "未调度",
    detail_last_run_time: "上次运行时间",
    detail_never_run: "从未运行",
    detail_execution_count: "执行次数",
    detail_execution_history: "执行历史",
    detail_refresh: "刷新",
    // Execution History Table Headers
    execution_start_time: "开始时间",
    execution_end_time: "结束时间",
    execution_duration: "持续时间",
    execution_result: "结果",
    execution_error: "错误",
    execution_no_history: "无执行历史",
    execution_never_executed: "此计划尚未执行",
    detail_dependencies: "依赖关系",
    detail_no_dependencies: "无依赖关系",
    detail_triggers_these_schedules: "触发以下计划:",
    detail_triggered_by: "由以下触发:",
    detail_statistics: "统计信息",
    detail_success_rate: "成功率",
    detail_average_duration: "平均持续时间",
    detail_last_error: "最后错误",
    detail_no_errors: "无错误",
    detail_quick_actions: "快速操作",
    detail_edit_schedule: "编辑计划",
    detail_pause: "暂停",
    detail_resume: "恢复",
    detail_delete: "删除",
    detail_delete_schedule: "删除计划",
    detail_delete_confirm: '您确定要删除 "{name}" 吗？此操作无法撤销。',
    detail_go_to_list: "转到列表",
    detail_success: "成功",
    detail_error: "错误",
    detail_schedule_execution_started: "计划执行已开始",
    detail_schedule_paused_successfully: "计划暂停成功",
    detail_schedule_resumed_successfully: "计划恢复成功",
    detail_schedule_deleted_successfully: "计划删除成功",
    detail_failed_to_run_schedule: "运行计划失败",
    detail_failed_to_pause_schedule: "暂停计划失败",
    detail_failed_to_resume_schedule: "恢复计划失败",
    detail_failed_to_delete_schedule: "删除计划失败",
    detail_cancel: "取消",
    detail_confirm: "确认",
    // Cron Expression Builder
    cron_builder_button: "构建器",
    cron_builder_title: "Cron表达式构建器",
    cron_builder_quick_presets: "快速预设",
    cron_builder_custom_schedule: "自定义计划",
    cron_builder_minutes: "分钟",
    cron_builder_hours: "小时",
    cron_builder_days: "天",
    cron_builder_months: "月",
    cron_builder_weekdays: "星期",
    cron_builder_custom_minutes: "自定义分钟",
    cron_builder_custom_hours: "自定义小时",
    cron_builder_custom_days: "自定义天",
    cron_builder_custom_months: "自定义月",
    cron_builder_custom_weekdays: "自定义星期",
    cron_builder_copy: "复制",
    cron_builder_apply_expression: "应用表达式",
    // Preset names
    cron_preset_every_minute: "每分钟",
    cron_preset_every_hour: "每小时",
    cron_preset_daily: "每天",
    cron_preset_weekly: "每周",
    cron_preset_monthly: "每月",
    cron_preset_every_15_min: "每15分钟",
    cron_preset_every_30_min: "每30分钟",
    cron_preset_every_2_hours: "每2小时",
    cron_preset_weekdays_9_am: "工作日 9点",
    cron_preset_weekends_10_am: "周末 10点",
    // Select options
    cron_option_every_minute: "每分钟",
    cron_option_every_5_minutes: "每5分钟",
    cron_option_every_15_minutes: "每15分钟",
    cron_option_every_30_minutes: "每30分钟",
    cron_option_specific_minutes: "指定分钟",
    cron_option_every_hour: "每小时",
    cron_option_every_2_hours: "每2小时",
    cron_option_every_6_hours: "每6小时",
    cron_option_every_12_hours: "每12小时",
    cron_option_specific_hours: "指定小时",
    cron_option_every_day: "每天",
    cron_option_every_2_days: "每2天",
    cron_option_every_week: "每周",
    cron_option_specific_days: "指定天",
    cron_option_every_month: "每月",
    cron_option_every_3_months: "每3个月",
    cron_option_every_6_months: "每6个月",
    cron_option_specific_months: "指定月",
    cron_option_weekdays_only: "仅工作日",
    cron_option_weekends_only: "仅周末",
    cron_option_specific_weekdays: "指定星期",
    // Expression descriptions
    cron_desc_every_minute: "每分钟",
    cron_desc_at_minute_0: "在第0分钟",
    cron_desc_every_n_minutes: "每{count}分钟",
    cron_desc_at_minutes: "在第{minutes}分钟",
    cron_desc_of_every_hour: "每小时",
    cron_desc_at_midnight: "在午夜",
    cron_desc_every_n_hours: "每{count}小时",
    cron_desc_at_hour: "在{hour}:00",
    cron_desc_of_every_day: "每天",
    cron_desc_on_the_1st: "在1号",
    cron_desc_every_n_days: "每{count}天",
    cron_desc_on_day: "在第{day}天",
    cron_desc_of_every_month: "每月",
    cron_desc_every_n_months: "每{count}个月",
    cron_desc_in_month: "在{month}月",
    cron_desc_on_weekdays: "在工作日",
    cron_desc_on_weekends: "在周末",
    cron_desc_on_weekday: "在星期{weekday}",
    // AI Message Task
    ai_message: "AI消息任务",
    ai_message_task: "AI消息任务",
    ai_message_task_create: "创建AI消息任务",
    ai_message_task_edit: "编辑AI消息任务",
    ai_message_task_name: "任务名称",
    ai_message_task_name_hint: "为此AI消息任务输入一个名称",
    ai_message_task_description: "描述",
    ai_message_task_message: "AI消息",
    ai_message_task_message_hint: "每次定时运行时发送给AI服务器的提示",
    ai_message_task_system_prompt: "系统提示词",
    ai_message_task_system_prompt_hint: "AI的可选系统指令",
    ai_message_task_model: "AI模型",
    ai_message_task_model_hint:
      "“自动”使用服务器默认模型，可选择指定模型覆盖。",
    ai_message_task_allowed_tools: "允许的工具",
    ai_message_task_allowed_tools_hint:
      "选择AI在无人值守运行期间可以使用的内置工具",
    ai_message_task_auto_approve: "自动批准工具",
    ai_message_task_auto_approve_hint:
      "允许AI在无需手动确认的情况下执行选定工具",
    ai_message_task_auto_approve_warning:
      "警告：自动批准允许无人值守执行工具。请仅选择您信任的工具。",
    ai_message_task_max_tool_calls: "最大工具调用次数",
    ai_message_task_max_runtime: "最大运行时间（毫秒）",
    ai_message_task_max_continue_calls: "最大继续调用次数",
    ai_message_task_safety_limits: "安全限制",
    ai_message_task_no_tools_available: "没有可调度的工具",
    ai_message_task_blocked_tool: "v1中已阻止",
    ai_message_task_risk_low: "低风险",
    ai_message_task_risk_medium: "中等风险",
    ai_message_task_risk_high: "高风险",
    ai_message_task_risk_blocked: "已阻止",
    ai_message_task_run_history: "运行历史",
    ai_message_task_run_status: "运行状态",
    ai_message_task_run_started: "开始时间",
    ai_message_task_run_finished: "结束时间",
    ai_message_task_run_duration: "持续时间",
    ai_message_task_run_tool_calls: "工具调用",
    ai_message_task_run_blocked_tools: "被阻止的工具调用",
    ai_message_task_run_result: "AI响应",
    ai_message_task_run_error: "错误",
  },
  websiteAnalysis: {
    analyze_button: "AI分析",
    dialog_title: "网站分析",
    analyzing_multiple: "正在分析 {itemCount} 个选中的项目",
    business_info_label: "客户业务信息",
    business_info_placeholder:
      "请输入您的业务描述，例如：我们是一家专注于B2B SaaS解决方案的公司，主要开发项目管理和团队协作工具。",
    business_info_required: "业务信息是必填项",
    temperature_label: "温度参数",
    temperature_range_error: "温度值必须在0到2之间",
    save_for_future: "保存以供将来使用",
    customer_industry: "客户行业",
    probability: "潜在客户概率",
    analysis_result: "分析结果",
    analysis_success: "分析任务已启动成功",
    analysis_error: "分析失败",
    status: "分析状态",
    status_completed: "已完成",
    status_analyzing: "分析中",
    status_failed: "失败",
    status_pending: "待处理",
    ai_analysis: "AI分析",
    reasoning: "推理",
    client_business: "客户业务",
  },
  yellowPages: {
    ai_support: "AI 辅助",
    ai_support_hint:
      "Use AI to assist data alignment when selectors fail to read public data",
    ai_query_keywords: "AI 生成关键词",
    ai_query_keywords_default_seed: "商业",
    use_account: "使用账号",
    account_required_for_platform: "该平台需要账号：{name}",
    not_set: "未设置",
    enabled: "已启用",
    disabled: "已禁用",
    authentication: "认证",
    requires_cookies: "需要 Cookie",
    account_required_for_auth: "认证需要账号",
    requires_login: "需要登录",
    requires_api_key: "需要 API 密钥",
    requires_oauth: "需要 OAuth",
    location_required: "需要位置",
    platform_requires_location: "该平台搜索需要指定位置",
    list_manual_refresh: "手动刷新",
    list_auto_refresh_enabled: "自动刷新已开启（5秒）",
    list_auto_refresh_paused: "自动刷新已暂停（页面不可见）",
    list_auto_refresh_disabled: "自动刷新已关闭",
    list_auto_refresh_on: "自动刷新 开",
    list_auto_refresh_paused_btn: "自动刷新 已暂停",
    list_auto_refresh_off: "自动刷新 关",
    list_last_updated: "上次更新",
    list_auto_refresh_interval: "自动刷新：{seconds}秒",
    list_search_placeholder: "按任务名称或平台搜索...",
    list_filter_search: "Search",
    list_filter_status: "状态",
    list_filter_platform: "平台",
    list_filter_priority: "优先级",
    list_tasks_match_filters: "共 {totalUnfiltered} 个任务，{total} 个符合筛选",
    list_no_tasks_match_filters: "没有符合筛选条件的任务",
    list_adjust_search_or_clear: "请调整搜索条件或清除部分筛选",
    list_clear_all_filters: "清除全部筛选",
    list_applying_filters: "正在应用筛选...",
    list_task_not_running: "任务「{name}」当前未在运行",
    list_confirm_stop_task:
      "确定要停止任务「{name}」（PID：{pid}）吗？\n\n将终止正在运行的进程并更新任务状态。",
    list_task_stopped_success:
      "已成功停止任务「{name}」！\n\n任务状态已更新为「待处理」，表格将刷新显示。",
    list_task_stop_failed: "停止任务「{name}」失败：{message}",
    list_error_stopping_task: "停止任务「{name}」时出错：{message}",
    list_task_pause_failed: "暂停任务「{name}」失败：{message}",
    list_task_resumed_success: "任务「{name}」已恢复运行",
    list_task_resume_failed: "恢复任务「{name}」失败：{message}",
    list_task_still_running_results:
      "Task is still running. Results will be available when data alignment completes.",
    list_task_no_results_yet: "该任务暂无结果。",
    list_view_results_anyway_confirm: "是否仍要查看结果页？",
    list_unknown_error: "未知错误",
    list_unknown_error_occurred: "发生未知错误",
    // Task details view
    detail_task_information: "任务信息",
    detail_task_status: "任务状态",
    detail_task_actions: "任务操作",
    detail_field_name: "名称",
    detail_field_platform: "平台",
    detail_field_keywords: "关键词",
    detail_field_location: "位置",
    detail_field_max_pages: "最大页数",
    detail_field_priority: "优先级",
    detail_field_status: "状态",
    detail_field_progress: "进度",
    detail_field_results_count: "结果数量",
    detail_field_created: "创建时间",
    detail_field_last_updated: "最后更新",
    detail_action_start: "开始任务",
    detail_action_stop: "停止任务",
    detail_action_pause: "暂停任务",
    detail_action_resume: "恢复任务",
    detail_action_view_results: "查看结果",
    detail_action_edit: "编辑任务",
    detail_action_delete: "删除任务",
    detail_action_close: "关闭",
    detail_status_pending: "待处理",
    detail_status_running: "运行中",
    detail_status_completed: "已完成",
    detail_status_failed: "失败",
    detail_status_paused: "已暂停",
  },
  googleMaps: {
    title: "本地商家查找",
    description: "按关键字和位置搜索 Google 地图中的本地商家",
    query_label: "商家关键字",
    query_placeholder: "例如：牙医、意大利餐厅、水管工",
    query_hint: "输入要搜索的商家类型或名称",
    location_label: "位置",
    location_placeholder: "例如：纽约、伦敦、90210",
    location_hint: "输入城市、地址或邮编",
    max_results_label: "最大结果数",
    include_website: "包含网站",
    include_reviews: "包含评论",
    show_browser: "显示浏览器（调试）",
    start_search: "Start Search",
    cancel_search: "Cancel",
    searching: "搜索中...",
    status_idle: "准备搜索",
    status_validating: "验证输入...",
    status_launching: "启动浏览器...",
    status_loading: "加载 Google 地图...",
    status_extracting: "Aligning business information ({current}/{total})...",
    status_completed: "搜索完成",
    status_cancelled: "搜索已取消",
    status_failed: "搜索失败",
    status_timed_out: "搜索超时",
    no_results: "未找到商家",
    found_results: "在 '{location}' 找到 {count} 家 '{query}' 商家",
    col_name: "名称",
    col_category: "分类",
    col_rating: "评分",
    col_reviews: "评论数",
    col_address: "地址",
    col_phone: "电话",
    col_website: "网站",
    col_hours: "营业时间",
    export_csv: "Export To Routing Sheet",
    export_json: "导出 JSON",
    export_success: "导出成功",
    error_required: "关键字和位置为必填项",
    error_search_failed: "搜索失败：{error}",
    view_on_maps: "View public listing",
    reviews_count: "{count} 条评论",
    results_count: "结果数量",
    history_tab: "历史记录",
    history_empty: "暂无搜索历史",
    col_date: "日期",
    col_actions: "操作",
    view_results: "查看",
    delete_btn: "删除",
    account_label: "Source Channel Account",
    account_hint: "Select an account for the source channel",
    account_none: "无账号（匿名）",
    proxy_label: "代理",
    proxy_hint: "Rotate through selected proxies per card",
    proxy_placeholder: "选择代理...",
  },
  yandexMaps: {
    title: "本地商家查找",
    description: "通过关键词和位置搜索 Yandex 地图上的本地商家",
    query_label: "商家关键字",
    query_placeholder: "例如：牙医、意大利餐厅、水管工",
    query_hint: "输入要搜索的商家类型或名称",
    location_label: "位置",
    location_placeholder: "例如：莫斯科、圣彼得堡、俄罗斯",
    location_hint: "输入要搜索的城市或地区",
    max_results_label: "最大结果数",
    include_website: "包含网站",
    include_reviews: "包含评论",
    show_browser: "显示浏览器（调试）",
    language_label: "语言",
    language_placeholder: "例如：ru、en、tr",
    language_hint: "Channel Beta interface language code",
    region_label: "地区",
    region_placeholder: "例如：ru、kz、by",
    region_hint: "搜索上下文的地区代码",
    start_search: "Start Search",
    cancel_search: "Cancel",
    searching: "搜索中...",
    progress_label: "{current} / {total} 个商家",
    found_results: "在 '{location}' 找到 {count} 家 '{query}' 商家",
    copy_all: "全部复制",
    export_csv: "Export To Routing Sheet",
    export_json: "导出 JSON",
    reviews_count: "{count} 条评论",
    results_count: "结果数量",
    no_results: "未找到商家",
    status_idle: "准备搜索",
    status_validating: "验证输入...",
    status_launching: "启动浏览器...",
    status_loading: "加载 Yandex 地图...",
    status_extracting: "Aligning business information ({current}/{total})...",
    status_completed: "搜索完成",
    status_cancelled: "搜索已取消",
    status_failed: "搜索失败",
    status_captcha: "检测到验证码 — 无法继续",
    status_timed_out: "搜索超时",
    col_name: "名称",
    col_category: "分类",
    col_rating: "评分",
    col_reviews: "评论数",
    col_address: "地址",
    col_phone: "电话",
    col_website: "网站",
    col_hours: "营业时间",
    error_required: "关键字和位置为必填项",
    error_search_failed: "搜索失败：{error}",
    view_on_maps: "View public listing",
    account_label: "Source Channel Account",
    account_hint: "Select an account for the source channel",
    proxy_label: "代理",
    proxy_hint: "Rotate through selected proxies per card",
    proxy_placeholder: "选择代理...",
    search_tab: "搜索",
    history_tab: "历史记录",
    history_empty: "暂无搜索历史",
    col_date: "日期",
    col_actions: "操作",
    view_results: "查看",
    delete_btn: "删除",
  },
  mapScraper: {
    title: "本地商家查找",
    description:
      "Find public local business information through neutral source channels",
    provider_google: "Channel Alpha (Global)",
    provider_yandex: "Channel Beta (CIS Region)",
    search_tab: "搜索",
    history_tab: "历史记录",
    query_label: "商家关键字",
    query_placeholder: "例如：牙医、意大利餐厅、水管工",
    query_hint: "输入要搜索的商家类型或名称",
    location_label: "位置",
    location_placeholder_google: "例如：纽约、伦敦、90210",
    location_placeholder_yandex: "例如：莫斯科、圣彼得堡、俄罗斯",
    location_hint_google: "输入城市、地址或邮政编码",
    location_hint_yandex: "输入要搜索的城市或地区",
    max_results_label: "最大结果数",
    include_website: "包含网站",
    include_reviews: "包含评论",
    show_browser: "显示浏览器（调试）",
    language_label: "语言",
    language_placeholder: "例如：ru、en、tr",
    language_hint: "Channel Beta interface language code",
    region_label: "地区",
    region_placeholder: "例如：ru、kz、by",
    region_hint: "搜索上下文的地区代码",
    start_search: "Start Search",
    cancel_search: "Cancel",
    searching: "搜索中...",
    progress_label: "{current} / {total} 个商家",
    found_results:
      "在 '{location}' 找到 {count} 个 {provider} 商家，关键词 '{query}'",
    copy_all: "全部复制",
    export_csv: "Export To Routing Sheet",
    export_json: "导出 JSON",
    reviews_count: "{count} 条评论",
    no_results: "未找到商家",
    col_name: "名称",
    col_category: "分类",
    col_rating: "评分",
    col_reviews: "评论数",
    col_address: "地址",
    col_phone: "电话",
    col_website: "网站",
    col_hours: "营业时间",
    col_query: "查询",
    col_location: "位置",
    col_results: "结果",
    col_date: "日期",
    col_actions: "操作",
    error_search_failed: "搜索失败：{error}",
    view_on_maps: "View public listing",
    google_account_label: "Channel Alpha Account",
    yandex_account_label: "Channel Beta Account",
    account_hint: "Select an account for the source channel",
    proxy_label: "代理",
    proxy_hint: "Rotate through selected proxies per card",
    proxy_placeholder: "选择代理...",
    history_title: "{provider} 历史记录",
    history_empty: "暂无搜索历史",
    view_results: "查看",
    delete_btn: "删除",
  },
  fileOperations: {
    show_full_diff: "显示完整差异 ({count} 行)",
    collapse_diff: "收起差异",
    open_file_tooltip: "打开文件",
    open_with_tooltip: "打开文件",
  },
  agentWorkflow: {
    status_queued: "排队中",
    status_running: "运行中",
    status_waiting_policy: "等待策略审核",
    status_waiting_user: "等待用户操作",
    status_completed: "已完成",
    status_failed: "失败",
    status_cancelled: "已取消",
    status_timeout: "已超时",
    error_ai_disabled: "AI 功能未启用。",
    error_unknown_agent: "未知或已禁用的代理。",
    error_parse_failed: "代理输出无法解析。",
    tool_blocked: "工具被代理策略阻止",
    draft_label: "草稿 — 需要审核",
  },
  aiProvider: {
    title: "AI 提供商",
    description:
      "自定义提供商可通过你自己的模型驱动 AI 对话。托管版 aiFetchly AI 功能仍需订阅。",
    mode_hosted: "托管版 aiFetchly",
    mode_local: "自定义 / 本地提供商",
    hosted_enabled: "你的账户已启用托管版 aiFetchly AI。",
    hosted_requires_subscription: "托管版 aiFetchly AI 需要订阅。",
    preset: "提供商预设",
    name: "提供商名称",
    base_url: "基础 URL",
    base_url_placeholder: "http://localhost:11434/v1",
    api_key: "API 密钥（可选）",
    api_key_configured: "已配置 API 密钥",
    clear_api_key: "清除 API 密钥",
    default_model: "默认模型",
    context_size: "上下文大小（可选）",
    refresh_models: "刷新模型",
    test_connection: "测试连接",
    save: "保存",
    models_loaded: "模型列表已加载。",
    saved_local: "本地 AI 提供商已保存。AI 对话现在可以使用你配置的模型。",
    saved_hosted: "托管版 AI 提供商已保存。",
    api_key_cleared: "API 密钥已清除。",
    reasoning_settings_title: "推理",
    reasoning_settings_description: "控制 AI 对话是否显示模型推理过程。",
    show_reasoning_process: "显示推理过程",
    cap_label_models: "模型",
    cap_label_chat: "对话",
    cap_label_streaming: "流式",
    cap_label_tools: "工具",
    cap_label_vision: "视觉",
    cap_label_context: "上下文",
    cap_supported: "支持",
    cap_unsupported: "不支持",
    cap_unknown: "未知",
    cap_failed: "失败",
    indicator_hosted: "托管",
    indicator_local: "本地",
    indicator_offline: "本地离线",
    indicator_unavailable: "模型不可用",
    tool_warning: "此本地提供商尚未确认支持工具。本对话已禁用工具。",
  },
  aiArtifacts: {
    preview_title: "AI 产物",
    open: "打开产物",
    copy_html: "复制 HTML",
    copy_success: "已复制 HTML。",
    copy_error: "无法复制 HTML。",
    not_found: "未找到产物。",
    unavailable: "产物不可用",
    generated_by_ai: "由 AI 生成",
    html_artifact: "HTML 产物",
    version_label: "版本 {version}",
  },
  insights: {
    title: "洞察",
    subtitle: "在一个工作区查看业务信号、发现潜在客户并管理外联。",
    contact_profile: "联系人画像",
    statistics_description: "跟踪活动、趋势和营销表现。",
    market_description: "研究市场并找到机会。",
    contact_description: "为潜在客户提取并丰富联系人画像。",
    email_service_description: "配置 SMTP 和 IMAP 邮件服务连接。",
    email_receive_description: "查看和管理通过已配置服务接收的入站邮件。",
    email_reply_description: "审查并审核 AI 生成的自动邮件回复。",
    directory_description: "浏览目录任务和已收集的商家记录。",
    maps_description: "从地图服务中查找本地商家。",
    outreach_description: "管理外联任务、模板、过滤器和回复。",
    proxy_description: "管理和验证您的代理服务器。",
    tool_account_description: "添加、管理并连接自动化工具账户。",
  },
  outreach: {
    subtitle: "管理外联任务、模板、过滤器、邮件服务和回复。",
    tasks_description: "创建和管理批量邮件外联任务。",
    template_description: "创建和管理外联活动的邮件模板。",
    filter_description: "定义过滤器以细分和定位外联受众。",
    service_description: "配置 SMTP 和 IMAP 邮件服务连接。",
    received_messages: "接收的消息",
    receive_description: "查看和管理通过配置的服务接收的入站邮件。",
    reply_audit: "回复审核",
    audit_description: "审核 AI 生成的回复、审批状态和审核日志。",
  },
  aiChatV2: {
    queue: {
      queued: "已排队",
      steering: "转向中…",
      applied: "已应用",
      dispatching: "发送中…",
      sent: "已发送",
      paused: "队列已暂停",
      cancelled: "已移除",
      failed: "无法发送",
      steer: "转向",
      steer_aria: "用此消息调整当前回复方向",
      remove: "移除",
      send_next: "发送下一条",
      resume: "恢复队列",
      direction_updated: "方向已更新",
      recovered_after_restart: "重启后已恢复",
      attachments_not_steerable: "带附件的消息将在当前回复完成后发送。",
      limit_reached: "队列已达上限，请先移除一条消息。",
      steer_failed: "无法转向此消息。",
      queue_disabled: "消息队列当前已禁用。",
    },
    artifactExport: {
      permissionTitle: "导出生成的工件",
      permissionDescription:
        "将这些由 AiFetchly 生成的工件复制到 {destination}。",
      savedToWorkspace: "已将 {fileName} 保存到你的工作区。",
      saveFailed: "无法保存到工作区。",
    },
    imageTool: {
      name: "附加本地图片",
      permissionTitle: "将本地图片发送到 AI 服务器",
      permissionDescription:
        "助手将读取所选图片并将准备好的副本发送到 {destination}。批准后将其附加到此请求。",
      validating: "正在验证图片…",
      reading: "正在读取图片…",
      normalizing: "正在准备图片…",
      ready: "图片已就绪",
      attachedCount: "已附加 {count} 张图片",
      summary: "已为下一次 AI 请求准备 {count} 张图片。",
      errors: {
        workspaceRequired: "需要先批准一个工作区。",
        invalidArguments: "请求参数无效。",
        imageLimitReached: "此请求的图片数量已达上限。",
        pathOutsideWorkspace: "路径位于已批准的工作区之外。",
        pathNotFound: "找不到该文件。",
        pathIsDirectory: "该路径是目录,不是图片。",
        imageFileTooLarge: "图片文件过大。",
        unsupportedImageType: "不支持的图片类型。",
        imageSignatureMismatch: "文件内容与扩展名不匹配。",
        imageDimensionsTooLarge: "图片尺寸过大。",
        imagePayloadTooLarge: "图片数据过大。",
        imageProcessingFailed: "图片处理失败。",
        permissionDenied: "权限被拒绝。",
        cancelled: "附加操作已取消。",
      },
    },
    atMentions: {
      ariaLabel: "提及工作区文件",
      noWorkspace: "选择一个工作区以提及文件。",
      chooseWorkspace: "选择工作区",
      noMatches: "没有匹配的文件",
      file: "文件",
      directory: "目录",
      tooManyMentions: "提及过多。请删除一些文件后重试。",
      fileNotFound: "此工作区中未找到该文件。",
      outsideWorkspace: "提及的路径超出已批准的工作区。",
      invalidLineRange: "行范围的起始必须小于结束。",
      binaryFile: "二进制文件",
      tooLarge: "文件过大，无法包含。",
    },
    pastedText: {
      chip_label: "已粘贴文本 #{id} · {lines} 行",
      truncated_chip_label: "已截断的粘贴文本 #{id} · {lines} 行",
      view_content: "查看已粘贴内容",
      hide_content: "隐藏已粘贴内容",
      removed: "已移除已粘贴内容",
      loading: "正在加载已粘贴内容...",
    },
    goalLoop: {
      objectiveRequired: "请提供目标内容。用法：/goal <目标>",
      createFailed: "无法创建目标。",
      countRequired: "请提供迭代次数。",
      countRange: "迭代次数必须在 1 到 10 之间。",
      noActiveGoal: "请先用 /goal <目标> 设定一个目标。",
      startFailed: "无法启动循环。",
      stop: "停止循环",
      statusRunning: "运行中",
      statusComplete: "已完成",
      statusBlocked: "已阻塞",
      statusNeedsInput: "需要输入",
      statusFailed: "失败",
      statusCancelled: "已取消",
    },
    scheduledLoop: {
      createFailed: "无法创建定时循环。",
      controlFailed: "无法更新定时循环。",
      noActiveLoop: "此会话没有活动的定时循环。",
      clearConfirm: "此会话有活动的定时循环。清除将同时停止该循环。是否继续？",
      pause: "暂停",
      resume: "恢复",
      stop: "停止循环",
      stopRun: "停止本次运行",
      statusActive: "活动中",
      statusPaused: "已暂停",
      statusRunning: "运行中",
      statusExpired: "已过期",
      statusFailed: "已失败",
      statusStopped: "已停止",
      toolApprovalTitle: "批准无人值守工具",
      toolApprovalIntro:
        "定时循环在无人值守下运行。请在下方选择内置工具与扩展能力（导入技能、MCP、子代理）。Shell 与标记已处理仍被阻止。",
      toolApprovalCommand: "命令",
      noReadOnlyTools: "没有可用的只读工具。",
      allowedTools: "允许的只读工具",
      allowedToolsHint: "可选择循环可运行的工具",
      autoApprove: "自动运行已批准的工具",
      autoApproveWarning: "在循环激活期间，这些工具将在每次执行时自动运行。",
      createLoop: "创建定时循环",
      enableTools: "允许内置工具无人值守运行",
      extendedCapabilities: "扩展能力",
      allowSkills: "允许导入技能",
      allowMcp: "允许 MCP 工具",
      allowSubagents: "允许子代理",
      readOnlyAutoApproved: "只读工具自动批准",
      automationTools: "自动化工具",
      automationToolsHint: "可选 — 选择网络检查工具",
      highImpactTools: "写入 / 邮件工具 — 输入名称以启用",
      highImpactWarning:
        "这些将在每次执行时无人值守运行。注入内容可能覆盖文件或以你的身份发送邮件。请逐个输入工具名称以确认。",
      typeToConfirm: "输入 {name} 以确认",
      errors: {
        INVALID_LOOP_SYNTAX: "无法解析 /loop 命令。",
        INVALID_INTERVAL: "间隔必须在 1 分钟到 24 小时之间。",
        INVALID_LOOP_LIMIT: "运行次数或有效期限制无效。",
        PROMPT_REQUIRED: "定时循环需要提供提示词。",
      },
    },
    voice: {
      microphone: "语音输入",
      start_recording: "开始录音",
      stop_recording: "停止录音",
      recording: "录音中...",
      transcribing: "识别中...",
      speak_response: "朗读回复",
      stop_speaking: "停止朗读",
      speaking: "朗读中...",
      permission_denied: "未授予麦克风权限。",
      no_microphone: "未找到麦克风。",
      recording_failed: "录音失败。",
      transcription_failed: "语音识别失败。",
      tts_failed: "语音播放失败。",
      model_missing: "未安装语音模型。",
      install_model: "安装语音模型",
      installing_model: "安装中...",
      model_install_failed: "语音模型安装失败。",
      runtime_unavailable: "本地语音运行时不可用。",
      runtime_install_title: "安装本地语音运行时？",
      runtime_install_message:
        "语音输入需要本地语音运行时和 Whisper Base 语音模型。现在下载并安装吗？",
      runtime_install_confirm: "下载并安装",
      runtime_install_size:
        "运行时下载：{runtimeSize}。Whisper Base 模型：约 {modelSize}。",
      runtime_install_preparing: "正在准备下载...",
      runtime_install_downloading_model: "正在下载 Whisper Base... {pct}%",
      runtime_install_verifying_model: "正在验证 Whisper Base...",
      runtime_install_extracting_model: "正在安装 Whisper Base...",
      runtime_install_prepare_failed: "无法准备语音运行时下载。",
      runtime_install_failed: "语音运行时安装失败。",
      empty_transcript: "未检测到语音。",
      settings_title: "语音",
      enable_input: "启用语音输入",
      enable_spoken_responses: "启用语音回复",
      disable_spoken_responses: "关闭语音回复",
      auto_send: "自动发送语音识别结果",
      speak_after_voice_input: "仅在语音输入后朗读",
      stt_language: "语音识别语言",
      tts_language: "语音回复语言",
      voice_model: "语音模型",
      speech_speed: "语速",
      stt_model: "语音识别模型",
      tts_model: "语音回复模型",
      max_recording_duration: "最长录音时长",
      open_settings: "语音设置",
      voice_models: "语音模型",
      model_name_whisper_tiny: "Whisper Tiny（快速语音识别）",
      model_name_whisper_base: "Whisper Base（推荐语音识别）",
      model_name_whisper_small: "Whisper Small（高精度语音识别）",
      model_name_piper_amy: "Piper VITS en_US Amy（文本转语音）",
      model_type_stt: "语音识别",
      model_type_tts: "文本转语音",
      installed: "已安装",
      download: "下载",
      cancel_download: "取消",
      downloading_pct: "下载中… {pct}%",
      verifying: "校验中…",
      extracting: "解压中…",
      download_error: "错误：{error}",
      unsupported_mime: "此浏览器不支持录音。",
      conversion_failed: "无法转换录音。",
      open_model_settings: "打开设置",
      chat_unavailable: "聊天不可用，您的转写内容已保留在输入框中。",
      tts_model_missing: "语音回复需要语音模型，请先安装再启用。",
      install_tts_model: "安装语音模型",
      tts_model_install_failed: "语音模型安装失败。",
      tts_runtime_missing:
        "语音回复需要本地语音运行时，但尚未安装。请在聊天中使用语音输入进行安装，然后再启用语音回复。",
      settings_unavailable: "语音输入不可用——请打开设置",
      input_disabled: "语音输入已关闭——打开设置开启",
      permission_retry_hint: "请允许麦克风权限后重试。",
      busy: "已有操作正在进行中。",
      settings_load_failed: "无法加载语音设置。",
      settings_save_failed: "无法保存语音回复偏好设置。",
      retry: "重试",
    },
    title: "AI 助手",
    show_reasoning: "显示推理",
    hide_reasoning: "隐藏推理",
    reasoning_title: "推理",
    reasoning_streaming: "推理中…",
    reasoning_unavailable: "此模型不支持推理。",
    copy_reasoning: "复制推理",
    reasoning_copied: "推理已复制",
    new_conversation: "新对话",
    empty_title: "开始对话",
    empty_description: "随便问。历史记录只保存在本设备。",
    input_placeholder: "发送消息…",
    send: "发送",
    stop: "停止",
    retry: "重试",
    clear_conversation: "清空对话",
    clear_all: "全部清空",
    clear_chat: "清空聊天",
    compact_conversation: "压缩对话",
    compact_completed: "对话已压缩到记忆中。",
    context_usage: "上下文",
    context_usage_tooltip: "{used} / {total} tokens ({percent}%)",
    loading_models: "正在加载模型…",
    loading_history: "正在加载历史…",
    streaming: "生成中…",
    thinking: "AI 正在思考…",
    reconnecting: "正在重新连接…",
    reconnect_attempt: "(第 {n}/{max} 次尝试)",
    recovery: {
      attempt: "(第 {n}/{max} 次尝试)",
      stop_hint: "按停止键取消",
      api_retry: "正在重新连接 AI 服务器…",
      overload_retry: "AI 服务器过载，正在重试…",
      output_token_recovery: "正在恢复被截断的回复…",
      reactive_compact: "正在总结对话以适应上下文…",
      context_collapse_drain: "正在裁剪较早的上下文…",
      model_fallback: "正在切换到备用模型…",
      persistent_retry: "正在后台持续重试…",
    },
    cancelled: "已取消",
    ai_disabled: "AI 功能仅对订阅用户开放。",
    model_unavailable: "所选模型不可用。",
    model_selector_label: "模型",
    model_loading: "正在加载模型…",
    model_none_available: "没有可用模型",
    model_none_hint: "未配置可用模型。",
    model_open_settings: "打开模型服务设置",
    model_auto: "自动",
    model_auto_default: "默认",
    model_free: "免费",
    server_unavailable: "无法连接到 AI 服务器。",
    unsupported_tool_call: "助手请求了一个暂不支持的工具。",
    conversation_history: "对话历史",
    conversation_running: "对话进行中",
    manage_mcp_tools: "管理 MCP 工具",
    no_conversations: "暂无对话",
    search_conversations: "搜索对话...",
    no_search_results: "未找到相关对话",
    clear_confirm_title: "清空对话？",
    clear_confirm_body: "这将删除该对话中的所有消息。",
    tool_call_title: "工具调用",
    tool_result_title: "工具结果",
    tool_name: "工具",
    tool_arguments: "参数",
    tool_result_details: "详情",
    generated_image_alt: "AI 生成的图片",
    open_generated_image: "打开生成的图片",
    permission_denied: "权限被拒绝。该工具不会执行。",
    permission_resume_failed: "授权后无法继续执行该工具。",
    permission_resume_no_tool_id: "缺少工具调用信息，无法继续执行。",
    auth_expired: "您的登录已过期，请重新登录。",
    quota_exhausted:
      "您的订阅计划中包含的 AI 代币已用尽。请充值以继续使用 AI 功能。",
    empty_response_error:
      "AI 返回了空响应。这通常是临时的服务器问题（速率限制、超时或 502 错误）。请重新发送您的消息。",
    file_change_one: "个文件更改",
    file_changes_other: "个文件更改",
    tool_running: "运行中...",
    tool_approval_mode_label: "工具审批",
    tool_approval_mode_ask: "需要时询问",
    tool_approval_mode_auto: "自动批准",
    tool_approval_mode_full: "完全访问",
    tool_approval_mode_ask_desc: "在需要权限的工具运行前询问。",
    tool_approval_mode_auto_desc: "自动批准本对话中受信任的非 shell 工具。",
    tool_approval_mode_full_desc:
      "自动批准本对话中注册的工具。安全限制仍然生效。",
    tool_approval_mode_full_confirm_title: "启用完全访问？",
    tool_approval_mode_full_confirm_text:
      "注册工具将在本对话中无需审批提示直接运行。依赖安装和硬安全限制仍需要您的批准。",
    tool_approval_mode_full_enable: "启用完全访问",
    tool_approval_mode_full_cancel: "取消",
    tool_approval_auto_approved: "已由对话模式自动批准",
    tool_approval_waiting: "等待批准",
    tool_approval_blocked: "已被安全策略阻止",
    agent_task_list: "后台代理",
    attachments: {
      add: "附加文件",
      remove: "移除附件",
      too_many: "最多允许 {max} 个文件。",
      too_large: "{name} 超过 {maxSize} 限制。",
      unsupported: "{name} 是不支持的文件类型。",
      preparing: "正在准备附件…",
      document_indexing: "正在索引文档…",
      document_ready: "文档就绪",
      document_failed: "文档处理失败",
      image_alt: "附件图片：{name}",
    },
    generatedImageRefs: {
      useAsReference: "用作参考",
      edit: "编辑",
      saveToWorkspace: "保存到工作区",
      remove: "移除",
      clearAll: "全部清除",
      moveUp: "上移",
      moveDown: "下移",
      referenceTrayTitle: "参考图片",
      limitReached: "每次请求最多可参考 3 张图片。",
      batchOffer: "已选择超过 3 张图片。改为按批次独立处理它们吗？",
      batchConfirmTitle: "按批处理？",
      batchConfirmBody:
        "所选的 {count} 张图片将在后台批次中独立编辑，可能需要一些时间。",
      send: "发送",
      stopBatch: "停止批次",
      retryFailed: "重试失败项（{count}）",
      progressSummary: "已完成 {completed}/{requested} · 并发数 {concurrency}",
      errors: {
        generated_image_reference_invalid: "该图片引用已失效，请重新选择图片。",
        generated_image_not_owned: "只能引用本对话中生成的图片。",
        generated_image_missing: "原图已不存在，请重新生成或选择其他参考图。",
        generated_image_outside_store:
          "图片来源不在受信任的存储区域内，无法使用。",
        generated_image_symlink_rejected:
          "该文件不是常规存储图片，请选择其他生成的作品。",
        generated_image_unsupported_type:
          "不支持的图片类型，请选择 PNG、JPEG 或 WebP 图片。",
        generated_image_too_large:
          "该图片超过大小限制，请尝试更少或更小的图片。",
        generated_image_dimension_limit:
          "该图片超过尺寸限制，请调整大小或以更小尺寸重新生成。",
        generated_image_reference_limit:
          "单次请求引用的图片过多，请使用批量处理来独立编辑。",
        generated_image_ambiguous:
          "有多张图片可能符合您的请求，请选择目标图片。",
        generated_image_fusion_limit:
          "组合图片一次最多 3 张，其余请单独或批量编辑。",
        generated_image_batch_partial:
          "部分批量任务失败，已成功的结果会保留，失败的项目可以重试。",
        generated_image_batch_cancelled:
          "批次已停止，已完成的结果会保留，您可以继续处理剩余项目。",
        image_edit_unavailable:
          "图片编辑功能不可用：尚未配置支持图生图的模型。请先配置后重试。",
        image_edit_provider_failed:
          "AI 服务商编辑图片失败。请重试，或检查服务商配置。",
      },
    },
  },
  aifetchlyConfig: {
    title: "AiFetchly 配置",
    reload: "重新加载配置",
    reloadStarted: "正在重新加载 AiFetchly 配置…",
    reloadResult:
      "已重新加载 AiFetchly 配置：\n- 命令：{commandCount}\n- 诊断：{diagnosticCount}",
    reloadFailed: "重新加载 AiFetchly 配置失败。",
    status: "配置状态",
    statusResult:
      "AiFetchly 配置状态：\n- 命令：{commandCount}\n- 代理：{agentCount}\n- 钩子：{hookCount}\n- 技能：{skillCount}\n- 诊断：{diagnosticCount}\n- 上次加载：{lastReloadAt}\n- 监视器：{watcherState}",
    statusEmpty: "未加载全局配置。",
    watcherNotStarted: "未启动（阶段 14）",
    diagnosticWarning: "警告",
    diagnosticError: "错误",
    diagnosticInfo: "信息",
    workspaceTrustTitle: "工作区 AiFetchly 配置",
    workspaceTrustBody:
      "此工作区定义了 AiFetchly 配置。启用命令前请先审查并信任该配置。",
    commandDisabledUntrusted: "命令 {name} 已禁用，因为工作区配置未被信任。",
  },
  workspaceTrust: {
    title: "工作区 AiFetchly 配置",
    body: "此工作区定义了 AiFetchly 配置。在启用其指令和命令之前，请先审查并信任该配置。",
    preview: "预览",
    trustInstructions: "仅信任指令",
    trustAll: "信任所有工作区 AI 配置",
    keepDisabled: "保持禁用",
    previewEmpty: "没有可预览的 AGENTS.md 内容。",
    trustFailed: "更新信任状态失败，请重试。",
  },
  slashCommands: {
    help: "帮助",
    clear: "清空对话",
    reloadConfig: "重新加载配置",
    status: "状态",
    agents: "代理",
    helpResultTitle: "可用命令",
    noMatches: "没有匹配的命令",
    unknownCommand: "未知的斜杠命令：/{name}",
    notDispatchable: "请在 / 后输入命令名称以运行。",
    notACommand: "此消息不是斜杠命令。",
    disabledCommand: "命令 /{name} 已禁用。",
    sourceBuiltin: "内置",
    sourceUser: "用户",
    sourceWorkspace: "工作区",
    sourcePlugin: "插件",
    argumentHint: "参数",
    argumentHintNone: "无",
  },
  agentTaskList: {
    title: "后台代理",
    no_tasks: "暂无代理任务",
    refresh: "刷新",
    cancel: "取消",
    just_now: "刚刚",
    status_queued: "排队中",
    status_running: "运行中",
    status_waiting_policy: "等待策略",
    status_waiting_user: "等待用户",
    status_completed: "已完成",
    status_failed: "失败",
    status_cancelled: "已取消",
    status_timeout: "超时",
  },
  aiChatV2Plan: {
    mode_chat: "聊天",
    mode_plan: "规划",
    questions_title: "需要澄清",
    submit_answers: "提交答案",
    other_option_label: "其他",
    other_option_description: "输入你自己的答案",
    custom_answer_placeholder: "在此输入你的答案...",
    approve: "批准",
    reject: "拒绝",
    request_changes: "请求修改",
    approved: "已批准",
    approved_continue_message: "计划已批准。请立即开始执行该计划。",
    rejected_continue_message:
      "计划已拒绝。请根据以下反馈修改计划并重新提交审批。",
    changes_requested_continue_message:
      "已请求修改计划。请根据以下反馈更新计划并重新提交审批。",
    approve_failed: "计划审批未完成，请重试。",
    reject_failed: "计划拒绝未完成，请重试。",
    changes_request_failed: "请求修改计划未完成，请重试。",
    reject_feedback: "拒绝原因",
    changes_feedback: "需要修改什么？",
    no_plan_yet: "尚无计划内容。",
    status_draft: "草稿",
    status_awaiting_question: "等待回答",
    status_awaiting_approval: "等待批准",
    status_approved: "已批准",
    status_rejected: "已拒绝",
    status_completed: "已完成",
    status_cancelled: "已取消",
  },
  subagents: {
    title: "子智能体",
    tab_subagents: "子智能体",
    add_button: "添加子智能体",
    edit_title: "编辑子智能体",
    create_title: "添加子智能体",
    empty_state: "未安装任何子智能体。",
    no_filter_results: "没有匹配筛选条件的子智能体。",
    column_agent: "智能体",
    column_description: "描述",
    column_source: "来源",
    column_plugin: "插件",
    column_mode: "模式",
    column_tools: "工具",
    column_model: "模型",
    column_status: "状态",
    column_health: "健康状态",
    column_actions: "操作",
    source_builtin: "内置",
    source_plugin: "插件",
    source_workspace: "工作区",
    source_user: "手动",
    status_active: "已启用",
    status_disabled: "已禁用",
    status_warning: "有警告",
    plugin_empty: "此插件不包含子智能体。",
    delete_confirm: "删除此子智能体？",
    readonly_plugin_hint: "插件子智能体为只读。可禁用或编辑插件源码。",
    readonly_workspace_hint:
      "工作区子智能体为只读。请编辑工作区智能体文件来修改。",
    readonly_builtin_hint: "内置子智能体为只读。",
    button_cancel: "取消",
    button_save: "保存",
    field_name: "名称",
    field_id: "ID",
    field_display_name: "显示名称",
    field_id_slug: "ID 标识",
    field_description: "描述",
    field_source_file: "来源文件",
    field_component_path: "组件路径",
    field_mode: "模式",
    field_system_prompt: "系统提示词",
    field_tools: "允许的工具",
    field_model: "默认模型",
    field_max_tool_calls: "最大工具调用数",
    field_max_runtime_seconds: "最大运行时间（秒）",
    field_max_continue_calls: "最大继续调用数",
    field_output_schema: "输出模式（JSON，可选）",
    field_warnings: "警告",
    field_no_warnings: "无警告",
    field_last_updated: "上次更新",
    field_created_at: "创建时间",
    field_enabled: "启用",
    hint_id_locked: "创建后 ID 不可更改。",
    hint_tools_comma: "以逗号分隔的工具名称。",
    hint_output_schema_json: "仅限有效的 JSON 对象。",
    validation_required: "必填。",
    validation_positive: "必须为正整数。",
    validation_output_schema_invalid: "输出模式不是有效的 JSON。",
  },
  plugins: {
    tabs: {
      discover: "发现",
      installed: "已安装",
      sources: "来源",
      issues: "问题",
    },
    sources: {
      browse: "浏览来源",
      manage: "管理来源",
    },
    title: "插件",
    import_button: "导入插件",
    install_button: "安装",
    validate_button: "验证包",
    reload_button: "重新加载",
    column_plugin: "插件",
    column_version: "版本",
    column_source: "来源",
    column_source_path: "导入位置",
    column_sub_agent: "子智能体",
    column_status: "状态",
    column_skills: "技能",
    column_hooks: "Hook",
    column_mcp_servers: "MCP 服务器",
    column_permissions: "权限",
    column_actions: "操作",
    source_local: "本地",
    source_builtin: "内置",
    source_marketplace: "市场",
    source_path_unknown: "未知",
    source_path_imported_from: "导入自",
    source_path_installed_at: "安装位置",
    status_healthy: "健康",
    status_disabled: "已禁用",
    status_needs_configuration: "需要配置",
    status_partial_load: "部分加载",
    status_invalid: "无效",
    status_missing_files: "文件缺失",
    tab_overview: "概览",
    tab_skills: "技能",
    tab_commands: "命令",
    tab_hooks: "Hook",
    tab_mcp_servers: "MCP 服务器",
    tab_permissions: "权限",
    tab_diagnostics: "诊断",
    tab_manifest: "清单",
    uninstall_confirm: "卸载此插件？将移除其技能和 MCP 服务器。",
    uninstall_button: "卸载",
    import_validation_failed: "包验证失败",
    import_success: "插件导入成功",
    import_failed: "插件导入失败",
    empty_state: "尚未安装插件。点击“导入插件”从 zip 文件安装。",
    select_zip: "选择一个插件 .zip 文件",
    validating: "验证中…",
    discover_tools: "发现工具",
    test_connection: "测试连接",
    export_diagnostics: "导出诊断",
    diagnostics_copied: "诊断已复制到剪贴板",
    no_diagnostics: "暂无诊断信息。",
    no_skills: "此插件暂无技能。",
    enabled_label: "已启用",
    command_count: "命令：{count}",
    hook_count: "Hook：{count}",
    no_commands: "此插件没有斜杠命令。",
    no_hooks: "此插件没有 Hook。",
    column_command: "命令",
    column_hook: "Hook",
    column_event: "事件",
    column_matcher: "匹配器",
    column_type: "类型",
    column_aliases: "别名",
    column_argument_hint: "参数提示",
    via_plugin: "来自插件：{name}",
    install_source: {
      button: "从来源安装",
      title: "从来源安装插件",
      kind_label: "来源类型",
      kind_local_zip: "本地 Zip",
      kind_local_folder: "本地文件夹",
      kind_git: "Git",
      kind_github: "GitHub",
      kind_npm: "npm",
      kind_url: "URL",
      zip_label: "选择 .zip",
      folder_label: "选择文件夹",
      folder_hint: "您的源文件夹将被复制到插件缓存，且不会被修改。",
      git_url: "Git 地址（https 或 ssh）",
      git_ref: "分支 / 标签 / 提交（可选）",
      github_url: "GitHub 仓库或发布资产地址",
      github_ref: "分支 / 标签（可选）",
      npm_package: "包名（例如 @scope/pkg）",
      npm_version: "版本（可选）",
      npm_registry: "仓库地址（可选，HTTPS）",
      npm_token: "认证令牌（可选，不保存）",
      npm_token_hint: "仅在本次安装中使用，不会持久化。",
      url_label: "地址（.zip、git 或 GitHub 地址）",
      url_hint:
        "自动识别：.zip 下载，git 地址克隆，GitHub 地址走 release 流程。",
      install_failed: "安装失败。",
      source_kind: "安装来源",
    },
    marketplace: {
      tab_installed: "已安装",
      tab_discover: "发现",
      tab_marketplaces: "插件市场",
      tab_errors: "错误",
      add_button: "添加插件市场",
      add_title: "添加插件市场",
      add_failed: "添加插件市场失败。",
      source_label:
        "插件市场来源（owner/repo、git 地址、文件夹或 marketplace.json 地址）",
      source_hint:
        "示例：owner/repo、https://github.com/owner/repo.git、/本地/文件夹、https://host/marketplace.json",
      ref_label: "分支 / 标签 / 提交（可选）",
      search_label: "搜索",
      refresh_button: "刷新",
      refresh_all_button: "全部刷新",
      remove_button: "移除",
      install_button: "安装",
      reinstall_button: "重新安装",
      view_details: "详情",
      confirm_remove: "移除该插件市场？来自该市场的已安装插件将保留。",
      confirm_risk: "我了解风险并希望安装。",
      column_marketplace: "插件市场",
      column_owner: "所有者",
      column_plugins: "插件数",
      column_last_fetched: "最近获取",
      health_label: "状态",
      errors_label: "错误",
      no_errors: "没有插件市场错误。",
      status_all: "全部",
      status_installed: "已安装",
      status_not_installed: "未安装",
      status_different_version: "已安装不同版本",
      status_unsupported: "不支持的来源",
      status_error: "错误",
      health_healthy: "健康",
      health_disabled: "已禁用",
      health_invalid: "无效",
      health_fetch_failed: "获取失败",
      health_missing_files: "文件缺失",
      risk_mcp: "启动 MCP 服务器",
      risk_hooks: "声明钩子",
      risk_monitors: "声明监视器",
      risk_npm: "从 npm 安装",
      risk_unpinned_git: "未固定到提交",
    },
  },
  toolProgress: {
    maps_starting: "正在开始地图搜索...",
    maps_fetching: "正在从 {platform} 地图加载结果...",
    maps_found: "已找到 {expected} 家商家中的 {collected} 家",
    maps_extracting: "正在提取第 {collected} 家（共 {expected} 家）商家的详情",
    maps_finalizing: "正在整理结果...",
    contact_starting: "正在开始联系方式提取...",
    contact_found: "已提取 {expected} 个联系方式中的 {collected} 个",
    contact_finalizing: "正在整理联系方式...",
    website_analyzing: "正在分析网站内容...",
    website_finalizing: "正在完成分析...",
    partial_result:
      "显示部分结果（{collected} / {expected}）。工具已超时，但已返回已获取的数据。",
  },
  workspace: {
    title: "工作区",
    badgeLabel: "工作区",
    notSet: "未设置工作区",
    selectFolder: "选择文件夹",
    changeFolder: "更换文件夹",
    approve: "批准工作区",
    revoke: "撤销工作区",
    rootPath: "根目录",
    description: "AI 文件工具仅在被批准的工作区内运行。",
    required: {
      title: "请选择工作区文件夹",
      body: "选择一个文件夹用于 AI 文件工具的读写操作。此文件夹之外的内容不会被访问。",
      pick: "选择文件夹",
      cancel: "取消",
    },
    chooseAction: "选择工作区",
    chooseHint: "选择一个文件夹，AI 文件工具才能读写文件。",
    loading: "正在加载工作区…",
    statusApproved: "已批准",
    statusPending: "等待批准",
    statusRevoked: "访问已撤销",
    loadFailed: "无法加载工作区。",
    retry: "重试",
    pathUnavailable: "工作区文件夹当前无法访问。",
    busyReason: "当前任务结束后可用",
},
  localAiRuntime: {
    title: "本地 AI 组件",
    embedding_title: "本地向量（设备端）",
    voice_title: "本地语音（设备端）",
    not_installed: "未安装",
    ready: "就绪",
    incompatible: "与当前版本不兼容",
    corrupted: "已损坏",
    checking: "检查中…",
    download_required: "需要下载",
    update_available: "有可用更新",
    install: "安装",
    repair: "修复",
    remove: "移除",
    check_update: "检查更新",
    download_size: "下载大小",
    installed_size: "安装大小",
    version: "版本",
    downloading: "下载中",
    verifying: "校验中",
    extracting: "解压中",
    testing: "检测中",
    activating: "启用中",
    cancel: "取消",
    retry: "重试",
    remove_models: "同时移除已下载的模型",
    install_confirm: "是否安装该组件？它将在需要时下载。",
    remove_confirm: "是否移除该组件？移除后相关功能将不可用。",
    errors: {
      catalog_unavailable: "组件目录不可用，远程功能仍可正常使用。",
      download_failed: "下载失败。",
      install_failed: "安装失败。请重试；如果仍然失败，请重启应用。",
      checksum_mismatch: "完整性校验失败。",
      archive_unsafe: "压缩包未通过安全检查。",
      incompatible: "该组件与当前应用版本不兼容。",
      health_check_failed: "组件健康检查失败。",
      busy: "已有操作正在进行中。",
      remove_failed: "移除失败，请在重启后重试。",
    },
  },
  portableMemory: {
    banner: "便携记忆",
    enable: "启用便携记忆",
    disabledHint:
      "记忆目前仅保存在 AiFetchly 内部。启用便携记忆后可与其它智能体共享项目上下文。",
    gitState: "Git",
    pendingReview: "待审核",
    rejected: "已拒绝",
    conflicted: "存在冲突",
    rescan: "重新扫描",
    enableTitle: "启用便携记忆",
    loadingPreview: "正在加载预览…",
    plannedFiles: "将要创建的文件",
    existingRecords: "检测到已有的记忆文件",
    identityInvalid:
      "工作区身份文件无效。请先修复或删除 .aifetchly/workspace.json。",
    visibility: "共享模式",
    visibilityLocal: "仅本地",
    visibilityLocalHint: "文件仅保留在本机，通常在 Git 中忽略。",
    visibilityTeam: "可共享给团队",
    visibilityTeamHint: "文件可提交到 Git，供团队成员和其它智能体使用。",
    teamWarning:
      "已提交的记忆即使之后删除也会留在 Git 历史中。提交前请检查差异。密钥过滤器可降低风险，但无法识别所有敏感内容。",
    importPolicy: "外部更改审核",
    policyReviewNew: "审核新记录",
    policyAutomatic: "自动导入",
    policyReviewAll: "审核全部更改",
    exportExisting: "导出现有记忆",
    exportNone: "不导出",
    exportActive: "导出活跃记忆",
    exportAll: "导出活跃和已归档记忆",
    bridges: "智能体说明桥接",
    bridgesHint:
      "可选的托管区块，引导 Claude Code、Codex 等智能体读取记忆索引。",
    enableConfirm: "启用",
    bridgeAction: {
      create: "将创建",
      insert: "将添加",
      replace: "将更新",
      "no-op": "已安装",
      blocked: "已阻止——请手动处理",
    },
    storageMode: "存储模式",
    storagePrivate: "私有（仅 SQLite）",
    storagePortableLocal: "便携 — 本地",
    storagePortableTeam: "便携 — 团队",
    regenerateIdentity: "重新生成身份",
    regenerateIdentityWarning:
      "重新生成工作区身份会为有意的分叉创建新的便携 UUID。记录 ID 在作用域唯一性下保留。原始与分叉将共存，无共享变更。",
    regenerateGitWarning:
      "当前身份被 Git 跟踪。重新生成会更改所有克隆的已提交身份；请与团队协调。",
    regenerateConfirm: "重新生成",
    sync: {
      synced: "已同步",
      "pending-review": "待审核",
      rejected: "已拒绝",
      conflicted: "冲突",
      missing: "缺失",
      detached: "已分离",
      private: "私有",
    },
    reviewTitle: "审核外部更改",
    loadingReview: "正在加载待处理更改…",
    noPending: "无待处理更改。",
    reviewNew: "新记录",
    reviewEdits: "已知记录的编辑",
    reviewDeletions: "已删除文件",
    approve: "批准",
    reject: "拒绝",
    approveDeletion: "确认删除",
    rejectDeletion: "保留记录",
    review: "审核",
    settings: "设置",
    revealFile: "显示文件",
    diagnosticsTitle: "便携记忆诊断",
    loadingDiagnostics: "正在加载诊断…",
    noDiagnostics: "无诊断。所有便携记忆文件均有效。",
    notRecoverable: "不可恢复",
    conflictTitle: "解决冲突",
    loadingConflict: "正在加载冲突…",
    noConflict: "未找到冲突。",
    conflictWarning:
      "在你读取与保存之间，记忆文件被外部编辑过。请选择要保留的版本。",
    aiFetchlyVersion: "AiFetchly 投影",
    fileVersion: "当前文件",
    fileAbsent: "(文件缺失)",
    mergeEditor: "合并编辑器",
    title: "标题",
    content: "内容",
    type: "类型",
    status: "状态",
    confidence: "置信度",
    actionUseFile: "使用文件版本",
    actionUseAiFetchly: "使用 AiFetchly 版本",
    actionMerge: "手动合并",
    resolve: "解决",
    typeProject: "项目",
    typeDecision: "决策",
    typeWorkflow: "工作流",
    typeConvention: "约定",
    typeReference: "参考",
    typeWarning: "警告",
    statusActive: "活跃",
    statusArchived: "已归档",
    statusContradicted: "已矛盾",
  },

  workspaceMemory: {
    title: "工作区记忆",
    panelTitle: "工作区记忆",
    memoryAction: "记忆",
    noWorkspace: "使用工作区记忆前请先选择工作区。",
    empty: "暂无工作区记忆。",
    emptyHint: "此处保存的记忆仅适用于当前工作区。",
    create: "创建记忆",
    edit: "编辑记忆",
    archive: "归档",
    delete: "删除",
    deleteConfirm: "永久删除此工作区记忆？",
    deleteSuccess: "工作区记忆已删除。",
    search: "搜索",
    searchPlaceholder: "搜索工作区记忆...",
    typeProject: "项目",
    typeDecision: "决策",
    typeWorkflow: "工作流",
    typeConvention: "约定",
    typeReference: "参考",
    typeWarning: "警告",
    statusActive: "活跃",
    statusArchived: "已归档",
    statusContradicted: "已矛盾",
    showArchived: "显示已归档",
    source: "来源",
    sourceManual: "手动",
    sourceChatV2: "聊天",
    sourceAgentTask: "智能体任务",
    sourceAutoDream: "自动整理",
    confidence: "置信度",
    lastUsedAt: "上次使用",
    updatedAt: "更新时间",
    injectionEnabled: "工作区记忆注入",
    autoDreamEnabled: "工作区自动整理",
    manualMemoryEnabled: "手动工作区记忆",
    runAutoSummary: "RUN AUTO SUMMARY",
    autoDreamRunning: "正在整合工作区记忆...",
    autoDreamFailed: "上次自动整理运行失败。",
    autoDreamLastRun: "上次运行：{time}",
    save: "保存",
    cancel: "取消",
    createError: "保存工作区记忆失败。",
    typeField: "类型",
    titleField: "标题",
    contentField: "内容",
    statusField: "状态",
    loadError: "加载工作区记忆失败。",
  },
  diagnostics: {
    title: "诊断",
    storageUsage: "已用存储：",
    refresh: "刷新",
    enableDebug: "启用调试日志（24小时后自动关闭）",
    debugExpiresAt: "调试日志到期时间：",
    allowUpload: "允许上传崩溃报告（仅手动发送）",
    openFolder: "打开诊断目录",
    exportReport: "导出诊断报告",
    sendReport: "发送崩溃报告",
    clearLocal: "清除本地诊断数据",
    clearConfirm: "确定清除所有本地诊断数据？此操作不可撤销。",
    selectCrash: "选择要发送的崩溃",
    exportFailed: "导出失败或已取消。",
    sendSuccess: "报告已发送，谢谢！",
    sendFailed: "发送失败：",
  },
  about: {
    title: "关于 AiFetchly",
    app_name: "AiFetchly",
    version: "版本",
    website: "网站",
    open_website: "打开网站",
    check_for_updates: "检查更新",
    restart_to_install: "重启并更新",
    status_idle: "点击检查最新版本。",
    status_checking: "正在检查更新…",
    status_up_to_date: "已是最新版本 ({version})。",
    status_downloading: "正在下载更新…",
    status_ready_to_restart: "更新已下载，重启以应用。",
    status_error: "无法检查更新，请稍后再试。",
    unsupported_development: "开发版本不支持更新检查。",
    unsupported_store: "更新由 Microsoft Store 管理。",
    unsupported_platform: "此平台不支持自动更新。",
    open_website_failed: "无法打开网站，请稍后再试。",
    cooldown_active: "刚刚已检查，请稍候一分钟后再试。",
  },
  workspaceChat: {
    mode: {
      classic: "使用经典聊天",
      makeDefault: "设为我的默认聊天",
    },
    newChat: "新聊天",
    automations: "自动化任务",
    customize: "自定义",
    insights: "洞察",
    knowledgeLibrary: "知识库",
    plugins: "插件",
    backToApp: "返回应用",
    loadOlder: "加载更早的消息",
    empty: {
      title: "随便问点什么，或在左侧选择一个会话。",
    },
    search: {
      placeholder: "搜索会话",
    },
    sidebar: {
      region: "聊天工作区",
      loadError: "工作区加载失败",
      empty: "暂无会话",
      unassigned: "其他聊天",
      workspace: "工作区",
      needsAttentionCount: "{count} 个会话需要关注",
    },
    status: {
      running: "运行中",
      queued: "排队中",
      awaitingPermission: "需要权限",
      awaitingUser: "需要您的输入",
      failed: "失败",
      interrupted: "已中断",
      cancelled: "已取消",
      completedUnread: "已完成，未读",
      completed: "已完成",
      needsAttention: "需要关注",
      idle: "",
    },
    headerStatus: {
      needsPermission: "需要权限",
      needsYourInput: "需要您的输入",
      recovering: "恢复中",
      failed: "失败",
      running: "运行中",
      runningTools: "运行中 · {count} 个工具",
      queued: "排队中",
    },
    header: {
      overflow: "会话菜单",
      rename: "重命名聊天",
      renamePrompt: "重命名聊天",
      compact: "压缩会话",
      clear: "清空消息",
      clearConfirm: "确定清空该会话的全部消息吗？",
      delete: "删除聊天",
      export: "导出会话",
      duplicate: "复制会话",
      deleteConfirm: "删除该会话？所有消息和生成的产物都将被移除，且无法恢复。",
    },
    runStrip: {
      goal: "目标：{objective}",
      loopRunning: "定时循环运行中",
      loopPaused: "定时循环已暂停",
      stop: "停止",
      details: "详情",
      running: "处理中…",
      queued: "排队中",
      permission: "等待批准",
      userInput: "等待您的回答",
      failed: "失败",
      recovering: "恢复中",
      interrupted: "已中断",
    },
    inspector: {
      region: "检查器",
      toggle: "切换检查器",
      artifacts: "产物",
      activity: "活动",
      context: "上下文",
      resize: "调整检查器宽度",
    },
    artifacts: {
      selectConversation: "选择一个会话以查看其生成的内容。",
      empty: "生成的报告和页面会显示在这里。请求生成 HTML 报告即可创建。",
    },
    activity: {
      goal: "目标",
      iterations: "迭代次数",
      scheduledLoop: "定时循环",
      pause: "暂停",
      resume: "继续",
      stopLoop: "停止循环",
      stopped: "已停止",
      nextRun: "下次运行",
      execution: "执行",
      runs: "运行记录",
      selectConversation: "选择一个会话以查看其运行记录。",
      empty: "暂无活动",
      owner: {
        interactive: "聊天",
        scheduled: "定时任务",
        goal: "目标",
        agent: "智能体",
      },
    },
    context: {
      selectConversation: "选择一个会话以查看其上下文。",
      workspace: "工作区",
      noWorkspace: "该聊天未绑定工作区。",
      usage: "上下文用量",
      used: "已用",
      tokens: "个 token",
      attachmentsSection: "附加上下文",
      noAttachments: "该会话暂无附件或引用文件。",
      atMention: "引用的文件",
      compact: "压缩会话",
      trusted: "已信任",
      revoked: "已撤销",
    },
    execution: {
      groupTitle: "执行",
      progress: "已完成 {completed}/{total}",
      runningTool: "正在运行 {tool}",
      legacyReceipt: "历史工具活动",
      artifactCreated: "HTML 报告已生成 — 可在“产物”中打开",
      openArtifact: "打开",
      imagesGenerated: "已生成图片",
      filesChanged: "文件已更改 — 见“活动”",
      permissionNeeded: "需要权限决定",
      structuredResult: "结构化结果 — 详情见“活动”",
    },
    plan: {
      version: "版本",
      current: "当前",
      changeReason: "变更原因",
      clarificationAnswers: "澄清回答",
      decisions: "决策记录",
      submitted: "方案已提交",
      readyForReview: "方案待审阅",
      steps: "{count} 个步骤",
      changedSince: "自 v{version} 以来的变更",
      reviewFullPlan: "查看完整方案",
      requestChanges: "请求修改",
      approve: "批准方案",
      planningQuestions: "方案问题",
      questionProgress: "第 {current} 题，共 {total} 题",
      multiSelect: "可选择多项",
      singleSelect: "选择一项",
      reviewAndSubmit: "检查并提交",
      answerReview: "您的回答",
      changeFeedbackPrompt: "这个方案需要修改什么？",
      moreActions: "更多操作",
      discardConfirm: "废弃此方案？此操作无法撤销。",
      discard: "废弃方案",
      submitError: "提交失败，请重试。",
      customAnswer: "自定义回答（可选）",
      approvedAt: "批准于",
      viewInActivity: "在“活动”中查看",
      receiptApproved: "方案已批准",
      receiptCompleted: "方案已完成",
      receiptRejected: "方案已废弃",
      receiptCancelled: "方案已取消",
      receiptChangesRequested: "已请求修改",
      status: {
        draft: "起草中",
        awaiting_question: "澄清中",
        awaiting_approval: "待批准",
        approved: "已批准",
        rejected: "已废弃",
        executing: "执行中",
        completed: "已完成",
        cancelled: "已取消",
      },
    },
  },
  aiContentReport: {
    action: "举报 AI 内容",
    actionAriaLabel: "举报此 AI 生成的内容",
    dialogTitle: "举报 AI 内容",
    outputPreview: "AI 内容",
    imagesLabel: "图片",
    imageAlt: "生成的图片 {n}",
    categoryLabel: "此内容有什么问题？",
    commentLabel: "补充说明（可选）",
    consent:
      "所选的 AI 内容和您的描述将发送给 AiFetchly 进行审核。您的提示词、其他消息、文件和 AI 推理过程不会被包含。",
    privacyPolicy: "隐私政策",
    submit: "提交举报",
    cancel: "取消",
    tryAgain: "重试",
    copyReference: "复制编号",
    success: "举报已提交。编号：{reportId}",
    reported: "已举报",
    imageUnavailable: "此图片无法附加。您仍可凭描述提交举报。",
    categories: {
      hate_or_harassment: "仇恨或骚扰",
      sexual_content: "色情内容",
      violence_or_self_harm: "暴力或自残",
      child_safety: "儿童安全",
      illegal_or_dangerous: "违法或危险",
      privacy_or_personal_data: "隐私或个人数据",
      misinformation_or_deception: "虚假信息或欺骗",
      copyright_or_ownership: "版权或所有权",
      other: "其他",
    },
    errors: {
      categoryRequired: "请选择一个类别。",
      imageRequired: "请至少选择一张图片进行举报。",
      noEvidence: "请添加描述以便提交举报。",
      network: "无法提交举报。您的填写内容已保留，可重试。",
      auth_failed: "身份验证失败。您的填写内容已保留，可重试。",
      invalid_evidence: "举报证据无效。您的填写内容已保留，可重试。",
      payload_too_large: "举报内容过大。您的填写内容已保留，可重试。",
      rate_limited: "提交的举报过多，请稍后再试。",
      service_disabled: "举报功能暂时不可用，请稍后再试。",
      server_error: "无法提交举报。您的填写内容已保留，可重试。",
      unknown: "无法提交举报。您的填写内容已保留，可重试。",
    },
  },
  communityPlugins: {
    title: "社区插件",
    refresh: "刷新",
    install: "安装",
    installed: "已安装",
    preview: "预览",
    upgrade: "升级",
    signIn: "登录",
    empty: "暂无可用插件",
    error: "无法连接插件中心",
    retry: "重试",
    installFuture: "将在未来版本中支持安装。",
    loading: "正在加载插件…",
    sessionExpired: "登录已过期",
    signInAgain: "重新登录",
    installFailed: "安装失败",
    searchLabel: "搜索插件",
    searchPlaceholder: "按名称、描述、作者或标签搜索",
    allTags: "全部",
    moreTags: "更多",
    filterLabel: "可用性",
    filterAll: "全部",
    filterAvailable: "可安装",
    filterInstalled: "已安装",
    resultCount: "{count} 个插件",
    noMatchesTitle: "没有匹配“{query}”的插件",
    noMatchesDescription: "请尝试其他搜索或清除筛选条件。",
    clearFilters: "清除筛选",
    manage: "管理",
    moreTagCount: "还有 {count} 个标签",
    statusInstalled: "已安装",
    statusUpgradeRequired: "需要升级",
    statusSignInRequired: "需要登录",
    statusComingSoon: "即将推出",
    statusUnavailable: "不可用",
  },
  ui: {
    inspector: {
      region: "检查器",
      close: "关闭检查器",
      scheduleTitle: "定时任务详情",
    },
    state: {
      loading: "加载中…",
      emptyTitle: "这里还没有内容",
      emptyBody: "创建第一条记录即可开始。",
      noResultsTitle: "没有匹配的记录",
      noResultsBody: "记录可能被当前筛选条件隐藏了。",
      clearFilters: "清除筛选",
      errorTitle: "出错了",
      errorBody: "操作失败，请重试。",
      errorLoad: "记录加载失败。",
      missingRecord: "该记录已不可用。",
      forbiddenTitle: "功能不可用",
      forbiddenBody: "当前方案未启用此功能。",
    },
    actions: {
      retry: "重试",
      cancel: "取消",
      copy: "复制",
      dismiss: "关闭",
    },
    detail: {
      status: "状态",
      schedule: "定时",
      updated: "更新于",
    },
    task: {
      active: "启用",
      paused: "已暂停",
      viewActivity: "查看活动",
    },
    settings: {
      categories: "设置分类",
      saving: "保存中…",
      saved: "已保存",
      saveFailed: "保存失败",
    },
    landing: {
      continue: "继续你的工作",
      attention: "需要关注",
      recentOutcomes: "近期结果",
      suggested: "建议操作",
    },
  },
  managedBrowser: {
    platform_fallback: "社交",
    unknown_account: "未知账号",
    proxy_active: "代理",
    screenshot: {
      capture: "刷新缩略图",
      alt: "最新浏览器截图",
    },
    start: {
      title: "启动托管浏览器",
      empty: "暂无可用账号。请先添加已保存登录的账号。",
      account_label: "账号",
      purpose_placeholder: "浏览器要做什么？",
      start: "启动浏览器",
      confirm_title: "启动托管浏览器？",
      confirm_body:
        "将为 {account} 打开一个可见的 Chrome 窗口，并使用其已保存的登录会话。",
    },
    approval: {
      title: "浏览器操作需要确认",
      body: "助手想在托管浏览器中执行一个重要操作：",
      allow: "允许一次",
      deny: "拒绝",
    },
    notices: {
      login_required: "需要登录——浏览器窗口正在等待您",
      login_verifying: "正在验证您的登录…",
      login_verified: "登录已验证——会话已保存",
      login_verification_failed: "暂时无法验证登录",
      session_persistence_failed: "无法保存刷新后的登录会话",
      challenge_detected: "检测到验证挑战——请接管",
      challenge_provider_started: "正在尝试自动解决挑战…",
      challenge_resolved: "挑战已解决",
      challenge_failed: "挑战无法自动解决",
      challenge_manual_action_required: "需要您在浏览器窗口中完成验证",
      task_resuming: "正在继续任务",
      browser_crashed: "浏览器会话意外结束",
      cache_clear_deferred: "缓存将在会话结束后清理",
      cache_clear_completed: "浏览器缓存已清理",
      cache_clear_failed: "浏览器缓存清理失败",
      browser_state_blocked: "已拦截并安全关闭浏览器弹窗（对话框、弹窗或下载）",
    },
    states: {
      starting: "启动中",
      validating_fingerprint: "检查浏览器",
      applying_session: "应用登录会话",
      verifying_login: "验证登录",
      login_required: "需要登录",
      user_login_in_progress: "等待您登录",
      verifying_manual_login: "正在验证您的登录",
      ready: "就绪",
      running: "工作中",
      awaiting_approval: "等待确认",
      challenge_detected: "验证挑战",
      challenge_resolving: "正在解决挑战",
      handoff: "您已接管",
      stopping: "停止中",
      stopped: "已停止",
      failed: "失败",
    },
    effects: {
      publish_video: "发布视频",
      upload: "上传文件",
      comment: "发表评论",
      reply: "回复评论",
      delete: "删除内容",
      subscribe: "订阅频道",
    },
    controls: {
      take_over: "接管",
      pause_ai: "暂停 AI",
      verify_login: "我已完成登录",
      resume: "继续任务",
      extend: "延长时间",
      stop: "停止浏览器",
    },
    errors: {
      ai_disabled: "AI 功能未启用",
      managed_browser_disabled: "托管浏览器已禁用",
      account_in_use: "该账号已被使用",
      global_session_limit: "已达到会话上限",
      browser_dependency_missing: "浏览器未安装或不可用",
      worker_exited: "浏览器进程意外退出",
      worker_start_timeout: "浏览器启动超时",
      challenge_requires_handoff: "此页面需要您的确认",
      action_not_allowed: "当前不允许此操作",
      cache_clear_deferred: "缓存清理等待会话结束后执行",
      internal_error: "出现异常",
    },
    handoff: {
      title: "浏览器交接",
      account_line: "您正在控制以下账号的浏览器窗口：",
      time_remaining: "此登录窗口剩余时间：",
      saved_login_hint:
        "请在打开的浏览器窗口中登录。已保存的登录会话会保留——清理缓存不会影响它。",
      verify_login: "我已完成登录",
      extend: "延长时间（10 分钟）",
      cancel_task: "取消任务",
      reasons: {
        login_required: "站点要求登录后才能继续。",
        login_expired: "已保存的登录会话已过期。",
        captcha_sensitive_flow: "此步骤需要人工确认。",
        mfa_prompt: "站点要求多因素认证。",
        passkey_prompt: "站点要求使用通行密钥。",
        password_field: "密码输入框需要由您填写。",
        recovery_flow: "账号进入了找回流程。",
        account_selection_ambiguous: "要使用的账号不明确。",
        browser_permission_prompt: "浏览器请求了只有您能授予的权限。",
        destructive_action_unclear: "此操作可能具有破坏性，需要您确认。",
        repeated_action_failure: "该步骤反复失败，请手动完成。",
        user_requested: "您要求接管此步骤。",
        challenge_unresolved: "验证挑战无法自动解决。",
      },
    },
    settings: {
      title: "托管浏览器",
      description:
        "控制 AI 执行已登录任务时使用的内置社交浏览器，并管理其缓存。",
      browser_enabled: "启用托管浏览器",
      active_session_title: "浏览器会话正在进行",
      active_session_body: "禁用托管浏览器前，需要决定如何处理进行中的会话。",
      active_finish: "完成当前会话",
      clear_account_label: "账号",
      clear_selected: "清理所选账号",
      cache_phase_scanning: "正在扫描缓存…",
      cache_max_label: "缓存上限（MB）",
      clear_choice_stop: "停止浏览器并立即清理",
      clear_choice_defer: "浏览器关闭后清理",
      cache_phase_deleting: "正在删除…",
      cache_phase_done: "完成",
      cache_phase_failed: "失败",
      active_stop_now: "立即停止",
      stop_failed:
        "无法停止进行中的会话——设置保持启用。请重试或在聊天卡片中停止浏览器。",
      cache_enabled: "在会话之间保留磁盘缓存",
      clear_cache_on_exit: "应用退出时清理缓存",
      cache_size: "缓存大小：{size}",
      last_cleared: "上次清理 {when}",
      clear_all: "清理缓存",
      clear_preserves_logins: "清理缓存会保留已保存的登录和账号数据。",
      clear_confirm_title: "清理浏览器缓存？",
      clear_confirm_body: "约 {size} 的缓存文件将被删除。",
      clear_confirm_preserved: "您已保存的登录会话和账号数据将被保留。",
      clear_confirm_ok: "清理缓存",
      disabled_reasons: {
        user_setting_disabled: "托管浏览器已在设置中关闭。",
        release_flag_disabled: "托管浏览器已被发布标志暂时禁用。",
      },
    },
  },
  aiConversationReport: {
    action: "举报对话",
    actionAriaLabel: "举报此对话以供审核",
    unavailable: "对话举报功能当前不可用。",
    noEligibleOutputs: "此对话中暂无可举报的 AI 输出。",
    dialogTitle: "举报对话",
    selectionInstruction: "选择要举报的 AI 输出。",
    selectionCount: "已选择 {n} 项",
    selectionCountOfMax: "已选择 {n}/{max} 项",
    selectAll: "全选",
    includeRelatedUserContext: "包含我触发该 AI 输出的相关消息",
    userMessageWillBeSent:
      "继续后，您选择的相关消息将与 AI 输出一并发送给 AiFetchly。",
    relatedUserLabel: "您的消息——将被发送",
    attachmentOmitted: "您消息中的附件已被省略；仅包含消息文本。",
    imageLabel: "将图片包含在报告中",
    generatedAtLabel: "生成时间",
    consentDefault:
      "仅发送您选择的 AI 输出和说明。您的其他消息、文件及 AI 推理过程不会包含在内。",
    consentWithUserContext:
      "您已选择包含相关消息。该消息将与您选择的 AI 输出一并发送。",
    truncationWarning: "过长的输出已裁剪以符合报告大小限制。",
    listTruncated:
      "显示前 {shown} 条，共 {total} 条输出。请在对话中滚动以举报其他输出。",
    continueAndSubmit: "提交举报",
    cancel: "取消",
    conversationChanged: "报告打开期间对话已更改。请重新打开。",
    categoryLabel: "此对话有什么问题？",
    commentLabel: "补充说明（可选）",
    itemTypes: {
      text: "文本",
      image: "图片",
      mixed: "文本和图片",
      plan: "计划",
      artifact: "产物",
    },
    errors: {
      selectionRequired: "请至少选择一个要举报的 AI 输出。",
      selectionLimit: "最多可选择 10 个 AI 输出。",
      imageLimit: "每份报告最多包含 3 张图片。",
      relatedMessageUnavailable: "所选输出没有可用的相关消息。",
      unsupportedSchema: "不支持此举报类型。请更新应用。",
    },
  },
  // 应用退出与系统托盘（PRD application-exit-and-system-tray FR-08）
  applicationLifecycle: {
    closeTitle: "关闭 AiFetchly？",
    closeDescription:
      "退出将停止正在运行的任务。保持运行会隐藏窗口，任务继续在系统托盘中运行。",
    activeTasks: "{n} 个任务正在运行。",
    keepRunning: "保持在系统托盘运行",
    exitApplication: "退出应用程序",
    cancel: "取消",
    exiting: "正在退出 AiFetchly…",
    stoppingTasks: "正在停止运行中的任务…",
    forceStop: "正在强制停止剩余任务…",
    finalize: "正在保存结果并关闭…",
    trayUnavailable: "系统托盘不可用。窗口将保持打开，请使用“退出”关闭应用。",
    trayOpen: "打开 AiFetchly",
    trayExit: "退出应用程序",
    trayTooltip: "AiFetchly",
  },
};
