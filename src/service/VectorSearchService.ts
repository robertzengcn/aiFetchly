...
export class VectorSearchService {
  ...
  private availableEmbeddingModelsCache: Set<string> | null = null;
  ...
  async search(
    ...
    try {
      ...
      for (const [modelKey, docs] of documentsByModel.entries()) {
        ...
        // Generate query embedding for this model with resilient model-name fallbacks.
        const queryVector = await this.generateQueryEmbeddingForModel(
          query,
          modelName,
          numericDimensions
        );
      ...
      const results = await this.getDetailedResults(
        combinedResults.chunkIds,
  ...
  private normalizeModelKey(modelName: string): string {
    const withoutProvider = modelName.includes("/")
      ...
      : modelName;
    return withoutProvider.toLowerCase().replace(/[^a-z0-9]/g, "");
  }
  ...
  private getModelCandidates(
    ...
    const pushCandidate = (candidate: string): void => {
      ...
      if (!orderedCandidates.includes(trimmedCandidate)) {
        orderedCandidates.push(trimmedCandidate);
      }
    };

    pushCandidate(modelName);
    const slashCandidate = modelName.includes("/")
      ? modelName.split("/").pop()
      : null;
    if (slashCandidate) {
      pushCandidate(slashCandidate);
    }

    const normalizedInput = this.normalizeModelKey(modelName);
    for (const availableModel of availableModels) {
      const normalizedAvailable = this.normalizeModelKey(availableModel);
      if (
        normalizedAvailable === normalizedInput ||
        normalizedAvailable.includes(normalizedInput) ||
        normalizedInput.includes(normalizedAvailable)
      ) {
  ...
  private async generateQueryEmbeddingForModel(
    query: string,
    modelName: string,
    ...
    for (const candidateModel of modelCandidates) {
      try {
        ...
        if (!response.status || !response.data || !response.data[0]) {
          lastErrorMessage = response.msg || "Unknown error";
          continue;
        }
  ...
  private async getAvailableEmbeddingModels(): Promise<Set<string>> {
    if (this.availableEmbeddingModelsCache) {
      return this.availableEmbeddingModelsCache;
    }

    try {
      const response = await this.ragConfigApi.getAvailableEmbeddingModels();
      const modelEntries = response.data?.models;

      if (!response.status || !modelEntries) {
        this.availableEmbeddingModelsCache = new Set<string>();
        return this.availableEmbeddingModelsCache;
      }

      this.availableEmbeddingModelsCache = new Set<string>(
        Object.keys(modelEntries)
      );
      return this.availableEmbeddingModelsCache;
    } catch (error) {
      console.warn(
   ...
   */
  private async groupDocumentsByModel(
    documents: Array<{ id: number; vectorIndexPath: string | null }>
  ): Promise<
    Map<string, Array<{ id: number; vectorIndexPath: string | null }>>
  > {
    const documentsByModel = new Map<
      string,
   ...
   */
  private async getAllDocumentsWithEmbeddings(): Promise<
    Array<{ id: number; vectorIndexPath: string | null }>
  > {
    try {
    ...
    try {
      ...
      if (document && document.modelName) {
        ...
        if (!document.vectorDimensions) {
          throw new Error(
            `No vector dimensions found for document ${documentId}`
          );
        }
        return {
