...
export class DocumentService {
    ...
    constructor() {
        ...
        this.stagedAttachmentRoot = path.join(app.getPath('userData'), 'ai-chat-attachments');
    }
    ...
    async stageAttachmentMarkdown(
        ...
        const safeConversationId = this.sanitizePathSegment(conversationId || 'default');
    ...
    private resolveSupportedExtension(fileName: string, mimeType: string): '.pdf' | '.csv' | '.docx' {
        ...
        if (lowerName.endsWith('.xlsx') || lowerMime === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
            throw new Error('Excel attachments are not supported');
        }
