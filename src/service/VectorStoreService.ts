...
export class VectorStoreService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      log.info(`${this.databaseType} vector database initialized successfully`);
    } catch (error) {
      log.error(
  ...
  async storeEmbedding(embeddingData: {
    ...
    if (
      ...
      if (embeddingData.vectorIndexPath) {
        indexExists = this.checkIndexExistsByPath(
      ...
      const pooledInstance = this.getPooledDocumentInstance(
        embeddingData.documentId,
        modelConfig
      );
      ...
      if (!indexExists) {
        ...
        if (embeddingData.vectorIndexPath) {
          await this.createDocumentIndexByPath(
            embeddingData.vectorIndexPath,
            embeddingData.documentId,
            modelConfig
          );
      ...
      await this.addVectorsWithInstance(
        pooledInstance,
        embeddingData.embedding,
        embeddingData.chunkId
      );
    } else {
      throw new Error(
        "Document ID, model, and dimensions are required for document-specific indexing"
      );
    }
  }
   ...
   * @param chunkIds - Array of chunk IDs corresponding to vectors
   */
  private async addVectorsWithInstance(
    instance: IVectorDatabase,
    vectors: number[],
    chunkIds: number
  ): Promise<void> {
    ...
    try {
      ...
      log.error(`Failed to add vectors to ${this.databaseType} index:`, error);
  ...
  private getInstanceKey(
    ...
    if (documentId) {
      return VectorDatabaseKeyGenerator.generateDocumentKey(
        documentId,
        modelConfig,
        this.indexPath
      );
    } else {
      return VectorDatabaseKeyGenerator.generateModelKey(
        modelConfig,
        this.indexPath
      );
    }
   ...
   */
  private getPooledDocumentInstance(
    documentId: number,
    modelConfig: EmbeddingModelConfig
  ): IVectorDatabase {
   ...
   * @returns Pooled vector database instance
   */
  private getPooledModelInstance(
    modelConfig: EmbeddingModelConfig
  ): IVectorDatabase {
   ...
   * @returns The file path of the created vector index
   */
  async createDocumentIndexByPath(
    indexPath: string,
    documentId: number,
    modelConfig: EmbeddingModelConfig,
    indexType = "Flat"
  ...
  getDocumentIndexPath(
    ...
    // model ID is still kept on RAGDocumentEntity.modelName for routing/display.
    const safeModelKey = toPathSafeModelKey(modelConfig.name);
  ...
  /**
   * Best-effort removal of a document's vector index for a specific model.
   * Used by the local-fallback rebuild path to discard a partial remote index
   * before writing local vectors, so a document never mixes embedding spaces.
   * Failures are logged and swallowed — clearing is best-effort.
   */
  async deleteDocumentIndexByPath(
    documentId: number,
    modelConfig: EmbeddingModelConfig
  ): Promise<void> {
    try {
    ...
    try {
      const indexPath = this.getDocumentIndexPath(documentId, modelConfig);
