...
export class StreamEventProcessor {
  ...
  private handleErrorEvent(extractContent: () => string): void {
    const errorMessage =
      extractContent() || "An error occurred during streaming";
  ...
  private sendPlanAbortNotification(classifiedError: ClassifiedError): void {
    ...
    const chunk: ChatStreamChunk = {
      ...
      eventType: StreamEventType.ERROR,
      conversationId: this.state.streamConversationId,
      errorMessage: `Plan aborted: ${ErrorClassifier.getErrorDescription(
        classifiedError
      )}`,
    };
    this.event.sender.send(AI_CHAT_STREAM_CHUNK, JSON.stringify(chunk));
  ...
  private checkPlanFailureThreshold(): void {
    ...
    const failedSteps = this.state.currentPlan.steps.filter(
      (s) => s.status === PlanStepStatus.FAILED
    ).length;
    const failureThreshold = Math.ceil(
      this.state.currentPlan.steps.length *
        EXECUTION_CONFIG.MAX_FAILED_STEP_PERCENTAGE
    );
  ...
  private handlePlanCreatedEvent(streamEvent: StreamEvent): void {
    try {
      ...
      const threadId =
        (planData as { thread_id?: unknown }).thread_id &&
        typeof (planData as { thread_id?: unknown }).thread_id === "string"
          ? (planData as { thread_id: string }).thread_id
          : (planDataInput as { thread_id?: string; threadId?: string })
              .thread_id || (planDataInput as { threadId?: string }).threadId;
   ...
   */
  private extractPlanDataFromEvent(
    streamEvent: StreamEvent
    ...
    if (
      ...
      !("name" in eventData.data) &&
      !("id" in eventData.data)
    ) {
  ...
  private handlePlanStepStartEvent(streamEvent: StreamEvent): void {
    try {
      ...
      if (this.state.currentPlan) {
        const stepIndex = this.state.currentPlan.steps.findIndex(
          (s) => s.stepId === stepId
        );
        if (stepIndex >= 0) {
          this.state.currentPlan.steps[stepIndex].status =
   ...
   */
  private updatePlanStepState(
    stepInfo: ReturnType<typeof this.extractStepInfo>
  ): void {
    if (!this.state.currentPlan) return;

    const stepIndex = this.state.currentPlan.steps.findIndex(
      (s) => s.stepId === stepInfo.stepId
    );
    if (stepIndex >= 0) {
      this.state.currentPlan.steps[stepIndex].status = stepInfo.success
   ...
   */
  private saveStepCompletionMessage(
    stepInfo: ReturnType<typeof this.extractStepInfo>
  ): void {
  ...
  private sendStepCompleteChunk(
    ...
    const chunk: ChatStreamChunk = {
      ...
      planProgress: this.state.currentPlan
        ? {
            completed: this.state.currentPlan.steps.filter(
              (s) =>
                s.status === PlanStepStatus.COMPLETED ||
                s.status === PlanStepStatus.FAILED
            ).length,
  ...
  private parseStepsFromPlanData(planData: PlanCreatedEventData): PlanStep[] {
    ...
    if (planData.plan && Array.isArray(planData.plan)) {
      return planData.plan
        .filter(
          (stepStr): stepStr is string =>
            typeof stepStr === "string" && stepStr.trim().length > 0
        )
        .map((stepStr, index) => {
          ...
          return {
            ...
            stepNumber: stepNumber,
            title:
              title.length > PLAN_CONFIG.MAX_STEP_TITLE_LENGTH
