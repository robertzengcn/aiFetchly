...
export class WebsiteAnalysisService {
  ...
  static async startBatchAnalysis(
    ...
    const searchResults = await searchResultModule.getSearchResultsByIds(
      resultIds
    );
    ...
    try {
      await searchResultModule.updateAiAnalysisStatusBatch(
        resultIdsToUpdate,
        "analyzing"
      );
    } catch (error) {
  ...
  > {
    ...
    while (Date.now() - startTime < maxWaitTime) {
      ...
      const allComplete = searchResults.every(
        (result) =>
          result.ai_analysis_status === "completed" ||
          result.ai_analysis_status === "failed"
      );
      ...
      if (allComplete) {
        ...
        return searchResults
          .filter(
            (result): result is typeof result & { id: number } =>
  ...
  static async getPageContentAsMarkdown(
    ...
    // F3 fix — fail-closed at the service boundary before reaching the worker.
    // The worker re-validates, but blocking here means we never spawn a
    // Puppeteer navigation for an unsafe destination.
    const urlCheck = await UrlGuard.validateWithDns(trimmed);
    if (!urlCheck.safe) {
      throw new Error(`URL rejected by SSRF guard: ${urlCheck.error}`);
    }

    const markdown = await this.scrapeWebsite(urlCheck.normalizedUrl!);
    const maxLength =
      options?.maxLength ?? ANALYSIS_CONFIG.READ_URL_DEFAULT_MAX_LENGTH;
