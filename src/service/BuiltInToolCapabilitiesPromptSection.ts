...
| Send NEW outbound/marketing emails to contacts/customers (NOT inbox replies). Use SMTP senders from \`list_email_services\`. If the user asked to send without review / send directly, call \`start_email_send_task\` with \`skip_review=true\` (do NOT draft first, do NOT wait for Review). Otherwise \`draft_outbound_email_batch\` then STOP and wait for the user to click Review. Chat confirmation (e.g. "yes, send it") may send the existing draft — do NOT re-draft. Do NOT use \`send_email_reply\` or \`list_email_inboxes\` for this. | \`list_email_services\`, \`list_email_templates\`, \`draft_outbound_email_batch\`, \`start_email_send_task\` | \`send marketing email outbound\` |
...
| Automation schedule / cron / run later / recurring task — \`create_schedule\` needs an existing AI message task: call \`create_ai_message_task\` first (or find one via \`list_ai_message_tasks\`), then pass its \`task_id\` | \`list_ai_message_tasks\`, \`create_ai_message_task\`, \`list_schedules\`, \`get_schedule_details\`, \`list_schedule_executions\`, \`create_schedule\`, \`update_schedule\`, \`delete_schedule\`, \`pause_schedule\`, \`resume_schedule\`, \`run_schedule_now\` | \`schedule cron automation\` |
