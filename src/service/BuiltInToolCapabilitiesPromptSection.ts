...
| Create/overwrite/edit a workspace file (file_write is create+overwrite; file_edit is exact string replace) | \`file_write\`, \`file_edit\` | \`file create write edit\` |
...
| Run a local shell command (needs confirmation) | \`shell_execute\` | \`shell command run\` |
