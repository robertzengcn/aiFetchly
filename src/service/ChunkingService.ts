...
export class ChunkingService {
  ...
  async chunkDocument(
    ...
    if (options) {
      if (options.chunkSize !== undefined)
        chunkingOptions.chunkSize = options.chunkSize;
      if (options.overlapSize !== undefined)
        chunkingOptions.overlapSize = options.overlapSize;
      if (options.strategy !== undefined)
        chunkingOptions.strategy = options.strategy;
      if (options.preserveWhitespace !== undefined)
        chunkingOptions.preserveWhitespace = options.preserveWhitespace;
      if (options.minChunkSize !== undefined)
        chunkingOptions.minChunkSize = options.minChunkSize;
    }
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const existingChunk = await this.ragChunkModule.findChunkByHash(
        contentHash,
        document.id
      );
      ...
      switch (fileExt) {
        ...
        case ".csv": {
          ...
          return {
            ...
            originalFormat: "csv",
            metadata: {
              characterCount: csvMarkdown.length,
              wordCount: csvMarkdown.split(/\s+/).filter((w) => w.length > 0)
                .length,
            },
          };
        }
        ...
        case ".xls": {
          ...
          return {
            ...
            originalFormat: "xlsx",
            metadata: {
              characterCount: xlsxMarkdown.length,
              wordCount: xlsxMarkdown.split(/\s+/).filter((w) => w.length > 0)
                .length,
            },
          };
        }
   ...
   */
  private getOptimalChunkingStrategy(
    contentType: string,
    userStrategy: string
  ): string {
    // If user explicitly specified a strategy, respect it (unless it's incompatible)
    if (
      userStrategy !== "sentence" &&
      userStrategy !== "paragraph" &&
      userStrategy !== "semantic" &&
      userStrategy !== "fixed"
    ) {
      return userStrategy;
    }
  ...
  private async generateChunks(
    ...
    const effectiveStrategy = this.getOptimalChunkingStrategy(
      contentType,
      options.strategy
    );
  ...
  private chunkBySentences(
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      if (
        this.estimateTokenCount(potentialChunk) > options.chunkSize &&
        currentChunk
      ) {
        // Save current chunk
        chunks.push({
   ...
   */
  private chunkByMarkdownStructure(
    content: string,
    options: ChunkingOptions
  ): ChunkResult[] {
    const chunks: ChunkResult[] = [];
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isCodeContinuation =
        isCodeBlock ||
        (currentChunk.includes("```") && !currentChunk.endsWith("```"));
      ...
      if (shouldBreak) {
        ...
        if (isHorizontalRule) {
          ...
          const overlapText = previousChunk
            ? this.getOverlapText(previousChunk, options.overlapSize)
            : "";

          if (overlapText) {
            // Find where overlap starts in original content
            const overlapStartInChunk =
              previousChunk.length - overlapText.length;
  ...
  private shouldBreakMarkdownChunk(
    ...
    isTableRow: boolean,
    isListContinuation: boolean,
    isTableContinuation: boolean,
    isCodeContinuation: boolean
  ): boolean {
    ...
    if (
      isHeading &&
      currentTokenCount > options.chunkSize * 0.7 &&
      potentialTokenCount <= options.chunkSize
    ) {
      return true;
    }
  ...
  private chunkByHtmlStructure(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const isMajorElement =
        /<(h[1-6]|div|section|article|header|footer|main|nav|aside)/i.test(
          line
        );
      const isChunkTooLarge =
        this.estimateTokenCount(potentialChunk) > options.chunkSize;
      const isEndOfElement = tagDepth === 0 && currentChunk.trim().length > 0;

      if (
        (isMajorElement || isChunkTooLarge || isEndOfElement) &&
        currentChunk.trim()
      ) {
        // Save current chunk
        chunks.push({
