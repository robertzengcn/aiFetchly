...
export class PlanValidator {
   ...
   */
  static validatePlanCreatedData(
    data: unknown
  ): ValidationResult<PlanCreatedEventData> {
    const errors: string[] = [];
    ...
    if (planArray && dataObj) {
      ...
      const content =
        ValidationUtils.sanitizeString(planData.content) || "Execution Plan";
      let title = content;
      ...
      return {
        ...
        data: result as PlanCreatedEventData,
        errors,
      };
    }
    ...
    const planIdError = ValidationUtils.validateNonEmptyString(
      planData.plan_id,
      "plan_id"
    );
    if (planIdError) errors.push(planIdError);

    const titleError = ValidationUtils.validateNonEmptyString(
      planData.title,
      "title"
    );
    if (titleError) errors.push(titleError);
   ...
   */
  static validatePlanStepData(
    data: unknown
  ): ValidationResult<PlanStepEventData> {
    const errors: string[] = [];
    ...
    if (explicitStepId) {
      ...
      errors.push(
        "Plan step requires step_id or a valid step_index (0-based integer)"
      );
      return { isValid: false, errors };
    }

    // Optional validations (explicit step_number overrides index-derived default)
    const stepNumberError = ValidationUtils.validatePositiveInteger(
      stepData.step_number,
      "step_number"
    );
    if (stepNumberError) errors.push(stepNumberError);
    else if (stepData.step_number !== undefined)
      result.step_number = stepData.step_number as number;
    ...
    if (descriptionValue !== undefined) {
      if (typeof descriptionValue !== "string") {
        ...
        if (descLengthError) errors.push(descLengthError);
          ...
          result.description = ValidationUtils.sanitizeString(descriptionValue);
      }
    }
    ...
    if (typeof stepData.success === "boolean")
      result.success = stepData.success;
    if (typeof stepData.plan_id === "string")
      result.plan_id = ValidationUtils.sanitizeString(stepData.plan_id);
    if (typeof stepData.reason === "string")
      result.reason = ValidationUtils.sanitizeString(stepData.reason);
   ...
   */
  static validatePlanControlData(
    data: unknown
  ): ValidationResult<PlanControlEventData> {
    const errors: string[] = [];
