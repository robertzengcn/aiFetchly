...
export class BackgroundScheduler extends BaseDb {
  ...
  getSchedulerStatus(): SchedulerStatus {
    ...
    return {
      ...
      activeSchedules:
        this.scheduleManager.getSchedulerStatus().activeSchedules,
  ...
  private async processDependencyQueue(): Promise<void> {
    try {
      ...
      const dependencySchedules =
        await this.scheduleTaskModel.getSchedulesByTriggerType(
   ...
   */
  private async executeScheduleWithRetry(
    ...
    try {
      ...
      try {
        ...
        await this.scheduleTaskModel.incrementExecutionCount(
          queueItem.scheduleId,
          true
        );
        await this.scheduleTaskModel.updateLastRunTime(
          queueItem.scheduleId,
          new Date()
        );
        ...
        if (schedule.trigger_type === TriggerType.CRON) {
          ...
          await this.scheduleTaskModel.updateNextRunTime(
            queueItem.scheduleId,
            nextRunTime
          );
        }
        ...
        await this.scheduleTaskModel.updateLastErrorMessage(
          queueItem.scheduleId,
          errorMessage
        );
  ...
  private async handleJobCompletion(
    ...
    try {
      ...
      const execution = await this.scheduleExecutionLogModel.getExecutionById(
        executionId
      );
      if (!execution) {
        return;
  ...
  private addToExecutionQueue(queueItem: ExecutionQueueItem): void {
    ...
    const existingIndex = this.executionQueue.findIndex(
      (item) => item.scheduleId === queueItem.scheduleId
    );
    if (existingIndex !== -1) {
  ...
  private async loadPendingExecutions(): Promise<void> {
    try {
      const runningExecutions =
        await this.scheduleExecutionLogModel.getRunningExecutions();
  ...
  async getDetailedStats(): Promise<DetailedStats> {
    try {
      ...
      for (const taskType of taskTypes) {
        const schedules = await this.scheduleTaskModel.getSchedulesByTaskType(
          taskType
        );
        const activeSchedules = schedules.filter((s) => s.is_active);

        const stats = await this.taskExecutorService.getTaskExecutionStatistics(
          taskType
        );
      ...
      const recentExecutions =
        await this.scheduleExecutionLogModel.getRecentExecutions(10);
