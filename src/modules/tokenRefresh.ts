...
import { TOKENNAME, REFRESHTOKEN, TOKENEXPIRY, REFRESHTOKENEXPIRY } from "@/config/usersetting";
...
export class TokenRefreshService {
    ...
    private static _autoRefreshTimer: ReturnType<typeof setInterval> | null = null;
    ...
    static startAutoRefresh(): void {
        if (TokenRefreshService._isAutoRefreshRunning) {
            log.info('[TokenRefresh] Auto-refresh is already running, skipping start');
            return;
        }
    ...
    private static async performAutoRefreshCheck(): Promise<void> {
        ...
        if (refreshExpiryStr) {
            ...
            if (!isNaN(refreshExpiry) && now >= refreshExpiry) {
                log.warn('[TokenRefresh] Refresh token has expired, stopping auto-refresh and signing out');
                TokenRefreshService.stopAutoRefresh();
                ...
                try {
                    ...
                    log.error('[TokenRefresh] Error during signout after refresh token expiry:', signoutError);
                }
                return;
            }
        }
        ...
        if (!accessToken || accessToken.trim().length === 0) {
            ...
            if (tokenExpiryStr) {
                ...
                if (!isNaN(tokenExpiry)) {
                    ...
                    log.info(`[TokenRefresh] Access token expires in ${Math.round(timeUntilExpiry / 1000)}s, refreshing now`);
                }
            }
        ...
        try {
            ...
            log.error(`[TokenRefresh] Background refresh failed (${TokenRefreshService._consecutiveFailures}/${TokenRefreshService.MAX_CONSECUTIVE_FAILURES}):`, errorMsg);
            ...
            if (errorMsg.includes('Invalid or expired refresh token') ||
                ...
                log.warn('[TokenRefresh] Refresh token is invalid, stopping auto-refresh');
                TokenRefreshService.stopAutoRefresh();
                return;
            }

            // Stop after too many consecutive failures
            if (TokenRefreshService._consecutiveFailures >= TokenRefreshService.MAX_CONSECUTIVE_FAILURES) {
                log.error('[TokenRefresh] Max consecutive failures reached, stopping auto-refresh');
                TokenRefreshService.stopAutoRefresh();
                ...
                try {
                    ...
                    log.error('[TokenRefresh] Error during signout after max failures:', signoutError);
                }
            }
        }
    }
