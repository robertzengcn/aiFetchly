...
export class Token {
  ...
  /**
   * Permanently remove a stored value. Unlike setValue(key, "") — which
   * leaves an encrypted empty string behind — this deletes the key entirely,
   * so secrets such as the local provider API key do not linger after clear.
   */
  public deleteValue(key: string): void {
    this.store.deleteValue(key);
  }
  //check whether a key has a stored value (without decrypting).
