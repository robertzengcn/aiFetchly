...
export class ProxyModule extends BaseModule implements IProxyApi {
  ...
  async getProxylist(
    ...
    try {
      ...
      const response: ProxylistResp = {
        status: false,
        msg:
          error instanceof Error ? error.message : "Failed to get proxy list",
        data: {
          total: 0,
          records: [],
        },
  ...
  async saveProxy(entity: ProxyEntityType): Promise<SaveProxyResp> {
    try {
      ...
      if (!entity.id) {
        const exists = await this.proxyModel.proxyExists(
          entity.host,
          entity.port
        );
        if (exists) {
   ...
   * Used by the AI import tool for duplicate detection and reload.
   */
  async getProxiesByHostPortPairs(
    pairs: ReadonlyArray<{ host: string; port: string }>
  ): Promise<ProxyEntityType[]> {
    try {
      const entities = await this.proxyModel.findByHostPortPairs(pairs);
      return entities.map((proxy) => ({
        id: proxy.id,
        host: proxy.host,
        port: proxy.port,
        user: proxy.user,
