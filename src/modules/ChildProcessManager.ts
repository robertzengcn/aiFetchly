...
export class ChildProcessManager extends BaseModule {
  ...
  private messageHandlers: Map<
    MessageType,
    ((message: IPCMessage) => Promise<void>)[]
  > = new Map();
  ...
  private setupProcessEventHandlers(
    ...
    childProcess.on("exit", (code, signal) => {
      console.log(
        `Child process ${processId} exited with code ${code} and signal ${signal}`
      );
      // Report abnormal exits to the crash reporter. Best-effort: never
      // let diagnostics failures break the existing exit handling.
      if (code !== 0 || signal) {
        try {
          getCrashReporterFromGlobal()?.recordWorkerExit({
            workerType: "yellow-pages-scraper",
            taskId,
          ...
          /* swallow — diagnostics must never break worker exit */
        }
      }
  ...
  private async handleStatusUpdate(
    ...
    if (processInfo) {
      ...
      console.log(
        `Process ${processId} status: ${message.status} - ${message.message}`
      );
    }
  }
  ...
  private async handleProgressUpdate(
    ...
    console.log(
      `Process ${processId} progress: ${message.progress}% (${message.currentPage}/${message.totalPages})`
    );
  }
  ...
  private async handleTaskCompleted(
    ...
    console.log(
      `Task completed in process ${processId}:`,
    ...
    return new Promise((resolve, reject) => {
      ...
      const checkReady = () => {
        ...
        if (Date.now() - startTime > timeout) {
          reject(
            new Error(`Process ${processId} not ready within ${timeout}ms`)
          );
          return;
        }
  ...
  getStatistics(): {
    ...
    return {
      ...
      activeProcesses: processes.filter(
        (p) => p.status === TaskStatus.InProgress
      ).length,
