...
export class EmailSearchTaskModule extends BaseModule {
  ...
  constructor() {
    ...
    this.emailsearchResultDetaildb = new EmailsearchResultDetailModel(
      this.dbpath
    );
  ...
  public async searchEmail(taskId: number) {
    ...
    const errorLogfile = path.join(
      logpath,
      "emailsearch",
      taskId.toString() + "_" + uuid + ".error.log"
    );
  ...
  public async saveSearchResult(taskId: number, res: EmailResult) {
    ...
    data.phone = res.aiEnrichment?.phone || null;
    data.address = res.aiEnrichment?.address || null;
    data.socialLinks = res.aiEnrichment?.socialLinks?.join(";") || null;
    data.aiEnrichmentStatus = res.aiEnrichment?.status || "none";
    data.aiEnrichmentError = res.aiEnrichment?.error || null;
    data.aiConfidence = res.aiEnrichment?.confidence || 0;
  ...
  public async getTaskResult(
    ...
    for (const value of res) {
      ...
      const emails = await this.emailsearchResultDetaildb.getItemsByResultId(
        value.id
      );
      ...
      const item: EmailResultDisplay = {
        ...
        socialLinks: value.socialLinks
          ? value.socialLinks.split(";")
          : undefined,
        aiEnrichmentStatus:
          (value.aiEnrichmentStatus as AiEnrichmentStatus) || undefined,
        aiConfidence: value.aiConfidence || undefined,
      };
      result.push(item);
    }
  ...
  public async getAllEmails(taskId: number): Promise<EmailItem[]> {
    ...
    for (let i = 0; i < res; i = i + loopNum) {
      const result = await this.emailsearchresultdb.getTaskResult(
        taskId,
        i,
        loopNum
      );
      for (const value of result) {
        if (value.id) {
          const emailsArr =
            await this.emailsearchResultDetaildb.getItemsByResultId(value.id);
  ...
  public async updateTask(
    ...
    // Update proxies
    await this.emailsearchTaskProxydb.deleteEmailSearchTaskProxyByTaskId(
      taskId
    );
    if (data.proxys && data.proxys.length > 0) {
      for (const proxy of data.proxys) {
        if (proxy.id) {
          await this.emailsearchTaskProxydb.createEmailSearchTaskProxy(
            taskId,
            proxy.id
          );
        }
  ...
  public async deleteTask(taskId: number): Promise<void> {
    ...
    for (const result of results) {
      if (result.id) {
        ...
        const details = await this.emailsearchResultDetaildb.getItemsByResultId(
          result.id
        );
