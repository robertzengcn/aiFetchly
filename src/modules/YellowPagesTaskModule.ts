...
export class YellowPagesTaskModule extends BaseModule {
  ...
  /**
   * Ensures DB connection is initialized, then runs the given async operation.
   * Use for all public methods that touch the database.
   */
  ...
  async createTask(taskData: YellowPagesTaskData): Promise<number> {
    return this.withConnection(async () => {
      try {
        ...
        return await this.yellowPagesTaskModel.saveYellowPagesTask(
          modelTaskData
        );
      } catch (error) {
  ...
  async updateTask(
    id: number,
    updates: Partial<YellowPagesTaskEntity>
  ): Promise<void> {
    return this.withConnection(async () => {
      try {
        ...
        if (updates.platform !== undefined)
          modelUpdates.platform = updates.platform;
        ...
        if (updates.location !== undefined)
          modelUpdates.location = updates.location;
        if (updates.max_pages !== undefined)
          modelUpdates.max_pages = updates.max_pages;
        if (updates.concurrency !== undefined)
          modelUpdates.concurrency = updates.concurrency;
        if (updates.status !== undefined)
          modelUpdates.status = updates.status as YellowPagesTaskStatus;
        if (updates.scheduled_at !== undefined)
          modelUpdates.scheduled_at = updates.scheduled_at;
        if (updates.completed_at !== undefined)
          modelUpdates.completed_at = updates.completed_at;
        if (updates.error_log !== undefined)
          modelUpdates.error_log = updates.error_log;
        if (updates.run_log !== undefined)
          modelUpdates.run_log = updates.run_log;
        if (updates.account_id !== undefined)
          modelUpdates.account_id = updates.account_id;
        ...
        if (updates.delay_between_requests !== undefined)
        ...
        const success = await this.yellowPagesTaskModel.updateTask(
          id,
          modelUpdates
        );
        if (!success) {
  ...
  async listTasks(
    ...
    return this.withConnection(async () => {
      try {
        ...
        const tasks = await this.yellowPagesTaskModel.listTasks(
          modelPage,
          size,
          sort
        );
        ...
        const taskSummaries = await Promise.all(
          tasks.map((task) => this.convertEntityToTaskSummary(task))
        );
  ...
  async getTaskStatistics(): Promise<{
    ...
    return this.withConnection(async () => {
      ...
      const running = (await this.getTasksByStatus(TaskStatus.InProgress))
        .length;
      const completed = (await this.getTasksByStatus(TaskStatus.Completed))
        .length;
   ...
   */
  async getRecentTasks(limit = 10): Promise<YellowPagesTaskEntity[]> {
    return this.withConnection(async () => {
      try {
        // Get recent tasks by using the listTasks method with sorting by creation date
        const recentTasks = await this.yellowPagesTaskModel.listTasks(
          1,
          limit,
          {
            key: "createdAt",
   ...
   */
  async cleanupOldTasks(daysOld = 30): Promise<number> {
    return this.withConnection(async () => {
      try {
        ...
        const completedTasks = await this.getTasksByStatus(
          TaskStatus.Completed
        );

        // Filter tasks older than cutoff date
        const oldTasks = completedTasks.filter(
          (task) => task.completed_at && task.completed_at < cutoffDate
        );
  ...
  async handleTasksFromPreviousSession(): Promise<number> {
    return this.withConnection(async () => {
      try {
        ...
        const tasksWithPID = runningTasks.filter(
        ...
        for (const task of tasksWithPID) {
          try {
            ...
            await this.yellowPagesTaskModel.updateTaskErrorLog(
              task.id,
              errorMessage
            );
   ...
   */
  private async convertEntityToTaskSummary(
    entity: YellowPagesTaskEntity
  ): Promise<TaskSummary> {
    // Get the actual results count from the results module
    let resultsCount = 0;
    try {
      resultsCount = await this.yellowPagesResultModule.getResultsCountByTaskId(
        entity.id
      );
    } catch (error) {
  ...
  private applyFilters(
    ...
    if (filters.platform) {
      filteredTasks = filteredTasks.filter(
        (task) => task.platform === filters.platform
      );
    }
    ...
    if (filters.createdAfter) {
      filteredTasks = filteredTasks.filter(
        (task) => task.created_at >= filters.createdAfter!
      );
    }

    if (filters.createdBefore) {
      filteredTasks = filteredTasks.filter(
        (task) => task.created_at <= filters.createdBefore!
      );
    }
