...
export class YellowPagesPlatformModule extends BaseModule {
   ...
   * @returns Throws — creating platforms is not supported in read-only TS config mode
   */
  async createPlatform(): Promise<number> {
    try {
      throw new Error(
   ...
   * @param id The platform ID
   */
  async updatePlatform(id: string): Promise<void> {
    const existingPlatform = await this.getPlatformById(id);
    if (!existingPlatform) {
      throw new Error(`Platform with ID ${id} not found`);
    }
    throw new Error(
      "Updating platforms is not supported in read-only TS config mode. Edit files in src/config/platforms."
    );
  }
  ...
  async togglePlatformStatus(id: string): Promise<boolean> {
    ...
    throw new Error(
      "Toggling platform status is not supported in read-only TS config mode. Edit the TS file to change is_active."
    );
  }
  ...
  validatePlatformConfig(platform: Partial<PlatformConfig>): {
    ...
    if (
      platform.delay_between_requests !== undefined &&
      platform.delay_between_requests < 0
    ) {
      errors.push("Delay between requests cannot be negative");
    }

    if (
      platform.max_concurrent_requests !== undefined &&
      platform.max_concurrent_requests < 1
    ) {
