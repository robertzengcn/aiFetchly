...
export function getEnumValueByNumber(
  enumObj: any,
  value: number
): string | undefined {
  // console.log(value)
...
export function convertCookiesToNetscapeFile(
  ...
  for (const cookie of cookies) {
    ...
    lines.push(
      `${domain}\t${flag}\t${path}\t${secureFlag}\t${expiration}\t${name}\t${value}`
    );
  }
  //}
...
export async function scrollPageToBottom(
  ...
  while (currentScrolls < maxScrolls) {
    ...
    if (currentHeight === lastHeight) {
      ...
      await page.evaluate(
        "window.scrollTo(0, document.documentElement.scrollHeight * 1.5)"
      );
