 ...
 */
export interface YellowPagesOrchestratorDeps {
  yellowPagesModule: YellowPagesModule;
  processManager: YellowPagesProcessManager;
  browserManager: BrowserManager;
  accountCookiesModule: AccountCookiesModule;
  taskModel: YellowPagesTaskModel;
  platformRegistry: PlatformRegistry;
  resultModel: YellowPagesResultModel;
  initModule: YellowPagesInitModule;
  healthCheck: YellowPagesHealthCheck;
}

export class YellowPagesOrchestrator
  extends BaseModule
  implements ITaskManager
{
  /**
   * WS-5 R5.1 — injectable collaborators. Each is optional in the constructor;
   * omitting one yields the real module/service (production unchanged).
   */
  ...
  constructor(deps?: Partial<YellowPagesOrchestratorDeps>) {
    super();

    // Initialize all modules — defaults preserve production behavior; tests
    // inject fakes via `deps`.
    this.yellowPagesModule = deps?.yellowPagesModule ?? new YellowPagesModule();
    this.processManager =
      deps?.processManager ?? YellowPagesProcessManager.getInstance();
    this.browserManager = deps?.browserManager ?? new BrowserManager();
    ...
    this.initModule = deps?.initModule ?? new YellowPagesInitModule();
    this.healthCheck = deps?.healthCheck ?? new YellowPagesHealthCheck();
  }
  ...
  async createTask(taskData: YellowPagesTaskData): Promise<number> {
    ...
    try {
      ...
      if (!platform || !platform.is_active) {
        throw new Error(
          `Platform ${taskData.platform} is not supported or not active`
        );
      }
      ...
      log.info(
        `Orchestrator created task ${taskId} for platform ${taskData.platform}`
      );
      return taskId;
    } catch (error) {
  ...
  async getSystemHealthStatus(): Promise<{
    ...
    try {
      const [moduleHealth, processHealth, browserInfo, systemStatus] =
        await Promise.all([
      ...
      if (!this.isInitialized || !systemStatus.initialized) {
        ...
        moduleHealth.failedTasks > 0 ||
        !processHealth.processIsolation
      ) {
        overall = "warning";
      }
  ...
  private async performPreflightChecks(taskId: number): Promise<void> {
    ...
    if (taskStatus !== TaskStatus.Pending && taskStatus !== TaskStatus.Paused) {
      throw new Error(
        `Task ${taskId} is not in a startable state (current: ${TaskStatus[taskStatus]})`
      );
    }
