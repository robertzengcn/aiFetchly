...
export class EmailMarketingSendLogModule extends BaseModule {
  ...
  async updateTask(
    id: number,
    updates: EmailMarketingSendLogEntity
  ): Promise<void> {
  ...
  async getSendlogList(
    taskId: number,
    page: number,
    limit: number,
    where?: string,
    sortby?: SortBy
  ): Promise<{ records: EmailMarketingSendLogEntity[]; total: number }> {
    const records =
      await this.emailMarketingSendLogdb.listEmailMarketingSendLog(
        taskId,
        page,
        limit,
        where,
    ...
    const total = await this.emailMarketingSendLogdb.countEmailMarketingSendLog(
      taskId,
      where
    );
  ...
  // authorized outbound sends.
  async getRecentSendlogList(
    page: number,
    limit: number,
    where?: string,
    sortby?: SortBy
    ...
    const records =
      await this.emailMarketingSendLogdb.listAllRecentEmailMarketingSendLog(
        page,
        limit,
        where,
        sortby
      );
    const total = await this.emailMarketingSendLogdb.countAllRecent(where);
    return { records, total };
  }
  ...
  async aggregateByDateRange(
    ...
    const rows = await this.emailMarketingSendLogdb.aggregateByDateRange(
      startDate,
      endDate,
      granularity
    );
  ...
  async countStatusByDateRange(
    startDate: Date,
    endDate: Date
  ): Promise<Array<{ status: number; count: number }>> {
    return this.emailMarketingSendLogdb.countByStatusWithinDateRange(
      startDate,
      endDate
    );
  }
