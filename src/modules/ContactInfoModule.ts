...
export class ContactInfoModule extends BaseModule {
     ...
     */
    async getContactInfoByResultIds(resultIds: number[]): Promise<ContactExtractionResult[]> {
        await this.ensureConnection();

        const contactInfoList = await contactInfoRepository.findByResultIds(resultIds);
     ...
     * @param data The contact info data
     */
