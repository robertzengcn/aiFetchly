...
export class TaskExecutorService {
    ...
    try {
      ...
      switch (schedule.task_type) {
        ...
        case TaskType.EMAIL_EXTRACT:
          taskOutputId = await this.executeEmailExtractionTask(
            schedule.task_id
          );
          break;
        ...
        case TaskType.GOOGLE_MAPS:
          taskOutputId = await this.executeGoogleMapsTask(schedule.task_id);
          break;
  ...
  /**
   * Execute a Google Maps task by re-running a saved search record.
   * @param taskId The Google Maps search record ID
   * @returns The task output ID
   */
  async executeGoogleMapsTask(taskId: number): Promise<number> {
    try {
      ...
      if (!record) {
        throw new Error(`Google Maps search record ${taskId} not found`);
      }
      ...
      await this.googleMapsModule.executeSearch(
        input,
        undefined,
        undefined,
  ...
  /**
   * Execute a Yandex Maps task by re-running a saved search record.
   * @param taskId The Yandex Maps search record ID
   * @returns The task output ID
   */
  async executeYandexMapsTask(taskId: number): Promise<number> {
    try {
      ...
      if (!record) {
        throw new Error(`Yandex Maps search record ${taskId} not found`);
      }
      ...
      await this.yandexMapsModule.executeSearch(input, {
    ...
    try {
      ...
      switch (taskType) {
        case TaskType.SEARCH:
          ...
          break;
        case TaskType.EMAIL_EXTRACT:
          const emailTask = await this.emailSeachTaskModule.getTaskDetail(
            taskId
          );
        ...
        case TaskType.BUCK_EMAIL:
          ...
          break;
        case TaskType.YELLOW_PAGES:
          const yellowPagesStatus = await this.yellowPagesModule.getTaskStatus(
            taskId
          );
          ...
          break;
        case TaskType.GOOGLE_MAPS: {
          const gmRecord = await this.googleMapsModule.getSearchRecord(taskId);
          if (!gmRecord) {
            status = TaskStatus.Notstart;
          } else {
            switch (gmRecord.status) {
              case "completed":
                status = TaskStatus.Complete;
                break;
              case "failed":
  ...
  async cancelTask(taskId: number, taskType: TaskType): Promise<void> {
    try {
      ...
      switch (taskType) {
        case TaskType.SEARCH:
          await this.searchTaskModel.updateTaskStatus(
            taskId,
            "cancelled" as any
          );
          break;
        case TaskType.EMAIL_EXTRACT:
          ...
          if (emailTask) {
            ...
            await this.emailSeachTaskModule.updateTaskStatus(
              taskId,
              emailTask.status
            );
          }
        ...
        case TaskType.BUCK_EMAIL:
          ...
          if (buckEmailTask) {
            ...
            await this.buckEmailTaskModel.updateTaskStatus(
              taskId,
              buckEmailTask.status
            );
          }
          break;
        case TaskType.YELLOW_PAGES:
          ...
          break;
        case TaskType.GOOGLE_MAPS:
          await this.googleMapsModule.cancelSearch(
            `schedule-google-maps-${taskId}`
          );
          break;
        case TaskType.YANDEX_MAPS:
  ...
  async executeDependencyChain(
    ...
    try {
      ...
      const taskOutputId = await this.executeScheduledTask(
        schedule as any,
        parentExecutionId
      );
