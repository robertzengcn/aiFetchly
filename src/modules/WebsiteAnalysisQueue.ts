...
export class WebsiteAnalysisQueue {
  ...
  private maxConcurrency = 3; // Process up to 3 websites concurrently
  ...
  private batchContexts: Map<string, ModuleExecutionContext | undefined> =
    new Map(); // batchId -> optional context
   ...
   *        partial-snapshot registration (AI tool-progress pipeline).
  ...
  public async addBatch(
    ...
    this.batchContexts.set(batchId, context);

    // Emit an initial "running" progress event if context is present.
    if (context) {
      context.emitProgress?.({
        phase: "running",
        message: "Analyzing website content...",
        partialCount: 0,
        expectedCount: jobIds.size,
      });
    }
  ...
  private async processJob(job: AnalysisJob, batchId: string): Promise<void> {
    ...
    try {
      ...
      const searchResultModule = new SearchResultModule();
      await searchResultModule.updateAiAnalysisStatus(
        job.resultId,
        "analyzing"
      );
  ...
  private async scrapeWebsite(
    ...
    return new Promise((resolve, reject) => {
      const childProcess = spawnOwned(
        "website-analysis",
        () =>
          utilityProcess.fork(this.childProcessPath!, [], {
        stdio: "pipe",
  ...
  private notifyProgress(batchId: string, currentUrl?: string): void {
    ...
    const completed = Array.from(batchJobIds).filter((id) =>
      this.results.has(id)
    ).length;
    ...
    if (ctx) {
      const phase = completed >= total ? "finalizing" : "running";
      ctx.emitProgress?.({
        phase,
        message: currentUrl
          ? `Analyzing website content... (${currentUrl})`
          : `Completed ${completed} of ${total} analyses`,
        partialCount: completed,
        expectedCount: total,
      });
      if (completed > 0) {
        // Collect partial results for the snapshot.
        const partialResults: AnalysisResult[] = Array.from(batchJobIds)
          .map((id) => this.results.get(id))
          .filter((r): r is AnalysisResult => r !== undefined);
        ToolExecutor.updatePartialSnapshot(ctx.toolCallId, {
          collectedCount: completed,
          expectedCount: total,
          data: { results: partialResults },
        });
      }
    }
  }
  ...
  private checkBatchComplete(batchId: string): void {
    ...
    const allComplete = Array.from(batchJobIds).every((id) =>
      this.results.has(id)
    );
