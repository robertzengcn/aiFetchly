...
import {
  ...
  BusinessHours,
  Rating,
} from "@/modules/interface/IDataExtractor";
...
export class PagesJaunesAdapter extends BasePlatformAdapter {
   ...
   */
  async extractPhoneNumberWithReveal(
    page: Page,
    businessElement: any
  ): Promise<string | undefined> {
    try {
      log.info(
        "🔍 Attempting to extract phone number with reveal method for PagesJaunes.fr"
      );
          ...
          if (phoneRevealButtons.length > 0) {
            ...
            for (const button of phoneRevealButtons) {
              ...
              if (isClickable) {
                ...
                log.info(
                  `📞 Found clickable phone reveal button with selector: ${selector} (${phoneRevealButtons.length} total found)`
                );
                break;
              }
            }
      ...
      if (!phoneRevealButton) {
        log.info(
          "📞 No clickable phone reveal button found, trying direct phone extraction"
        );
        ...
        try {
          ...
          if (phoneElement) {
            ...
            if (phoneText && this.isValidPhoneNumber(phoneText)) {
              ...
              log.info(
                `📞 Successfully extracted revealed phone: ${phoneNumber} using selector: ${selector}`
              );
              break;
            }
          }
      ...
      if (!phoneNumber) {
        log.info(
          "📞 Phone not found in business element, searching page-wide..."
        );
        for (const selector of revealedPhoneSelectors) {
          try {
            ...
            if (phoneElement) {
              ...
              if (phoneText && this.isValidPhoneNumber(phoneText)) {
                phoneNumber = phoneText;
                log.info(
                  `📞 Successfully extracted revealed phone (page-wide): ${phoneNumber} using selector: ${selector}`
                );
                break;
              }
            }
          ...
          try {
            ...
            for (const phoneElement of phoneElements) {
              ...
              if (phoneText && this.isValidPhoneNumber(phoneText)) {
                phoneNumber = phoneText;
                log.info(
                  `📞 Successfully extracted revealed phone (retry): ${phoneNumber}`
                );
                break;
              }
            }
      ...
      if (!phoneNumber) {
        log.info(
          "📞 Failed to extract revealed phone number, falling back to direct extraction"
        );
        return await this.extractDirectPhoneNumber(businessElement);
      }
   ...
   */
  private async extractDirectPhoneNumber(
    businessElement: any
  ): Promise<string | undefined> {
    try {
  ...
  private async extractText(
    ...
    try {
      ...
      if (targetElement) {
        const text = await targetElement.evaluate(
   ...
   */
  async extractWebsiteWithReveal(
    page: Page,
    businessElement: any
  ): Promise<string | undefined> {
    try {
      log.info(
        "🌐 Attempting to extract website URL with reveal method for PagesJaunes.fr"
      );
      ...
      for (const selector of websiteRevealSelectors) {
        try {
          ...
          if (websiteRevealElement) {
            usedSelector = selector;
            log.info(
              `🌐 Found website reveal element with selector: ${selector}`
            );
            break;
          }
        } catch (error) {
      ...
      if (!websiteRevealElement) {
        log.info(
          "🌐 No website reveal element found, trying direct website extraction"
        );
   ...
   */
  private async extractDirectWebsite(
    businessElement: Page
  ): Promise<string | undefined> {
    try {
      ...
      for (const selector of valueSpanSelectors) {
        try {
          ...
          if (valueElement) {
            ...
            if (websiteUrl) {
              log.info(
                `🌐 Extracted website URL from span.value: ${websiteUrl}`
              );
              return websiteUrl;
            }
          }
      ...
      for (const selector of directWebsiteSelectors) {
        try {
          ...
          if (websiteElement) {
            ...
            if (websiteUrl && this.isValidWebsiteUrl(websiteUrl)) {
              log.info(
                `🌐 Extracted direct website URL from href: ${websiteUrl}`
              );
              return websiteUrl;
            }
          }
