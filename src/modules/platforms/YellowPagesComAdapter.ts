...
export class YellowPagesComAdapter extends BasePlatformAdapter {
    ...
    const businesses = await page.evaluate(() => {
      ...
      businessElements.forEach((element) => {
        ...
        business.reviewCount =
          reviewElement?.textContent?.trim().replace(/[()]/g, "") || null;

        // Extract years in business
        const yearsElement = element.querySelector(
          "div.badges div.years-with-yp > div.count strong"
        );
      ...
      const details = await page.evaluate(() => {
        ...
        hoursElements.forEach((row) => {
          ...
          if (dayElement && timeElement) {
            businessHours[dayElement.textContent?.trim() || ""] =
              timeElement.textContent?.trim() || "";
          }
        });
        result.businessHours =
          Object.keys(businessHours).length > 0 ? businessHours : null;
  ...
  private parseAddress(addressText: string): Address | null {
    ...
    const city =
      parts.slice(1, -1).join(", ").trim() || cityStateParts.join(" ");
    ...
    try {
      return await page.evaluate(() => {
        ...
        return currentPageElement
          ? parseInt(currentPageElement.textContent || "1")
          : 1;
      });
    } catch {
      return 1;
  ...
  async applyCookies(page: Page, cookies: any): Promise<void> {
    ...
    try {
      ...
      const yellowPagesCookies = cookies.filter(
        (cookie) =>
          cookie.domain &&
          isSameRegistrableHost(cookie.domain, ALLOWED_HOSTS.yellowpages)
      );
  ...
  async handleSiteSpecificFeatures(page: Page): Promise<void> {
    try {
      ...
      const cookieBanner = await page.$(
        ".cookie-banner, .gdpr-banner, #cookieConsentBanner"
      );
      if (cookieBanner) {
        const acceptButton = await page.$(
          ".cookie-banner .accept, .gdpr-banner .accept, #cookieConsentBanner .accept"
        );
        if (acceptButton) {
