...
export class YellowPagesProcessManager extends BaseModule {
  ...
  private getModulePathFromClass(adapterClass: any): string {
    try {
      ...
      log.warn("Could not determine module path for adapter class:", error);
      return `@/modules/platforms/${adapterClass.name}`;
    }
  }
    ...
    try {
      ...
      const platformInfo: PlatformInfo = {
        ...
        selectors: {
          ...
          hours:
            platform.selectors?.hours &&
            typeof platform.selectors.hours === "object" &&
            "container" in platform.selectors.hours
              ? platform.selectors.hours
              : undefined,
          ...
          reviews:
            platform.selectors?.reviews &&
            typeof platform.selectors.reviews === "object" &&
            "container" in platform.selectors.reviews
              ? platform.selectors.reviews
              : undefined,
          ...
          searchForm:
            platform.selectors?.searchForm &&
            typeof platform.selectors.searchForm === "object" &&
            "keywordInput" in platform.selectors.searchForm
              ? platform.selectors.searchForm
              : undefined,
          pagination:
            platform.selectors?.pagination &&
            typeof platform.selectors.pagination === "object" &&
            "nextButton" in platform.selectors.pagination
              ? platform.selectors.pagination
              : undefined,
          navigation:
            platform.selectors?.navigation &&
            typeof platform.selectors.navigation === "object" &&
            "detailLink" in platform.selectors.navigation
              ? platform.selectors.navigation
              : undefined,
        },
      ...
      const errorLogfile = path.join(
        logpath,
        "yellowpages_" + taskId + "_" + uuid + ".error.log"
      );
      const runLogfile = path.join(
        logpath,
        "yellowpages_" + taskId + "_" + uuid + ".runtime.log"
      );

      // Update task with log file paths
      await this.taskModel.updateTaskRunLog(taskId, runLogfile);
      await this.taskModel.updateTaskErrorLog(
        taskId,
    ...
    childProcess.on("message", (raw: unknown) => {
      ...
      switch (message.type) {
        ...
        case "SCRAPING_PAGE_COMPLETE":
          if (
            message.type === "SCRAPING_PAGE_COMPLETE" &&
            message.page &&
            message.totalPages
          ) {
            log.info(
          ...
          if (message.type === "SCRAPING_CLOUDFLARE_DETECTED") {
            ...
            if (processInfo?.logFiles) {
              const cloudflareMessage = `[${new Date().toISOString()}] CLOUDFLARE DETECTED: ${contentMessage}. URL: ${
                message.details?.url || "unknown"
              }, Timestamp: ${message.details?.timestamp || "unknown"}, Info: ${
                message.details?.additionalInfo || "No additional info"
              }`;
        ...
        case "TASK_RESUMED":
          {
            ...
            sendSystemMessage({
              ...
              data: {
                ...
                content:
                  message.content ||
                  `Yellow Pages task ${taskId} has been resumed successfully.`,
              },
            });
          }
          break;
        ...
        case "AI_SUPPORT_REQUEST":
          if (isAiSupportRequestMessage(message)) {
            this.handleAiSupportRequest(
              message as AiSupportRequestMessage,
              childProcess
            );
          }
          break;
  ...
  private handleProgressMessage(
    ...
    if (processInfo) {
      ...
      if (processInfo.logFiles) {
        const progressMessage = `[${new Date().toISOString()}] Progress: ${JSON.stringify(
          progress
        )}`;
  ...
  private async handleCompletionMessage(
    ...
    if (processInfo) {
      ...
      if (processInfo.logFiles) {
        const completionMessage = `[${new Date().toISOString()}] Task completed successfully with ${
          results.length
        } results`;
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        if (processInfo?.logFiles) {
          const resultsMessage = `[${new Date().toISOString()}] Saved ${
            saveResult.createdIds.length
          } results to database (${
      ...
      await this.taskModel.updateTaskStatus(
        taskId,
        YellowPagesTaskStatus.Completed
      );
  ...
  private handleProcessExit(
    ...
    if (processInfo) {
      ...
      if (processInfo.logFiles) {
        const finalStatusMessage = `[${new Date().toISOString()}] Process final status: ${
          processInfo.status
      ...
      try {
        ...
        sendSystemMessage({
          ...
          data: {
            ...
            content: `Failed to terminate process for Yellow Pages task ${taskId}: ${
      ...
      if (!processInfo) {
        ...
        try {
          ...
          if (task.status === YellowPagesTaskStatus.Completed) {
            throw new Error(
              `Cannot pause task ${taskId} - task is already completed`
            );
          } else if (task.status === YellowPagesTaskStatus.Failed) {
            throw new Error(
              `Cannot pause task ${taskId} - task has failed and cannot be paused`
            );
          } else {
            throw new Error(
              `No active process found for task ${taskId} (status: ${task.status}). The task may have been terminated or crashed.`
            );
          }
        } catch (statusError) {
          throw new Error(
            `No active process found for task ${taskId}: ${
    ...
    try {
      ...
      if (!processInfo) {
        ...
        try {
          ...
          if (
            task.status === YellowPagesTaskStatus.Paused ||
            ...
            throw new Error(
              `Task ${taskId} is in state '${task.status}' which cannot be resumed`
            );
          }
        } catch (restartError) {
  ...
  async checkForOrphanedProcesses(): Promise<{
    ...
    try {
      ...
      const tasksWithValidPID = runningTasks.filter(
        (task) => task.pid !== undefined && task.pid !== null && task.pid > 0
      );
      const tasksWithZeroPID = runningTasks.filter((task) => task.pid === 0);
      const tasksWithInvalidPID = runningTasks.filter(
      ...
      const result = {
        totalChecked:
          tasksWithValidPID.length +
          tasksWithZeroPID.length +
          tasksWithInvalidPID.length,
        orphanedFound,
        failedUpdates,
      };
