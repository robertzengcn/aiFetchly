...
import {
  SearchTaskModel,
  SearchTaskStatus,
  SearchTaskUpdateFields,
} from "@/model/SearchTask.model";
...
import type {
  TaskDetailsForEdit,
  SearchTaskUpdateData,
  SearchTaskStatusValue,
} from "@/entityTypes/searchControlType";
...
export class SearchModule extends BaseModule {
  ...
  public async searchByKeywordAndEngine(
    ...
    if (!selectedAccounts || selectedAccounts.length === 0) {
      ...
      if (platformId) {
        try {
          const accounts =
            await this.socialAccountModule.getSocialAccountsByPlatform(
              platformId
            );
    ...
    const errorLogfile = path.join(
      logpath,
      "search_" + taskId.toString() + "_" + uuid + ".error.log"
    );
    const runLogfile = path.join(
      logpath,
      "search_" + taskId.toString() + "_" + uuid + ".runtime.log"
    );
  ...
  public async runSearchTask(taskId: number): Promise<void> {
    ...
    const twoCaptchaToken =
      await this.systemSettingGroupModule.getGroupItembyName(twocaptchagroup);
    if (twoCaptchaToken) {
      //find 2captcha enable key
      const twocaptchenable = twoCaptchaToken.settings.find(
        (item) => item.key === twocaptcha_enabled
      );
      if (twocaptchenable) {
        const token = twoCaptchaToken.settings.find(
          (item) => item.key === twocaptchatoken
        );
        if (token) {
    ...
    if (data.localBrowser && data.localBrowser.length > 0) {
      const external_system_group =
        await this.systemSettingGroupModule.getGroupItembyName(external_system);
      if (external_system_group) {
        const chromePath = external_system_group.settings.find(
          (item) => item.key === chrome_path
        );
        if (chromePath) {
          localBrowserexcutepath = chromePath.value;
        }
        const firefoxPath = external_system_group.settings.find(
          (item) => item.key === firefox_path
        );
        if (firefoxPath) {
    ...
    child.on("spawn", async () => {
      ...
      if (child.pid) {
        ...
        const searchController = (
          await import("@/controller/SearchController")
        ).SearchController.getInstance();
        searchController.registerProcess(taskId, child);
      }
      child.postMessage(
        JSON.stringify({ action: "searchscraper", data: data }),
        [port1]
      );
    ...
    child.on("message", (message: unknown) => {
      try {
        // Handle AI_SUPPORT_REQUEST messages before standard parsing
        if (message && typeof message === "object" && message !== null) {
          ...
          if (msg.type === "AI_SUPPORT_REQUEST") {
            ...
            aiSupportHandler
              .handleAiSupportRequest(
                msg as unknown as AiSupportRequestMessage,
                child
              )
              .catch((err) => {
          ...
          try {
            ...
            if (
              ...
              parsed.type === "AI_SUPPORT_REQUEST"
            ) {
              const aiSupportHandler = new YellowPagesAiSupportHandler();
              aiSupportHandler
                .handleAiSupportRequest(
                  parsed as AiSupportRequestMessage,
                  child
                )
                .catch((err) => {
                  console.error("AI support request failed:", err);
            ...
            // Not JSON, continue to standard parsing
          }
        }
        ...
        const childdata = parseResult.data as {
          action?: string;
          data?: unknown;
        };
        ...
        if (childdata.action == "savesearchresult") {
          ...
          const cookiesData = childdata.data as {
            accountId: number;
  ...
  public async saveSearchtask(data: SearchDataParam): Promise<number> {
    ...
    const taskId = await this.taskdbModel.saveSearchTask(
      enginId,
      data.num_pages,
      data.concurrency,
      data.notShowBrowser,
  ...
  public async saveSearchtaskOnly(data: SearchDataParam): Promise<number> {
    ...
    const taskId = await this.taskdbModel.saveSearchTaskOnly(
      enginId,
      data.num_pages,
      data.concurrency,
      data.notShowBrowser,
  ...
  public async saveSearchResult(
    ...
    for (const item of data) {
      ...
      if (!keywordId) {
        ...
        keywordId = await this.serKeywordModel.getKeywordId(
          item.keyword,
          taskId
        );
      }
        ...
        for (const sitem of item.results) {
          ...
          if (sitem.link && sitem.link.length > 0) {
            if (!linkearr.includes(sitem.link)) {
              ...
              const res = await this.serResultModel.saveResult(
                reEntity,
                taskId
              );
  ...
  public async listSearchResult(
    ...
    if (search && search.trim().length > 0) {
      ...
      const allResults = await this.serResultModel.listSearchresult(
        keyarr,
        0,
        10000
      );
  ...
  public async getKeywrodsbyTask(taskId: number): Promise<Array<number>> {
    const keywordEnArr = await this.serKeywordModel.getKeywordsEntityByTask(
      taskId
    );
  ...
  public async getTaskEntityById(
    taskId: number
  ): Promise<SearchDataParam | null> {
   ...
   */
  public async updateSearchTask(
    taskId: number,
    updates: SearchTaskUpdateData
  ): Promise<boolean> {
    ...
    const taskUpdateSuccess = await this.taskdbModel.updateSearchTask(
      taskId,
      taskUpdates
    );
    ...
    if (updates.keywords !== undefined) {
      ...
      const existingKeywords = await this.serKeywordModel.getKeywordsByTask(
        taskId
      );
      ...
      const keywordsToRemove = existingKeywords.filter(
   ...
   */
  public async getTaskDetailsForEdit(
    taskId: number
  ): Promise<TaskDetailsForEdit> {
    const taskEntity = await this.taskdbModel.getTaskEntity(taskId);
    if (!taskEntity) {
      throw new Error("Task not found");
    }
   ...
   */
  public async updateAccountCookies(
    accountId: number,
    cookies: Array<CookiesType>
  ): Promise<void> {
    try {
