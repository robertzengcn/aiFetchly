...
const BUILT_IN_SKILLS: SkillDefinition[] = [
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        num_results: { type: 'number', description: 'Number of search results to return (default: 10)', default: 10 },
        show_browser: { type: 'boolean', description: 'Whether to show the browser window during scraping (default: false, headless)', default: false },
      },
      required: ['query'],
    },
    tier: 'main',
    ...
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('scrape_urls_from_google', args, context.conversationId);
  ...
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'The search query to send to Bing' },
        num_results: { type: 'number', description: 'Number of search results to return (default: 10)', default: 10 },
        market: { type: 'string', description: 'Market/region code (e.g., en-US, en-GB)', default: 'en-US' },
        show_browser: { type: 'boolean', description: 'Whether to show the browser window during scraping (default: false, headless)', default: false },
      },
      required: ['query'],
    },
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('scrape_urls_from_bing', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'The search query to send to Yandex' },
        num_results: { type: 'number', description: 'Number of search results to return (default: 10)', default: 10 },
        show_browser: { type: 'boolean', description: 'Whether to show the browser window during scraping (default: false, headless)', default: false },
      },
      required: ['query'],
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('scrape_urls_from_yandex', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'The search query to send to Baidu' },
        num_results: { type: 'number', description: 'Number of search results to return (default: 10)', default: 10 },
        show_browser: { type: 'boolean', description: 'Whether to show the browser window during scraping (default: false, headless)', default: false },
      },
      required: ['query'],
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('scrape_urls_from_baidu', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        urls: { type: 'array', items: { type: 'string' }, description: 'List of URLs to extract emails from' },
        content: { type: 'string', description: 'Raw HTML or text content to extract emails from (alternative to URLs)' },
        validate: { type: 'boolean', description: 'Whether to validate extracted email addresses (default: true)', default: true },
      },
      required: [],
    },
    tier: 'main',
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('extract_emails_from_urls', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        platform: { type: 'string', description: 'Platform name (e.g., "yellowpages.com"). Use get_available_yellow_pages_platforms to see available options.' },
        search_term: { type: 'string', description: 'Business name, category, or keyword to search for' },
        location: { type: 'string', description: 'City, state, or ZIP code to search in' },
        num_results: { type: 'number', description: 'Number of business results to return (default: 20)', default: 20 },
        include_emails: { type: 'boolean', description: 'Whether to attempt extracting email addresses from business pages (default: false)', default: false },
      },
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('search_yellow_pages', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        country_code: { type: 'string', description: 'Optional ISO 3166-1 alpha-2 country code to filter available platforms (e.g., "US", "DE", "FR").' },
      },
    },
    tier: 'main',
    requiresConfirmation: false,
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('get_available_yellow_pages_platforms', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        url: { type: 'string', description: 'The website URL to analyze (must be a valid HTTP/HTTPS URL).' },
        client_business: { type: 'string', description: 'Description of the client or target business to match against. The AI uses this to determine how relevant the website is to the business (match score and reasoning).' },
        temperature: { type: 'number', description: 'Temperature for AI analysis (0.0-1.0). Higher values make the analysis more creative. Default is 0.7.', default: 0.7, minimum: 0.0, maximum: 1.0 },
      },
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('analyze_website', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        result_ids: { type: 'array', items: { type: 'number' }, description: 'Array of search result IDs to analyze. Each ID should correspond to a search result that has a URL.' },
        client_business: { type: 'string', description: 'Description of the client business to match against. Used to determine how well each website matches the business (industry, match score, reasoning).' },
        temperature: { type: 'number', description: 'Temperature for AI analysis (0.0-1.0). Higher values make the analysis more creative. Default is 0.7.', default: 0.7, minimum: 0.0, maximum: 1.0 },
      },
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('analyze_website_batch', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        urls: { type: 'array', items: { type: 'string' }, description: 'Array of website URLs to analyze. Each URL should be a valid HTTP/HTTPS URL.' },
        client_business: { type: 'string', description: 'Description of the client business to match against. Used to determine how well each website matches the business (industry, match score, reasoning).' },
        temperature: { type: 'number', description: 'Temperature for AI analysis (0.0-1.0). Higher values make the analysis more creative. Default is 0.7.', default: 0.7, minimum: 0.0, maximum: 1.0 },
      },
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('analyze_websites', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        url: { type: 'string', description: 'The page URL to read (must be a valid HTTP/HTTPS URL).' },
        max_length: { type: 'number', description: 'Optional maximum characters of markdown to return. Use when only an overview is needed to avoid token limits.', default: 80000 },
      },
      required: ['url'],
    },
    tier: 'main',
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('read_url_content', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        seed_keywords: { type: 'array', items: { type: 'string' }, description: 'One or more seed keywords or topics to expand (e.g., ["cloud storage", "file sharing"])' },
        num_keywords: { type: 'number', description: 'Maximum number of keywords to generate per seed (default: 15)', default: 15 },
        keyword_type: { type: 'string', description: 'Type of keywords to generate: "seo" for search optimization, or other supported types (default: "seo")', default: 'seo' },
      },
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('generate_keywords', args, context.conversationId);
      return { success: true, result };
    },
  },
  {
    ...
    parameters: {
      type: 'object',
      properties: {
        urls: { type: 'array', items: { type: 'string' }, description: 'List of website URLs to extract contact information from (e.g., ["https://example.com", "https://company.com/contact"])' },
      },
      required: ['urls'],
    },
    tier: 'main',
    ...
    source: 'built-in',
    execute: async (args, context) => {
      const result = await ToolExecutor.execute('extract_contact_info', args, context.conversationId);
      return { success: true, result };
    },
  },
];
