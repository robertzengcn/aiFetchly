...
export class VectorModel extends BaseDb {
  ...
  async addVectorToVirtualTable(
    ...
    if (vectors.length !== dimension) {
      throw new Error(
        `Vector array length ${vectors.length} does not match expected dimension ${dimension}`
      );
    }

    if (!this.sqliteDb.connection.isInitialized) {
      throw new Error(
        "DataSource must be initialized before adding to virtual table"
      );
    }
    ...
    try {
      ...
      if (!/^[a-zA-Z0-9_-]+$/.test(virtualTableName)) {
        throw new Error(
          `Invalid virtual table name format: '${virtualTableName}'. Only alphanumeric characters, underscores, and hyphens are allowed.`
        );
      }
      ...
      if (!exists) {
        ...
        const metadata = await this.metadataModel.findByVirtualTableName(
          virtualTableName
        );
        ...
        if (metadata) {
          ...
          try {
            ...
            log.info(
              `Created virtual table '${virtualTableName}' directly (dimension: ${dimension})`
            );
          } catch (createError) {
            const errorMessage =
              createError instanceof Error
                ...
                : String(createError);
            throw new Error(
              `Virtual table '${virtualTableName}' does not exist and failed to create it: ${errorMessage}`
            );
          }
        }
      }
      ...
      log.info(
        `Inserted vector into virtual table '${virtualTableName}' (chunk_id: ${chunkId})`
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      log.error(
        `Failed to add vector to virtual table '${virtualTableName}':`,
        errorMessage
      );
   ...
   */
  async deleteVectorsFromVirtualTable(
    chunkIds: number[],
    virtualTableName: string
  ): Promise<void> {
    if (chunkIds.length === 0) {
    ...
    if (!this.sqliteDb.connection.isInitialized) {
      throw new Error(
        "DataSource must be initialized before deleting from virtual table"
      );
    }
    ...
    try {
      ...
      log.info(
        `Deleted ${chunkIds.length} vectors from virtual table '${virtualTableName}'`
      );
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      log.error(
        `Failed to delete from virtual table '${virtualTableName}':`,
        errorMessage
      );
  ...
  async getTotalCountFromVecIndex(vecIndexName: string): Promise<number> {
    try {
      ...
      if (!/^[a-zA-Z0-9_-]+$/.test(vecIndexName)) {
        log.error(
          `Invalid vec index name format: '${vecIndexName}'. Only alphanumeric characters, underscores, and hyphens are allowed.`
        );
        return 0;
      }
      ...
      log.error(
        `Failed to get total count from vec index '${vecIndexName}':`,
        error
      );
      return 0;
    }
  ...
  async searchSimilarVectors(
    ...
    try {
      ...
      if (adjustedK !== k) {
        log.info(
          `Adjusted k from ${k} to ${adjustedK} (total vectors: ${totalVectors})`
        );
      }
      ...
      if (dimension && queryVector.length !== dimension) {
        throw new Error(
          `Query vector dimension ${queryVector.length} does not match expected dimension ${dimension}`
        );
      }
      ...
      throw new Error(
        `Failed to perform vector search. Please ensure sqlite-vec extension is properly installed and configured. Error: ${
  ...
  async searchSimilarVectorsWithVec0(
    ...
    try {
      ...
      if (adjustedK !== k) {
        log.info(
          `Adjusted k from ${k} to ${adjustedK} (total vectors in '${vecIndexName}': ${totalVectors})`
        );
      }
      ...
      if (vecTableExists) {
        try {
          // Apply the distance threshold on the computed L2 distance, mirroring
          // the filtered query shape used by searchSimilarVectors.
          const hasDistanceFilter = distance !== undefined && distance !== null;
          const vec0Query = hasDistanceFilter
            ? `
            ...
            : `
                        ...
                        ORDER BY
                            distance ASC LIMIT ?
                        `;
          const vec0Params = hasDistanceFilter
            ? [queryVectorBuffer, distance, adjustedK]
            : [queryVectorBuffer, adjustedK];
          const results = (await this.sqliteDb.connection.query(
            vec0Query,
          ...
          log.warn(
            "vec0 virtual table MATCH search failed, trying vec_distance_l2...",
            error
          );
        }
      }
  ...
  async ensureVirtualTable(
    ...
    if (!this.sqliteDb.connection.isInitialized) {
      throw new Error(
        "DataSource must be initialized before creating virtual table"
      );
    }
    ...
    if (!exists) {
      try {
        ...
        log.info(
          `vec0 virtual table '${virtualTableName}' created successfully for model '${modelName}' with dimension ${dimension}`
        );
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        log.warn(
          `Failed to create vec0 virtual table '${virtualTableName}':`,
          errorMessage
        );
        log.warn(
          "Vector search will fall back to vec_distance_l2 function on regular table"
        );
   ...
   */
  async getVirtualTableName(
    modelName: string,
    dimension: number
  ): Promise<string | null> {
    const metadata = await this.metadataModel.findByModelAndDimension(
      modelName,
      dimension
    );
  ...
  async searchSimilarVectorsForModel(
    ...
    const virtualTableName = await this.getVirtualTableName(
      modelName,
    ...
    if (virtualTableName) {
      ...
      return await this.searchSimilarVectorsWithVec0(
        queryVector,
        k,
        dimension,
        virtualTableName
