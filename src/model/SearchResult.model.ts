...
import { searchResultWriteSchema } from "@/schemas/entity/searchResult";
...
export class SearchResultModel extends BaseDb {
  ...
  constructor(filepath: string) {
    super(filepath);
    this.repository =
      this.sqliteDb.connection.getRepository(SearchResultEntity);
  }
  ...
  async saveResult(data: SearchResEntity, taskId: number): Promise<number> {
    ...
    // Scraped results can carry debug/temp fields (htmlSnippet, source_tag,
    // etc.) that don't map to TypeORM columns.
    ...
    resultEntity.title = stripped.title as string;
    resultEntity.link = stripped.link as string;
    resultEntity.snippet = stripped.snippet as string;
    resultEntity.domain = stripped.domain as string;
    resultEntity.record_time = stripped.record_time as string;
  ...
  async countByDateRange(startDate: Date, endDate: Date): Promise<number> {
    return this.repository
      ...
      .where("result.record_time >= :startDate", {
        startDate: startDate.toISOString(),
      })
      .andWhere("result.record_time <= :endDate", {
   ...
   */
  async getSearchResultsByTaskId(
    taskId: number,
    page = 0,
    size = 10
   ...
   */
  async getAllSearchResultsByTaskId(
    taskId: number
  ): Promise<SearchResEntity[]> {
    const results = await this.repository.find({
      where: { task_id: taskId },
   ...
   */
  async getBreakdownByEngine(
    startDate: Date,
    endDate: Date
  ): Promise<Array<{ engineId: number; count: number }>> {
    ...
    return rows
      .map((row: { engineId: string | number | null; count: string }) => {
      ...
      .filter(
        (item): item is { engineId: number; count: number } => item !== null
      );
  }
   ...
   */
  async updateAiAnalysisStatusBatch(
    resultIds: number[],
    status: string
  ): Promise<number> {
    try {
