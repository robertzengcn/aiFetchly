...
export class ScheduleExecutionLogModel extends BaseDb {
  ...
  async logExecution(
    ...
    executionLog.completion_time =
      status !== ExecutionStatus.RUNNING ? new Date() : null;
  ...
  async updateExecutionStatus(
    ...
    const updateData: QueryDeepPartialEntity<ScheduleExecutionLogEntity> = { status };
   ...
   * @returns Array of execution log entities
   */
  async getExecutionsByTriggerType(
  ...
  async getExecutionById(
    executionId: number
  ): Promise<ScheduleExecutionLogEntity | null> {
  ...
  async getExecutionsInDateRange(
    ...
    if (scheduleId) {
      queryBuilder.andWhere("execution.schedule_id = :scheduleId", {
        scheduleId,
      });
    }
