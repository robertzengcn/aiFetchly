...
export class BuckEmailTaskdb {
  ...
  create(task: BuckemailEntity): number {
    ...
    const info = stmt.run({
      ...
      emailtaskentityId: task.emailtaskentityId ?? null,
      email_list_json: task.email_list_json ?? null,
      notduplicate: task.notduplicate ?? 0,
    });
  ...
  //list task
  public listBuckEmailtask(
    page: number,
    size: number,
    sort?: SortBy
  ): Array<BuckemailEntity> {
