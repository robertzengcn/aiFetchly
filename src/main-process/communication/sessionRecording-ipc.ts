...
import {
  ...
  sessionRecordingExportInputSchema,
  sessionRecordingClearInputSchema,
} from "@/schemas/ipc/sessionRecording";

/**
 * IPC Handlers for Session Recording Management
 *
 * 6 handlers migrated to registerValidatedHandler:
 *  - toggle / get-status / get-sessions / export / clear / get-directory
 *
 * Envelope caveat: original returned { success: boolean, ... } shape;
 * wrapper wraps in { status: true, msg: 'ok', data: <handler return> }.
 * So callers that previously read `result.success` should now read
 * `result.data.success`. This is a wire-shape change for this file
 * specifically - documented for frontend review.
 */
...
export function registerSessionRecordingIpcHandlers(): void {
  ...
  registerValidatedHandler(
    ...
    async () => {
      ...
      return { enabled: isRecordingEnabled };
    }
  );
  ...
  registerValidatedHandler(
    ...
    noInputSchema,
    async () => {
      return { directory: sessionsDirectory };
    }
  );
...
async function exportSessions(options: any): Promise<any> {
  ...
  switch (format) {
    ...
    case "openai":
      ...
      exportData = sessions.flatMap(
        (session) =>
          session.trainingData?.map((point: any) => ({
            messages: [
              {
                role: "user",
                content: `Given this HTML state: ${point.state}`,
              },
              {
                role: "assistant",
                content: `Perform this action: ${point.action}`,
              },
            ],
          })) || []
      );
  ...
  const exportPath = path.join(
    sessionsDirectory,
    `export_${format}_${Date.now()}.${format === "csv" ? "csv" : "json"}`
  );
