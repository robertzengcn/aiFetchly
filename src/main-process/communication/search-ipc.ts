...
export function registerSearchIpcHandlers(): void {
  ipcMain.on(SEARCHSCRAPERAPI, async (event, arg: unknown): Promise<void> => {
    ...
    if (!("searchEnginer" in qdata)) {
      ...
      ).sender.send(SEARCHEVENT, JSON.stringify(comMsgs));
      return;
    }
    ...
    try {
      ...
      const comMsgs: CommonDialogMsg = {
        ...
        data: {
          ...
          title: "search.scraper_failed",
          content:
            error instanceof Error ? error.message : "search.scraper_failed",
        },
      };
      (
        event as {
  ...
  ipcMain.handle(
    LISTSESARCHRESUT,
    async (event, data): Promise<CommonResponse<SearchtaskItem>> => {
  ...
  ipcMain.handle(
    TASKSEARCHRESULTLIST,
    async (event, data): Promise<CommonResponse<SearchResEntity>> => {
      const qdata = JSON.parse(data as string) as SearchResultFetchparam;
      if (!("taskId" in qdata)) {
  ...
  ipcMain.handle(
    SAVESEARCHERRORLOG,
    async (event, data): Promise<string | undefined> => {
      const qdata = JSON.parse(data as string) as { id: number };
      const { filePath } = await dialog.showSaveDialog({
        title: "Save Text File",
        defaultPath: path.join(
          app.getPath("documents"),
          qdata.id.toString() + "_search-error-log.txt"
        ),
  ...
  ipcMain.on(RETRYSEARCHTASK, async (event, data): Promise<void> => {
    ...
    if (!qdata.id) {
      ...
      ).sender.send(SYSTEM_MESSAGE, JSON.stringify(resp));
      return;
    }
  ...
  ipcMain.handle(
    GET_SEARCH_TASK_DETAILS,
    async (event, data): Promise<CommonMessage<TaskDetailsForEdit>> => {
      const qdata = JSON.parse(data as string) as { id: number };
      if (!qdata.id) {
  ...
  ipcMain.handle(
    UPDATE_SEARCH_TASK,
    async (event, data): Promise<CommonMessage<any>> => {
      ...
      try {
        const searchControl = SearchController.getInstance();
        const success = await searchControl.updateSearchTask(
          qdata.id,
          qdata.updates
        );
  ...
  ipcMain.handle(
    CREATE_SEARCH_TASK_ONLY,
    async (event, data): Promise<CommonMessage<number>> => {
      const qdata = JSON.parse(data as string) as Usersearchdata;
  ...
  ipcMain.handle(
    ...
    async (event, data): Promise<CommonMessage<any | null>> => {
      ...
      try {
        ...
        const exportData = await searchControl.exportSearchResults(
          qdata.taskId,
          format
        );
        ...
        const defaultFilename = `search_results_task_${qdata.taskId}_${
