...
export function resetAiChatV2RuntimeForDatabaseSwitch(): void {
  ...
  if (queueService) {
    ...
    // event associations are dropped (design §16.2). The next database runs
    // its own recovery on first use.
    queueService = null;
  }
...
/**
 * Broadcaster-backed event sink: forwards engine events to every live
 * window on the existing stream channels. Queue-dispatched turns start in
 * the main process, so the per-turn renderer listener is no longer the
 * only consumer (design §14.2).
 */
  ...
  const sink = createEventSink({
    sender: {
      send: (channel: string, message: string) => {
        if (channel === AI_CHAT_V2_STREAM_COMPLETE) {
          broadcaster.emitStreamComplete(
            JSON.parse(message) as ChatV2StreamChunk
          );
        } else {
          broadcaster.emitStreamChunk(JSON.parse(message) as ChatV2StreamChunk);
        }
      },
    },
  });
  // Forward the new direction_updated event type through the same chunk
  // channel: map it onto a ChatV2StreamChunk so the renderer pipeline stays
  // uniform. Contains ids and offsets only — never steering content.
  const originalEmit = sink.emit.bind(sink);
  return {
    emit: (event: AIChatQueryEvent) => {
      if (event.type === "direction_updated") {
        broadcaster.emitStreamChunk({
          eventType: "direction_updated" as ChatV2StreamChunk["eventType"],
          conversationId: event.conversationId,
          messageId: event.messageId,
          toolCallId: undefined,
          // Direction fields ride on the chunk via a dedicated sub-object.
          directionBoundary: event.boundary,
          directionPendingMessageIds: [...event.pendingMessageIds],
          directionContentOffset: event.contentOffset,
        } as unknown as ChatV2StreamChunk);
        return;
      }
      originalEmit(event);
    },
...
async function ensureQueueRecovered(): Promise<void> {
  ...
  try {
    ...
    console.error("[ai-chat-v2] pending queue recovery failed:", err);
  }
}
...
async function handleHistory(
  ...
  try {
    ...
    try {
      ...
      console.error("[ai-chat-v2] pending list failed:", err);
    }
    return ok({
      conversationId,
      messages: views,
...
async function handleClearConversation(
  ...
  try {
    ...
    try {
      ...
      console.error("[ai-chat-v2] pending clearConversation failed:", err);
    }
    const module = new AIChatV2Module();
...
> {
  ...
  try {
    try {
      ...
      console.error("[ai-chat-v2] pending clearAll failed:", err);
    }
    const module = new AIChatV2Module();
...
/**
 * registerValidatedHandler variant that checks CHAT availability before
 * parsing — the queue serves the same users the chat stream does (hosted
 * subscription OR valid local provider), so canUseChat is the correct gate
 * here rather than the hosted-only USER_AI_ENABLED check.
 */
