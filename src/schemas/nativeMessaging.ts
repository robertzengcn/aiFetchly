...
/** Entropy size for the one-time requestSecret generated by the registry. */
export const REQUEST_SECRET_BYTES = 32;
/** ASCII-only secret charset (base64url). Rejects surrogate-pair inputs that
 *  could otherwise make the byte-length differ from the JS string length. */
...
export const nativeImportRequestSchema = z.strictObject({
  ...
  requestSecret: z.string().min(16).max(128).regex(SECRET_CHARSET),
