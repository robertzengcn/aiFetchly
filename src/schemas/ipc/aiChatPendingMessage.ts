...
const streamRequestSchema = z.strictObject({
  ...
  generatedImageReferences: z
    ...
    .max(3)
    .optional(),
  // Trusted channel for the user-confirmed batch reference set (>3 images).
  // Mirrors the ai-chat-v2:stream handler's contract: the handler validates,
  // stages the set into ConfirmedBatchReferenceRegistry, and strips the field
  // before the request is stored or reaches the engine/model.
  confirmedGeneratedImageBatch: z
    .strictObject({
      references: z
        .array(
          z.strictObject({
