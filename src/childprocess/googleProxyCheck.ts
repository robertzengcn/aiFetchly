...
import {
  googleProxyCheckWorkerInboundSchema,
  type GoogleProxyCheckWorkerInbound,
} from "@/schemas/worker/googleProxyCheck";
 ...
 */
async function checkGooglePass(
  proxy: ProxyParseItem,
  timeout = 15000
): Promise<boolean> {
  let browser;
  try {
    ...
    // Check for reCAPTCHA
    const hasRecaptcha =
      (await page.$('.g-recaptcha, #recaptcha, [class*="recaptcha"]')) !== null;

    // Check for search input (indicates success)
    const searchInput = await page.$(
      'input[name="q"], #search, textarea[name="q"]'
    );
...
if (parentPort) {
  parentPort.on("message", async (e: { data: string }) => {
    ...
    try {
      ...
      const validation = parseWorkerMessage<GoogleProxyCheckWorkerInbound>(
        raw,
        googleProxyCheckWorkerInboundSchema()
      );
      if (!validation.success) {
        log.warn(
          "[googleProxyCheck] dropped malformed CHECK_GOOGLE_PASS:",
          validation.error
        );
        return;
      }
      ...
      if (message.type === "CHECK_GOOGLE_PASS") {
        log.info(
          `🔍 Checking Google pass for proxy: ${message.proxy.host}:${message.proxy.port}`
        );
        const result = await checkGooglePass(
          message.proxy as unknown as ProxyParseItem,
          message.timeout
        );
