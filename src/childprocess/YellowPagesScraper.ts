...
interface PendingAiRequest {
  resolve: (result: AiSupportResult) => void;
...
export class YellowPagesScraper {
  ...
  // AI support
  private aiSupportEnabled: boolean;
  ...
  constructor(taskData: TaskData, platformInfo: PlatformInfo) {
    ...
    this.aiSupportEnabled = taskData.aiSupportEnabled ?? false;
   ...
   */
  handleAiSupportResponse(message: Record<string, unknown>): void {
  ...
  private async requestAiSupport(params: {
    ...
    if (!this.aiSupportEnabled) {
      return {
        ...
        errorMessage: "AI support disabled",
      };
    }
    ...
    if (!parentPort) {
      return {
        success: false,
        requestType: params.requestType,
        errorMessage: "No parentPort available",
      };
    }
    ...
    const requestId = `ai-${this.taskData.taskId}-${
      this.aiRequestCounter
    }-${Date.now()}`;
  ...
  async start(): Promise<void> {
    try {
      ...
      console.log(`📋 Task configuration:`, {
        ...
        hasCookies: !!(
          this.taskData.cookies && this.taskData.cookies.length > 0
        ),
  ...
  private async scrapeTask(): Promise<ScrapingResult[]> {
    ...
    const hasCustomSearch =
      this.adapter && this.adapterSupportsFeature("custom-search");
    const hasCustomExtraction =
      this.adapter && this.adapterSupportsFeature("custom-extraction");
    const hasCustomPagination =
      this.adapter && this.adapterSupportsFeature("custom-pagination");
    const hasCustomPageLoad =
      this.adapter && this.adapterSupportsFeature("custom-page-load");
    const hasCustomEmailExtraction =
      this.adapter && this.adapterSupportsFeature("custom-email-extraction");
    const hasCustomPhoneExtraction =
      this.adapter && this.adapterSupportsFeature("custom-phone-extraction");
    const hasCustomWebsiteExtraction =
      this.adapter && this.adapterSupportsFeature("custom-website-extraction");
    ...
    if (this.adapter) {
      ...
      console.log(`📋 Adapter methods:`, {
        ...
        extractEmailFromDetailPage: hasCustomEmailExtraction
          ? "Custom"
          : "Default",
        extractPhoneNumberWithReveal: hasCustomPhoneExtraction
          ? "Custom"
          : "Default",
        extractWebsiteWithReveal: hasCustomWebsiteExtraction
          ? "Custom"
          : "Default",
        extractAddressFromBusinessSection: hasCustomAddressExtraction
          ? "Custom"
          : "Default",
      });
    } else {
    ...
    while (currentPage <= maxPages && hasMoreContent && this.isRunning) {
      ...
      try {
        ...
        if (currentPage === 1) {
          ...
          hasMoreContent = await this.handleEnhancedPagination(
            currentPage - 1,
            maxPages
          );
    ...
    try {
      ...
      if (this.sessionManager.getRecordingStatus() && this.page) {
        ...
        this.sessionManager.logAction(
          currentState,
          `goto('${this.platformInfo.base_url}')`
        );
      }
      ...
      const hasSearchFormSelectors =
        this.platformInfo.selectors.searchForm &&
   ...
   */
  private async fillSearchFormWithPlatformSelectors(
    keyword: string,
    location: string
  ): Promise<void> {
  ...
  private async navigateToPage(pageNum: number): Promise<void> {
    ...
    try {
      ...
      const hasPaginationSelectors =
        this.platformInfo.selectors.pagination &&
   ...
   */
  private async navigateToPageWithPlatformSelectors(
    pageNum: number
  ): Promise<void> {
    ...
    try {
      ...
      if (pagination.pageNumbers) {
        const pageSelector = pagination.pageNumbers.replace(
          "{page}",
          pageNum.toString()
        );
   ...
   */
  private async navigateToPageWithGenericSelectors(
    pageNum: number
  ): Promise<void> {
    if (!this.page) return;
   ...
   */
  private buildFallbackSearchUrl(
    keyword: string,
    location: string,
    pageNum: number
  ): string {
    ...
    const searchUrlPattern =
      settings.searchUrlPattern || `${this.platformInfo.base_url}/search`;
    ...
    try {
      ...
      for (let i = 0; i < businessElements.length; i++) {
        ...
        try {
          ...
          const businessId = await this.generateBusinessIdentifier(
            currentElement,
            businessSelectors
          );
          ...
          const result = await this.extractBusinessFromElement(
            currentElement,
            businessSelectors
          );
          console.log(
  ...
  private async generateBusinessIdentifier(
    ...
    try {
      ...
      return `fallback:${elementIndex}:${fallbackText
        .substring(0, 50)
        .toLowerCase()}`;
    } catch (error) {
        ...
        try {
          ...
          if (loadMoreButton) {
            ...
            if (isVisible && isClickable) {
              ...
              const currentBusinessCount = await this.page
              ...
              // Get new count
              const newBusinessCount = await this.page
                .$$(this.platformInfo.selectors.businessItem)
                .then((elements) => elements.length);
  ...
  private async handleEnhancedPagination(
    ...
    try {
      ...
      if (this.isInNewTab) {
        ...
        if (browser) {
          ...
          if (searchResultsPage) {
            ...
            await searchResultsPage.waitForSelector(
              this.platformInfo.selectors.businessList,
              { timeout: 10000 }
            );
      ...
      if (traditionalPaginationWorked) {
        ...
        await this.savePageHtmlForDebugging(
          currentPage + 1,
          "traditional_pagination_failed"
        );
  ...
  private async navigateToDetailPageAndExtract(
    ...
    try {
      ...
      if (this.sessionManager.getRecordingStatus() && this.page) {
        ...
        this.sessionManager.logAction(
          currentState,
          `click('${selectors.navigation.detailLink}')`
        );
      }
  ...
  private calculateUrlSimilarity(url1: string, url2: string): number {
    try {
      ...
      if (params1Str === params2Str) {
        ...
        const sharedParams = Array.from(params1.keys()).filter((key) =>
          params2.has(key)
        );
    ...
    const pageScores = await Promise.all(
      pages.map(async (page) => {
        ...
        try {
          ...
          if (this.searchPageUrl) {
            const similarityScore = this.calculateUrlSimilarity(
              url,
              this.searchPageUrl
            );
          ...
          if (
            errorKeywords.some((keyword) => url.toLowerCase().includes(keyword))
          ) {
            score -= 5;
          }
   ...
   */
  private async navigateBackToSearchResults(
    selectors: PlatformInfo["selectors"]
  ): Promise<void> {
    if (!this.page) return;
  ...
  private async extractEnhancedDataFromDetailPage(
    ...
    try {
      ...
      if (detailSelectors.businessName) {
        const enhancedName = await this.extractTextFromPage(
          detailSelectors.businessName
        );
      ...
      if (detailSelectors.website) {
        const website = await this.extractAttributeFromPage(
      ...
      if (detailSelectors.fullAddress) {
        const fullAddress = await this.extractTextFromPage(
          detailSelectors.fullAddress
        );
        if (fullAddress) {
      ...
      if (
        ...
        try {
          ...
          const adapterAddress =
            await this.adapter.extractAddressFromBusinessSection(this.page!);
      ...
      if (detailSelectors.businessHours) {
        const detailedHours = await this.extractObjectFromPage(
          detailSelectors.businessHours
        );
      ...
      if (detailSelectors.description) {
        const fullDescription = await this.extractTextFromPage(
      ...
      if (detailSelectors.contactInfo) {
        const contactInfo = await this.extractTextFromPage(
          detailSelectors.contactInfo
        );
        if (contactInfo) {
          // Parse contact info for additional fields
          const parsedContact = this.parseContactInfo(contactInfo);
          enhancedResult.contact_person =
            parsedContact.contactPerson || enhancedResult.contact_person;
          enhancedResult.fax_number =
            parsedContact.faxNumber || enhancedResult.fax_number;
        }
      }

      // Extract services if available
      if (detailSelectors.services) {
        const services = await this.extractArrayFromPage(
          detailSelectors.services
        );
      ...
      if (detailSelectors.additionalPhone) {
        const additionalPhone = await this.extractTextFromPage(
          detailSelectors.additionalPhone
        );
      ...
      if (detailSelectors.socialMedia) {
        const socialMedia = await this.extractArrayFromPage(
          detailSelectors.socialMedia
        );
      ...
      if (detailSelectors.categories) {
        const detailedCategories = await this.extractArrayFromPage(
          detailSelectors.categories
        );
      ...
      if (detailSelectors.yearEstablished) {
        const yearEstablished = await this.extractNumberFromPage(
          detailSelectors.yearEstablished
        );
      ...
      if (detailSelectors.numberOfEmployees) {
        const numberOfEmployees = await this.extractTextFromPage(
          detailSelectors.numberOfEmployees
        );
        if (numberOfEmployees)
          enhancedResult.number_of_employees = numberOfEmployees;
      }

      // Extract payment methods if available
      if (detailSelectors.paymentMethods) {
        const paymentMethods = await this.extractArrayFromPage(
          detailSelectors.paymentMethods
        );
      ...
      if (detailSelectors.specialties) {
        const specialties = await this.extractArrayFromPage(
          detailSelectors.specialties
        );
    ...
    if (
      ...
      try {
        const aiResult = await this.requestAiSupport({
          ...
          pageUrl: this.page?.url() || "",
          businessName: enhancedResult.business_name,
          stepContext: "detail_page_extraction",
          errorInfo:
            "Selector-based extraction found no email, phone, or website",
        });

        if (aiResult.success && aiResult.data) {
          const d = aiResult.data;
          if (d.emails && d.emails.length > 0 && !enhancedResult.email) {
            enhancedResult.email = d.emails[0];
          }
          if (d.phones && d.phones.length > 0 && !enhancedResult.phone) {
            enhancedResult.phone = d.phones[0];
          }
          if (d.address && !enhancedResult.address?.street) {
            enhancedResult.address = {
              ...enhancedResult.address,
              street: d.address,
            };
   ...
   */
  private async extractAttributeFromPage(
    selector: string,
    attribute: string
  ): Promise<string | undefined> {
    if (!this.page) return undefined;
   ...
   */
  private async captureDetailPageForTraining(
    businessName: string
  ): Promise<void> {
    if (!this.page) return;
    ...
    try {
      ...
      console.log(`📊 Detail page training data captured:`, {
        ...
        htmlLength: rawHtml.length,
        hasBusinessInfo:
          !!pageStructure.structureInfo["h1"] ||
          !!pageStructure.structureInfo[".business-name"],
   ...
   */
  private async extractArrayFromPage(
    selector: string
  ): Promise<string[] | undefined> {
    if (!this.page) return undefined;
   ...
   */
  private async extractObjectFromPage(
    selector: string
   ...
   */
  private async extractNumberFromPage(
    selector: string
   ...
   */
  private parseContactInfo(contactText: string): {
    contactPerson?: string;
    faxNumber?: string;
  } {
  ...
  private parseAddressString(addressString: string): {
    ...
    try {
      ...
      if (match) {
        ...
        const stateMatch = cleanAddress.match(
  ...
  private async extractBusinessFromElement(
    ...
    try {
      ...
      const business_name = await this.extractText(
        element,
      ...
      if (selectors.website) {
        websiteUrl = await this.extractAttribute(
          element,
          selectors.website,
          "href"
        );
      ...
      const result: ScrapingResult = {
        business_name,
        email: selectors.email
          ? await this.extractText(element, selectors.email)
          : undefined,
        phone: phoneNumber,
        website: websiteUrl,
        address: {
          street: selectors.address
            ? await this.extractText(element, selectors.address)
            : undefined,
          city: selectors.address_city
            ? await this.extractText(element, selectors.address_city)
            : undefined,
          state: selectors.address_state
            ? await this.extractText(element, selectors.address_state)
            : undefined,
          zip: selectors.address_zip
            ? await this.extractText(element, selectors.address_zip)
            : undefined,
          country: selectors.address_country
            ? await this.extractText(element, selectors.address_country)
            : undefined,
        },
        social_media: selectors.socialMedia
          ? await this.extractArray(element, selectors.socialMedia)
          : undefined,
        categories: selectors.categories
          ? await this.extractArray(element, selectors.categories)
          : undefined,
        business_hours: selectors.businessHours
          ? await this.extractObject(element, selectors.businessHours)
          : undefined,
        description: selectors.description
          ? await this.extractText(element, selectors.description)
          : undefined,
        rating: selectors.rating
          ? await this.extractNumber(element, selectors.rating)
          : undefined,
        review_count: selectors.reviewCount
          ? await this.extractNumber(element, selectors.reviewCount)
          : undefined,
        fax_number: selectors.faxNumber
          ? await this.extractText(element, selectors.faxNumber)
          : undefined,
        contact_person: selectors.contactPerson
          ? await this.extractText(element, selectors.contactPerson)
          : undefined,
        year_established: selectors.yearEstablished
          ? await this.extractNumber(element, selectors.yearEstablished)
          : undefined,
        number_of_employees: selectors.numberOfEmployees
          ? await this.extractText(element, selectors.numberOfEmployees)
          : undefined,
        payment_methods: selectors.paymentMethods
          ? await this.extractArray(element, selectors.paymentMethods)
          : undefined,
        specialties: selectors.specialties
          ? await this.extractArray(element, selectors.specialties)
          : undefined,
      };
      ...
      for (const selector of cloudflareSelectors) {
        try {
          if (selector.startsWith("text=")) {
            ...
            const element = await this.page!.$eval(selector, () => true).catch(
              () => false
            );
            if (element) {
              console.log(
      ...
      if (isRobotVerification) {
        ...
        try {
          const pageInfo = await this.page.evaluate(() => {
            ...
            const hasCaptcha = !!document.querySelector(
              '.captcha, .recaptcha, .hcaptcha, [class*="captcha"]'
            );
            const hasVerificationForm = !!document.querySelector(
              '[class*="verification"], [class*="robot"]'
            );
        ...
        const robotVerificationMessage = {
          ...
          details: {
            ...
            message:
              "The scraping process has been paused due to a robot verification challenge. Please complete the verification manually and resume the task.",
          },
        };

        // Send message to parent process
        const parentPortLocal3 = (
          process as unknown as {
            parentPort?: { postMessage: (message: unknown) => void };
          }
        ).parentPort;
          ...
          try {
            ...
            const pauseNotificationMessage = {
              ...
              details: {
                ...
                timestamp: timestamp,
                recommendation:
                  "Manual intervention required - wait 15-30 minutes before retrying",
              },
            };

            const parentPortLocal5 = (
              process as unknown as {
                parentPort?: { postMessage: (message: unknown) => void };
              }
            ).parentPort;
   ...
   */
  private async humanLikeMouseMove(
    page: Page,
    targetX: number,
    targetY: number
  ): Promise<void> {
  ...
  private adapterSupportsFeature(feature: string): boolean {
    ...
    switch (feature) {
      case "custom-search":
        return (
          this.adapter.searchBusinesses !==
          BasePlatformAdapter.prototype.searchBusinesses
        );
      case "custom-extraction":
        return (
          this.adapter.extractBusinessData !==
          BasePlatformAdapter.prototype.extractBusinessData
        );
      case "custom-pagination":
        return (
          this.adapter.handlePagination !==
          BasePlatformAdapter.prototype.handlePagination
        );
      case "custom-page-load":
        return (
          this.adapter.onPageLoad !== BasePlatformAdapter.prototype.onPageLoad
        );
      case "custom-email-extraction":
        return (
          this.adapter.extractEmailFromDetailPage !==
          BasePlatformAdapter.prototype.extractEmailFromDetailPage
        );
      case "custom-phone-extraction":
        return (
          this.adapter.extractPhoneNumberWithReveal !==
          BasePlatformAdapter.prototype.extractPhoneNumberWithReveal
        );
      case "custom-website-extraction":
        return (
          this.adapter.extractWebsiteWithReveal !==
          BasePlatformAdapter.prototype.extractWebsiteWithReveal
        );
      case "custom-address-extraction":
        return (
          this.adapter.extractAddressFromBusinessSection !==
          BasePlatformAdapter.prototype.extractAddressFromBusinessSection
        );
      default:
        return false;
    }
  ...
  private getAdapterCapabilities(): string[] {
    ...
    if (
      ...
      capabilities.push("custom-search");
    if (
      this.adapter.extractBusinessData !==
      BasePlatformAdapter.prototype.extractBusinessData
    )
      capabilities.push("custom-extraction");
    if (
      this.adapter.handlePagination !==
      BasePlatformAdapter.prototype.handlePagination
    )
      capabilities.push("custom-pagination");
    if (this.adapter.onPageLoad !== BasePlatformAdapter.prototype.onPageLoad)
      capabilities.push("custom-page-load");
    if (
      this.adapter.extractEmailFromDetailPage !==
      BasePlatformAdapter.prototype.extractEmailFromDetailPage
    )
      capabilities.push("custom-email-extraction");
    if (
      this.adapter.extractPhoneNumberWithReveal !==
      BasePlatformAdapter.prototype.extractPhoneNumberWithReveal
    )
      capabilities.push("custom-phone-extraction");
    if (
      this.adapter.extractWebsiteWithReveal !==
      BasePlatformAdapter.prototype.extractWebsiteWithReveal
    )
      capabilities.push("custom-website-extraction");
    if (
      this.adapter.extractAddressFromBusinessSection !==
      BasePlatformAdapter.prototype.extractAddressFromBusinessSection
    )
      capabilities.push("custom-address-extraction");
   ...
   */
  private async savePageHtmlForDebugging(
    pageNumber: number,
    reason: string
  ): Promise<void> {
    if (!this.page) return;
    ...
    if (message.type === "START" && message.taskData && message.platformInfo) {
      ...
      try {
        ...
        const errorMessage: ErrorMessage = {
          type: "ERROR",
          taskId: message.taskData.taskId,
          content: `Failed to initialize scraper: ${
            error instanceof Error ? error.message : String(error)
          }`,
      ...
      scraper.onProgress((progress) => {
        const progressMessage: ProgressMessage = {
          type: "PROGRESS",
          taskId: message.taskData.taskId,
          content: `Scraping progress: Page ${progress.currentPage}/${
            progress.totalPages
          } (${progress.percentage.toFixed(1)}%) - ${
            progress.resultsCount
          } results found`,
          progress,
        };
        if (parentPort) {
      ...
      try {
        ...
        const errorMessage: ErrorMessage = {
          type: "ERROR",
          taskId: message.taskData.taskId,
          content: `Failed to start scraping: ${
            error instanceof Error ? error.message : String(error)
          }`,
          error: error instanceof Error ? error.message : String(error),
        };
      ...
      if (globalScraper) {
        globalScraper.handleAiSupportResponse(
