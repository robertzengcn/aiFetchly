...
export class SearchScrape implements searchEngineImpl {
  ...
  constructor(options: ScrapeOptions) {
    ...
    let settings = (this.config as Record<string, unknown>)[
      `${this.config.platform}_settings`
    ];
    if (settings) {
  ...
  async exposeFunction() {
    ...
    const _attr = async (el: { $eval: (selector: string) => Promise<{ getAttribute: (a: string) => string | null } | null> }, s: string, attr: string) => {
      const n = await el.$eval(s);
