...
import { getAiSupportBridge } from "@/childprocess/utils/AiSupportBridge";
...
export class SearchScrape implements searchEngineImpl {
  ...
  protected async requestAiSupportObserveExecute(req: {
    ...
    if (!this.parentPort) {
      return {
        success: false,
        requestType: "observe_execute",
        errorMessage: "parentPort not available",
      };
    }
    ...
    const response = await getAiSupportBridge().requestAiSupport(
      this.parentPort,
      message
    );
  ...
  protected async attemptAIRecovery(
    ...
    if (!this.aiRecoveryEnabled || !this.parentPort) {
      return {
        success: false,
        error: "AI recovery not enabled or parentPort not available",
      };
    }
    try {
      ...
      const result = await runObserveExecuteLoop(
        ...
        },
        {
          ...
          captureState: async () => {
            const pageUrlNow = await this.page.url();
            let pageContentNow = await this.page.content();
            pageContentNow = pageContentNow
              .replace(
              ...
              .trim()
              .substring(0, 10000);
            let screenshotNow: string | undefined;
            try {
              screenshotNow = (await this.page.screenshot({
                type: "png",
                encoding: "base64",
                fullPage: false,
              })) as string;
            ...
            return {
              pageUrl: pageUrlNow,
              pageContent: pageContentNow,
              screenshot: screenshotNow,
            };
          },
        }
      );
  ...
  async run(data: {
    ...
    if (data.data.proxyServer) {
      if (data.data.proxyServer != undefined) {
        ...
        this.page.on("request", async (interceptedRequest) => {
          ...
          await useProxy(
            interceptedRequest,
            convertProxyServertourl(data.data.proxyServer!)
          );
          if (
            interceptedRequest.interceptResolutionState().action ===
            InterceptResolutionAction.AlreadyHandled
    ...
    if (
      !this.config.scrape_from_file ||
      this.config.scrape_from_file.length <= 0
    ) {
    ...
    if (this.hasCookies && this.page) {
      try {
        ...
        if (browserCookies && browserCookies.length > 0) {
          ...
          console.log(
            `Captured ${updatedCookies.length} updated cookies for account ${this.accountId}`
          );
        }
      } catch (error) {
    ...
    // If we have accountId and set cookies but capture failed/empty, pass back the cookies we set
    // so the main process can still update record_time (avoids stale update time in account_cookies)
    if (
      this.accountId != null &&
      updatedCookies == null &&
      this.hasCookies &&
      ...
      updatedCookies = data.data.cookies;
    }
  ...
  async load_search_engine(): Promise<boolean | void> {
    ...
    if (this.config.test_evasion === true) {
      ...
      if (this.config.debug_log_path) {
        ...
        await this.page.screenshot({
          fullPage: true,
          path: "headless-evasion-result.png",
        });
      }
      ...
      if (this.config.debug_log_path) {
        await new Promise((resolve) => setTimeout(resolve, 1000));
        this.logger.info(
          ("antoinevastel-result path:" +
            path.join(
              this.config.debug_log_path,
              "antoinevastel-result.png"
            )) as `${string}.png`
        );
        await this.page.screenshot({
          fullPage: true,
        ...
        this.logger.info(
          "Saved HTML content to: " +
            path.join(this.config.debug_log_path, "antoinevastel-result.html")
        );
      } else {
        await this.page.screenshot({
          fullPage: true,
          path: "antoinevastel-result.png",
        });
      }
    }
    ...
    if (this.proxy && this.config.log_ip_address === true) {
      ...
      if (
        this.metadata.ipinfo &&
        !this.proxy.includes(this.metadata.ipinfo.ip)
      ) {
        throw new Error(
          `Proxy output ip ${this.proxy} does not match with provided one`
        );
      } else {
    ...
    for (const keyword of this.keywords) {
      ...
      try {
        ...
        if (
          ...
          this.last_response = await this.page.goto(
            this.config.scrape_from_file
          );
        }
        ...
        do {
          ...
          if (parsed) {
            ...
            if (pareseres) {
              ...
              this.logger.warn(
                `No results found for keyword "${keyword}" on page ${this.page_num}`
              );
          ...
          if (this.config.screen_output && this.config.debug_log_path) {
            ...
            //     fullPage: false,
            // });
            const html_screenshot_path = path.join(
              this.config.debug_log_path,
              `html_debug_se_scraper_${
                this.search_engine_name
              }_${keyword}_${Date.now()}.png`
            );
            await this.page.screenshot({
              path: html_screenshot_path as `${string}.png`,
            });
          }
        ...
        this.logger.warn(
          `Problem with scraping ${keyword} in search engine: ${
            (e as Error).message
          }`
        );
        ...
        if (this.config.take_screenshot_on_error) {
          if (this.config.debug_log_path) {
            ...
            await this.page.screenshot({
              path: screenshot_path as `${string}.png`,
            });
            //await fs.promises.writeFile(screenshot_path, screenshot);
          } else {
            this.logger.info(
              `sceen path:` +
                `debug_se_scraper_${this.search_engine_name}_${keyword}.png`
            );
            await this.page.screenshot({
              path: `debug_se_scraper_${this.search_engine_name}_${keyword}.png`,
            });
          }
        }
        if (this.config.save_html) {
          ...
          if (this.config.debug_log_path) {
            this.logger.info(
              `Saving html to ${path.join(
                this.config.debug_log_path,
                `debug_se_scraper_${this.search_engine_name}_${keyword}.html`
              )}`
            );
            await fs.promises.writeFile(
              path.join(
        ...
        if (this.metadata.scraping_detected === true) {
          ...
          if (this.config.is_local === true) {
            ...
            this.logger.info(
              `You have ${this.SOLVE_CAPTCHA_TIME}ms to enter the captcha.`
            );
