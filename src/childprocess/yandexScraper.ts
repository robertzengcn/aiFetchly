...
export class YandexScraper extends SearchScrape {
  ...
  async parse_async(): Promise<SearchData> {
    ...
    for (const selector of alternativeSelectors) {
      ...
      if (results) {
        findelement = true;
        this.logger.info(
          `Found results with alternative selector: ${selector}`
        );
        ...
        this.logger.info(
          `Found ${searchRes.length} results with alternative selector: ${selector}`
        );
          ...
          if (
            ...
            try {
              ...
              try {
                ...
                if (response) {
                  ...
                  if (status === 200) {
                    ...
                    this.logger.debug(
                      `Successfully resolved redirect for: ${seval.link}`
                    );
                  } else {
                    this.logger.warn(
                      `Non-200 status (${status}) when resolving redirect for: ${seval.link}`
                    );
                  }
                } else {
                  this.logger.warn(
                    `Null response when resolving redirect for: ${seval.link}`
                  );
                }
              } catch (error) {
                const errorMessage =
                ...
                if (error instanceof TimeoutError) {
                  this.logger.warn(
                    `Navigation timed out for redirect link: ${seval.link}. Error: ${errorMessage}`
                  );
                } else {
                  this.logger.warn(
                    `Error navigating to redirect link: ${seval.link}. Error: ${errorMessage}`
                  );
                }
              } finally {
                if (!newPage.isClosed()) {
              ...
              const errorMessage =
                error instanceof Error ? error.message : String(error);
              this.logger.error(
                `Error creating new page for redirect: ${seval.link}. Error: ${errorMessage}`
              );
            }
          }
    ...
    if (!findelement) {
      ...
      throw new CustomError(
        "No search results found, may be element not found in the list page",
        202405301120304
      );
    }
  ...
  /**
   * Apply Yandex-specific anti-detection measures before navigating.
   * These run via evaluateOnNewDocument so they take effect before any page JS executes.
   */
  private async applyYandexAntiDetection(): Promise<void> {
    // Override navigator.webdriver to return undefined (not just delete it)
    ...
    // Simulate realistic mouse movements to pass behavioral analysis
    await this.page.evaluateOnNewDocument(() => {
      // Polyfill for EventSource to look more like a real browser
      const originalDateNow = Date.now;
      let offset = 0;
      Date.now = function () {
        return originalDateNow() + offset;
      };
      // Small offset to slightly jitter timestamps
      offset = Math.floor(Math.random() * 3);
    });

    // Set realistic navigator properties that Yandex checks
    await this.page.evaluateOnNewDocument(() => {
      // Ensure navigator.platform looks correct
      const platform = navigator.platform || "Win32";
      Object.defineProperty(navigator, "platform", {
        get: () => platform,
      });

      // Ensure hardwareConcurrency looks realistic
      Object.defineProperty(navigator, "hardwareConcurrency", {
        get: () => 8,
        configurable: true,
      });

      // Ensure deviceMemory looks realistic
      Object.defineProperty(navigator, "deviceMemory", {
        get: () => 8,
        configurable: true,
      });

      // maxTouchPoints should be 0 for desktop
    ...
    // Fix the CDP (Chrome DevTools Protocol) detection vector
    await this.page.evaluateOnNewDocument(() => {
      // Hide that the page is being controlled by CDP
    ...
    // Simulate realistic mouse position before page loads
    const viewport = this.page.viewport();
    if (viewport) {
      const startX =
        Math.floor(Math.random() * viewport.width * 0.5) +
        viewport.width * 0.25;
      const startY =
        Math.floor(Math.random() * viewport.height * 0.3) +
        viewport.height * 0.3;
  ...
  async load_start_page(): Promise<boolean | void> {
    // Apply page-level anti-detection BEFORE navigation
    await this.applyYandexAntiDetection();
    ...
    try {
      ...
      if (this.last_response) {
        ...
        if (status !== 200) {
          this.logger.warn(
            `Unexpected status code: ${status} for URL: ${startUrl}`
          );
        }
      } else {
    ...
    throw new CustomError(
      "No search input found with any of the common selectors",
      202405301120304
    );
  }
  ...
  async search_keyword(keyword: string) {
        ...
        if (input) {
          ...
          if (inputBox) {
            ...
            try {
              ...
              this.logger.info(
                `Successfully navigated after searching for keyword: ${keyword}`
              );
            } catch (navError) {
              this.logger.warn(
                `Navigation timeout after search, trying form submit. Error: ${
    ...
    try {
      ...
      const pageTitle = await this.page.title();
      this.logger.error(
        `Failed to find search input. Current URL: ${currentUrl}, Page title: ${pageTitle}`
      );
    } catch (pageError) {
      this.logger.error(
        `Could not get page state: ${
    ...
    const recovery = await this.attemptAIRecovery(
      "search_input",
      `Input keyword button not found for keyword: ${keyword}`,
      this.searchSelectors,
      { keyword }
    );
    if (recovery.success) {
