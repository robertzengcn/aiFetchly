...
const PHONE_PATTERN =
  /\b\d{3}[-.\s]?\d{3}[-.\s]?\d{4}\b|\(\d{3}\)\s*\d{3}[-.\s]?\d{4}\b/g;
 ...
 */
async function scanHomepageForContactInfo(
  page: Page
...
async function extractWithAI(
  ...
  try {
    ...
    if (response.status && response.data) {
      ...
      if (
        aiData.emails.length > 0 ||
        aiData.phones.length > 0 ||
        aiData.address
      ) {
        return {
          success: true,
    ...
    return {
      success: false,
      error: "AI extraction found no contact info",
      method: "failed",
    };
  } catch (error) {
 ...
 */
export async function discoverAndExtractContactInfo(
  url: string
): Promise<ExtractionResult> {
  // Validate URL before proceeding. F7 fix — use the async validator so we
  // also reject hosts whose DNS records resolve to loopback / link-local /
  // RFC1918 / cloud-metadata ranges (defends against DNS rebinding to the
  // desktop host's internal services).
  if (!(await validateUrlAsync(url))) {
    return {
      success: false,
      error: `Invalid or blocked URL: ${url}`,
      method: "failed",
    };
  }
    ...
    if (aiAvailable) {
      ...
      if (stage1Result.success && stage1Result.data) {
        ...
        const hasSocialLinks =
          !!stage1Result.data.socialLinks &&
          stage1Result.data.socialLinks.length > 0;
...
/**
 * Validate URL format and destination.
 *
 * F7 fix — the previous implementation only checked the syntactic http/https
 * scheme. The contact-extraction worker navigates Puppeteer to the URL and
 * pipes rendered content into the AI pipeline, so an attacker-controlled
 * link can drive the desktop host to fetch internal HTTP(S) services
 * (admin consoles, link-local metadata endpoints, RFC1918 services).
 *
 * The synchronous variant performs structural checks only (scheme, host
 * literal, userinfo). Callers that are about to navigate must use
 * validateUrlAsync which adds DNS-range resolution.
 */
export function validateUrl(url: string): boolean {
  return UrlGuard.validate(url).safe;
}
