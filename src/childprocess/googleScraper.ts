...
export class GoogleScraper extends SearchScrape {
    ...
    if (!findelement) {
      ...
      if (recoveryResult.success) {
        ...
        const networkIdleTimeout =
          this.config.ai_recovery?.networkIdleTimeoutMs || 30000;
        const recoveryDelay = this.config.ai_recovery?.recoveryDelayMs || 2000;

        await this.page
          .waitForNetworkIdle({ timeout: networkIdleTimeout })
          .catch(() => {
        ...
        for (const selector of alternativeSelectors) {
          ...
          if (results) {
            ...
            this.logger.info(
              "AI recovery successful, found results after recovery"
            );
            return result;
          }
        }
        ...
        if (input) {
          ...
          if (tagName === "textarea") {
            ...
            try {
              ...
              // If navigation doesn't happen, submit form by selector (avoid stale handle;
              // input.evaluate(el => ...) can throw "different JavaScript world" after timeout)
              await this.page.evaluate((sel: string) => {
                const el = document.querySelector(sel);
                if (el) {
                  const form = (el as HTMLElement).closest(
                    "form"
            ...
            // Handle input elements: prefer mouse movement if we have a valid box; else focus+type+Enter
            const inputBox = await input.boundingBox();
            if (!inputBox) {
              // Element not visible or detached (e.g. duplicate/hidden input); use keyboard path
              await input.focus();
              await this.page.keyboard.type(keyword, {
                delay: Math.random() * 100 + 50,
              });
          ...
          throw new CustomError(
            "input keyword button not found",
            202405301120303
          );
        }
      } catch (error) {
  ...
  async next_page(): Promise<boolean | void> {
    // Use phased, simulated manual scrolling to reach the bottom of the page
    const viewportHeight = await this.page.evaluate(() => window.innerHeight);
    const maxSteps = 50;
    let steps = 0;
    let reachedBottom = false;
    while (!reachedBottom && steps < maxSteps) {
      const stepPx = Math.floor(
        viewportHeight * (0.5 + Math.random() * 0.2) + Math.random() * 50
      );
      reachedBottom = await this.page.evaluate((step: number) => {
        ...
        const atBottom =
          after + window.innerHeight >= scrollHeight - 10 || after === before;
        return atBottom;
      }, stepPx);
      if (!reachedBottom) {
        await new Promise((r) =>
          setTimeout(r, 80 + Math.floor(Math.random() * 120))
        );
      }
      steps += 1;
    }
    ...
    if (recoveryResult.success) {
      ...
      console.log(`recoveryResult failed: ${recoveryResult.error}`);
      return false;
    }
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          process.env.TWOCAPTCHA_TOKEN.trim() !== ""
        ) {
          ...
          if (this.config.headless === false) {
            this.logger.info(
              `Browser is not headless. Waiting for manual captcha solving...`
            );

            //await this.sleep(this.SOLVE_CAPTCHA_TIME);
            this.logger.info(
              `You have ${this.SOLVE_CAPTCHA_TIME}ms to solve the captcha manually.`
            );
            await this.page
              .waitForNavigation({
              ...
              .catch(() => {
                this.logger.warn(
                  "Navigation timeout while waiting for captcha solution"
                );
              });
          } else {
            throw new CustomError(
              "Google detected unusual traffic and browser is in headless mode,but not captach service provided",
              202405301120306
            );
          }
        }
      ...
      throw new CustomError(
        "No search results found - possible detection or page load failure",
        202405301120304
      );
    } catch (error) {
      if (
