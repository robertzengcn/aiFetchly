...
export function registerProxyCommands(parent: Command): void {
  ...
  proxy
    ...
    .action(async (id, opts) => {
      try {
        ...
        formatItem(item as Record<string, unknown>, opts.json, 'proxy:detail', PROXY_DETAIL_FIELDS);
      } catch (error) {
  ...
  proxy
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        formatSuccess(`Proxy added with ID: ${saved.id}`, opts.json, 'proxy:add');
      } catch (error) {
  ...
  proxy
    ...
    .option('--json', 'Output as JSON')
    .action(async (id, opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('proxy:delete');
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ProxyEntity);
  ...
  proxy
    ...
    .description('Import proxies from file (one proxy per line: host:port:user:pass)')
    .option('--json', 'Output as JSON')
    .action(async (filePath, opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('proxy:import');
      try {
        await CliDatabase.ensureInitialized();
        ...
        const entities = lines.map((line: string) => {
          ...
          if (parts.length < 2) {
            throw new Error(`Invalid proxy format: ${line}. Expected host:port[:user[:pass]]`);
          }
          return repo.create({
            host: parts[0],
            port: parts[1],
  ...
  proxy
    ...
    .action(async (id, opts) => {
      try {
        ...
        const proxyData = item as Record<string, unknown>;
        const statusMessage = `Proxy ${proxyData.host}:${proxyData.port} - stored status: ${proxyData.country_code || 'unknown'}. ` +
