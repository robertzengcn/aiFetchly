...
import { formatPaginated, formatItem, formatSuccess, formatError } from '../output/formatter';
...
export function registerScheduleCommands(parent: Command): void {
  const schedule = parent.command('schedule').description('Manage scheduled tasks');
  ...
  schedule
    ...
    .action(async (id, opts) => {
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ScheduleTaskEntity);
        const item = await repo.findOne({ where: { id: parseInt(id) } as Record<string, unknown> });
        if (!item) {
          throw new Error(`Schedule not found: ${id}`);
        }
        formatItem(item as Record<string, unknown>, opts.json, 'schedule:detail', SCHEDULE_DETAIL_FIELDS);
      } catch (error) {
  ...
  schedule
    ...
    .requiredOption('--task-type <type>', 'Task type (search, email_extract, etc.)')
    .requiredOption('--task-id <id>', 'Task ID to schedule')
    .option('--json', 'Output as JSON')
    .action(async (opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('schedule:create');
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ScheduleTaskEntity);
        ...
        formatSuccess(`Schedule created with ID: ${saved.id}`, opts.json, 'schedule:create');
      } catch (error) {
  ...
  schedule
    ...
    .option('--json', 'Output as JSON')
    .action(async (id, opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('schedule:update');
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ScheduleTaskEntity);
        const existing = await repo.findOne({ where: { id: parseInt(id) } as Record<string, unknown> });
  ...
  schedule
    ...
    .option('--json', 'Output as JSON')
    .action(async (id, opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('schedule:delete');
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ScheduleTaskEntity);
  ...
  schedule
    ...
    .option('--json', 'Output as JSON')
    .action(async (id, opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('schedule:enable');
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ScheduleTaskEntity);
        const existing = await repo.findOne({ where: { id: parseInt(id) } as Record<string, unknown> });
  ...
  schedule
    ...
    .option('--json', 'Output as JSON')
    .action(async (id, opts) => {
      if (opts.parent?.parent?.readOnly) throw new ReadOnlyError('schedule:disable');
      try {
        await CliDatabase.ensureInitialized();
        const repo = CliDatabase.getRepository(ScheduleTaskEntity);
        const existing = await repo.findOne({ where: { id: parseInt(id) } as Record<string, unknown> });
    ...
    .action(async (scheduleId, opts) => {
      try {
        ...
        const [items, total] = await repo.findAndCount({
          where: { schedule_id: parseInt(scheduleId) } as Record<string, unknown>,
